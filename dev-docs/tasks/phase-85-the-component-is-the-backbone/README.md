...
| id | what | status |
|---|---|---|
| [CMP-001](CMP-001-THE-COMPONENT-INTERFACE-PLAYBOOK.md) | The ten interface patterns, the three grading floors, and the `States.currentState` defect that blocks the variant pattern | **AC1 ✅** the enum input is on the wire · **AC2 ✅ 2026-09-10 (s6)** — the ten patterns ship as `interfaceDoctrine` on `get_project_info` · **AC3 🟡 (s6)** — four examples built, one per pattern; the corpus-wide floors need ten more publishing components and four examples cannot supply them · **AC4 open** (needs CMP-002) |
