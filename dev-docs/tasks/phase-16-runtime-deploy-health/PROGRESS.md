...
|---|---|---|---|---|
| RUN-001 | Runtime React 17 → 19 | In progress (2026-07-24) | ↓ from 4–6 wks | Re-scope + slices 1–2 done, see [RUN-001-ASSESSMENT.md](./RUN-001-ASSESSMENT.md). Spec premise was stale: runtime ships vendored React **18.3.1** UMD (`1477a29` half-migrated it upstream); React 19's missing UMD build is the real problem. **Found + fixed a live P1 en route** (`0d075b2`): `1477a29`'s findDOMNode replacement never worked for any built-in component, so `setStyle`'s direct-DOM fast path (incl. transforms/animations) silently no-oped since Dec 2025 — fixed via the setDOMElement contract, live-verified in the preview. React 19 global bundles built + hooks-verified (`1c6790c`). Slice 3 done 2026-07-24: `runtimeVersion` now actually selects the React pair in editor preview, deploy, and noodl-preview (it was stamped but never consumed) — live-verified 19.0.0 vs 18.3.1 both ways in the preview webview and deployed folders. Cloud runtime React-free (excluded); SSR harness unwired (RUN-002's). Remaining: editor selection UI + migration scan (banner copy fix incl.), corpus 18-vs-19 pass, user docs |
