// Bundles BCN-004's live driver against the REAL `RestDataAdapter`.
//
...
await esbuild.build({
  entryPoints: [path.join(here, 'bcn-004-rest-driver.ts')],
  bundle: true,
  platform: 'node',
  format: 'cjs',
  target: 'node22',
  outfile: path.join(here, 'bcn-004-rest-driver.cjs'),
  logLevel: 'warning'
});
console.log('bundle OK → bcn-004-rest-driver.cjs');
