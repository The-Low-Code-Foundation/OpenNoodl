...
**Date:** 2026-07-25
**Status of task:** Step 1 (audit the existing infrastructure) **complete, with a recorded finding**. **Slice 1 (server-side SEO population) is now DONE** — SEO renders into the served `<head>` end-to-end, verified against the real SSR runtime bundle (see the §3.2 "Correction + FIX" box; the earlier "not cheap" reading was itself a misdiagnosis — the fix was a one-object-lifetime bug in `NoodlJSAPI`, not a router rewrite). Hydration (§3.1) remains a safe explicit `createRoot` with the finding documented; real hydration still needs synchronous-first-render. The busy-loop → `SSR_PageReady` swap, data resolution (§3.5), and the node server-compat crux (§3.3, Step 2) are the open work. Steps 5–8 not started.
...
1. **Server-side router/SEO population + the ready signal (§3.4, §3.2 producer side):** make the router complete its initial navigation server-side (it currently sits behind `typeof window` guards), which both populates `Noodl.SEO` for the (already-landed) head injection and unblocks data-dependent pages. Enable `SSR_PageReady`/`onPageReady`, replace the 1000-iteration busy loop. This is the prerequisite that turns the SEO consumer code into a real win.
