...
**Date:** 2026-07-25
**Status of task:** Step 1 (audit the existing infrastructure) **complete, with a recorded finding**. Two follow-on fixes were then *attempted and browser-verified* (hydration detection, SEO injection) — both proved to sit on deeper unfinished foundations; see the boxed updates in §3.1/§3.2. What actually landed: `renderDeployed` made a safe explicit `createRoot` with the finding documented, and a correct-but-currently-inert SEO head-injection helper + `data-ssr` root marker (the consumer side, waiting on the producer). Steps 2/5–8 not started.
