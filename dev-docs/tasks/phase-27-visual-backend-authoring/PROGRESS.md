...
| F57 | **A property-panel checkbox row did not respond to a dispatched click at its own box.** The Request node's *Allow Unauthenticated* toggle reports a real 32×19 rect and a real `<input type=checkbox>`, and `Input.dispatchMouseEvent` at its centre changed nothing (the parameter stayed unset). Not workflow-specific — `PropertyPanelCheckbox` is used by every node type — and not investigated here beyond confirming the click landed. Worth knowing before the next pass tries to drive one | `PropertyPanelCheckbox.module.scss`; observed live | **Closed 2026-08-02 — it was real, and it did not need the human click after all.** `document.elementFromPoint` at the toggle's own centre returned the **faux div**, not the input, and a trusted click there left `input.checked` unchanged — so the click was landing, on the wrong element. A regression from **PAR-002 (2026-07-26)**: the faux gained `position: relative` for its knob `::after`, which moved it into the positioned paint layer where, as the later sibling with no z-index, it covers the transparent input; there is no `<label>` and no `pointer-events` rule, so nothing forwards the click. **Every boolean in the node property editor and every settings toggle has been mouse-dead for a week** — keyboard-reachable, pointer-unreachable. Fixed with `pointer-events: none` on the visual, hover moved onto the input |
...
### Found by the 2026-08-02 live pass

Three more, all from running it rather than reading it.

| # | Finding | Where | Owner |
|---|---|---|---|
| F66 | **The editor spawns a prebuilt backend bundle and nothing rebuilds it on launch**, so every backend change is invisible to a dev-launched editor until someone runs the build by hand. Caught because WFA-007's own `POST /admin/workflow-defs/validate` answered *"Not found"* on a freshly started backend: `packages/nodegx-backend/dist/index.js` was a day old and predated the whole batch. `npm run build` in that package, then stop/start the backend, and the route answers. **This is a trap for every future backend live pass** — a change can look unshipped when it is merely unbuilt, and the failure is indistinguishable from a route that was never written | `packages/nodegx-backend/dist/`; the editor's backend spawn path | unowned — [OPEN-WORK.md](./OPEN-WORK.md) |
| F67 | **Accepting a proposal left the Workflows panel advertising it.** `onReject` called `context.onDone`; the accept path did not — so after *Accept all* the definition was written, the proposal file deleted, and the panel went on showing *"1 awaiting review"* with live buttons. Not cosmetic: its *Review on the canvas* silently reopens the copy still in memory, and accepting **that** writes the old candidate over whatever the workflow has become since — a write path fed by state the editor knows is gone | `models/workflow/workflowProposalReview.ts` | ✅ fixed 2026-08-02, verified live |
| F68 | **A proposal that omits a server-defaulted field reads as changing it.** The candidate staged in the live pass left `concurrency` out, and the review listed *"Changed metadata.concurrency: 1 → (unset)"* beside the real changes. The diff is faithful — the stored definition has `concurrency: 1` because the backend defaulted it — but an agent that round-trips a workflow without echoing server-defaulted fields will propose spurious changes, and a reviewer learns to skim them. Cosmetic today; it erodes exactly the trust the review surface exists to build | `models/workflow/workflowChangeSet.ts`; `WorkflowRegistry.normalize` | unowned — [OPEN-WORK.md](./OPEN-WORK.md) |

**WFA-007's own script, steps 1–4: pass.** A staged proposal appears as *Proposed for SQLite backend — 1 awaiting review* with the agent's note; **Review on the canvas** opens the diff document with **step cards, not red unknown nodes** — the one ordering this task depended on; `b` renders amber for Changed and the added `c` green, with Before/Changes/After and a walkthrough; **Accept all** persists to the backend and lands on the real workflow. `POST /admin/workflow-defs/validate` was also shown to be a true dry run — a candidate with an unknown kind came back `valid:false` with the kind list, and `wf_probe` never appeared in `/admin/workflow-defs`.

**Still not driven:** WFA-007's steps 5–9 (partial accept and its closure, the fix-request copy, the unregressed-AIX-003 check), its six Jasmine specs, and WFA-009's steps 5–8.
