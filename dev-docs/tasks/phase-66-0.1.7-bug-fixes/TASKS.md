...
| FIX-023 📋 | one typeless node kills the whole project's write surface | — | 1 | S+S | 📋 **RAISED s47, SCOPED s48** — `validate_project` and **every write** on `Puppy test 3` die with `io-error: Unexpected failure: Cannot read properties of undefined (reading 'startsWith')`. One node object with no `type` reaches `isComponentRef(type: string)`, which is `type.startsWith('/')` — **typed `string`, so no `tsc` gate can see it**. ✅ **Census before scoping: 28 v2 projects, exactly ONE carries such a node** (`Puppy test 3`, 1 of 119 node objects) — but it is the one Richard has a registered server for, so his own `nodegx-puppy-test-3` cannot author or validate. ✅ Reproduced s48 **on the packaged Aug-13 bundle**, so it is live on his machine. 🔴 **The guard exists in exactly ONE call site** (`ProjectStore.ts:429`) and five walkers have none — someone patched the line they stood on. 🔴 **The error naming neither node nor component is the bigger defect**: it turns a one-node problem into an unexplained dead server. **Not a regression — do not bisect.** |
