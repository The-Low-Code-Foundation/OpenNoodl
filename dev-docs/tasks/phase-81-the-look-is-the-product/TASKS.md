...
**Status legend**: ⬜ not started · 🟡 in progress · 🟢 done (WORTHY verdict recorded + Richard has seen it) · 🔴 blocked · ➡️ **carried to another phase — do not build it here**

⚠️ **Two rows left this phase on 2026-08-31.** VIB-005 and VIB-008 are now phase 82's REL-002a/b/c,
because 0.2.2's launch work was consolidated onto one board at Richard's request. Their close
condition is unchanged — this phase's §2 rubric and §3 protocol, restated verbatim in phase 82's
`TASKS.md`. **VIB-009 lists VIB-005 as a dependency; that dependency now resolves in phase 82.**
...
| VIB-005 | The Ambush Defaults | ➡️ **CARRIED to phase 82** | VIB-001 | 🔴 **Do not build this here.** Transferred 2026-08-31 by Richard's ruling that 0.2.2's launch work lives in one board: *"over several sessions all in phase 82."* It is now **REL-002a** in `phase-82-0.2.2-the-first-row-on-the-shelf/TASKS.md`, which restates the rubric and the close protocol in full. Its register rows (V1, V2, **V14** the `"Type here…"` default) went with it |
| VIB-006 | The Worked Page | 🟢 | VIB-002/003/004 | 🔴 **WORTHY — RICHARD RULED IT** 2026-08-31: *"It looks fucking pro, good job. VIB 006 screens are amazing"* (§13). **The phase's FIRST close on the look**, and README §3 is satisfied end to end. ⚠️ Read as a grade rather than as encouragement because of **the contrast inside his own sentence** — he called this *"fucking pro"* and VIB-011 *"passable"* in the same breath; the three earlier rulings were recorded PASSABLE precisely because they did not draw that distinction. ⚠️ **It closes the CORPUS seam, not the pipeline** — a page built by a session that had read the whole phase is no evidence about one that has read nothing. That is **VIB-010**. `docs/node-catalog/examples/ui-landing-page.json` ships: **14 components, 129 nodes, 8 bands, a 9-node page**, 7 distinct grounds, **8 distinct photographs**, three different faces, gated 67/67 strict. 🔴 **V8 re-derived before building and it held harder than the row said**: the most bands on any `Page` in the corpus was **2**, and **zero** examples had 3 bands with a picture and a glyph. 🔴 **`oversized-page` — an INFO that does not fail the run — named the better shape** (90-node page ⇒ eight section components). Four defects found by LOOKING that no gate could see (a nav that lost two links at 390); opened **V38** (`justifyContent` cannot centre a `Columns` child — three configurations, one rendering, so the parameter was DELETED rather than shipped inert) and **V39** (`ui-image-scrim-band`'s description still said its picture was empty). AC6 answers Richard's hero question. ⚠️ `typecheck:backend-tests` OOMs even narrowed to two files |
| VIB-007 | **The Loop** *(rescoped 2026-08-31; was "The Brief")* | 🟡 | VIB-001 | 🔴 **Rescoped by Richard's question about baking the magic in.** Was INSTRUCTION; is now **M1 mandatory render + M2 traps-as-diagnostics + M3 poverty findings** (`VIB-007-THE-LOOP.md`). The mapping that justifies it was run over all 41 register rows first: **18 of the 23 live rows (78%) fall out to a mechanism**, and **M2 alone covers 11**. 🔴 Instruction was measured and rejected as the lever: V17 is a session that shipped V1 **after reading V1**, and V35 leaves **1 token** of surface to instruct with 🟢 **AC2's first job DONE 2026-08-31: V22 RULED BY A RENDER and CLOSED.** An undeclared component port delivers **nothing** — declared arm 3/3 records' values, undeclared arm 3/3 its own placeholder, all four widths, and the repeater still draws the right NUMBER of rows. Shipped `undeclared-component-port` (ERROR) on **both** the corpus gate and the authoring door, repaired all 14 examples (16 ports), regenerated the enriched catalog and verified through the door. ⚠️ **AC2 still owes V23, V28, V29, V32, V33.** 🟢 **AC1 (M1) BUILT 2026-08-31 — `VIB-007-THE-LOOP.md` §7.** The render is mandatory and `done` is now a thing only a clean render can produce: a completion verdict on `render_report` / `apply_plan` / `create_component` / `update_component` / **`validate_project`**, expiring on a project content signature; `apply_plan` **refuses `render:"off"` on a visual plan, before any write**. 🔴 **It refuses to CERTIFY, never to write** — refusing the write was the first design and it destroys the very loop M1 exists to create. Demonstrated end to end on a deliberately poor page with a **real Chrome** (`tests/vib007-m1.door.ts`, 4/4): NOT DONE → named → fixed → **DONE**. ⚠️ **AC4/AC5 unstarted**, and **no register row closed by it** — the mapping's "would have prevented" is not "is now fixed". 🟢 **AC3 (M3) BUILT 2026-08-31 — §8, and V10 CLOSES.** Three poverty findings ship in `@nodegx/render-measure` — `no-imagery`, `no-display-type`, `single-ground` — each one of README §2's WordPress-starter tells verbatim, with `DISPLAY_TYPE_MIN_PX = 48` taken from the rubric and **checked against seven recorded builds by four authors, every one of which tops out at 48 or 60px** while the two SHITTY templates top out at 30. Both arms by a real Chrome: all three fire on the baseline door pages (8/8, 4/4 desktop, 6/8), **none** on the VIB-006 page. Needed two new measurements (`images.icons`, `grounds`) — the icon DOM was **probed, not inferred**, because the font branch carries no `ndl-icon-glyph` class. ⚠️ **`warning`, not `error` → V42**: app-chrome pages are exempt from marketing tells and the instrument cannot tell them apart, so M3 is **pressure, not a gate** — it rides on the `done: true` sentence and a `looksLike` block. 🔴 **Measuring the control found a defect that made the phase's only WORTHY page permanently uncertifiable**: a `StatTile` fallback of `"0"` and an instance legitimately setting `"0"` are the same picture, and `dead-placeholder-text` (ERROR) asserted the wrong one as fact. Fixed in `overriddenDefaults`; VIB-006 now renders **zero findings at both viewports** 🟢 **AC2's TAIL: FOUR OF FIVE BUILT 2026-08-31 — §9.** `raw-color-literal` switched on in `catalog:examples` (V32, one repair, ruled by a three-arm render that a token survives a `Color` node); `unsourced-image` on the corpus gate and the door (V33, with a gradient exemption the corpus's own prose asked for); the curated Lucide set widened 212 → 215 so the picker can offer every glyph the corpus teaches (V23); `raw-spacing-literal` (V28, one repair). 🔴 **Three of the five rows did not survive being measured, and two of the tabled predicates would have condemned CORRECT authoring** — V23's would have fired on the WORTHY page for glyphs that render, and V28's *"a `var()` in a units-typed port is dropped silently"* is **false**: a four-arm render reads the token's exact 64px. V28's *"30 raw pixel numbers"* re-derives to **1**. 🔴 **Two defects found in this session's own code by neighbouring specs**: the new spacing rule silently **downgraded** an existing `invalid-parameter-value` ERROR on `"16px"` to a warning, and its `COLUMNS_TYPE` guard was **dead on arrival**. 🟢 **AC2 CLOSED 2026-08-31 (session 11) — V29 BUILT, all six predicates ship — §10.** `unrealised-measure` (**warning**) on the authoring door. 🔴 **The row's shape has 13 corpus instances and ONE of them is the defect.** A render of all 7 examples carrying a capped `Text`, at 1280 and 1900, measured the painted extent of each band: **twelve read 374/374 and three of those are on `ui-landing-page`** — the page Richard ruled *"fucking pro"* — while `ui-image-scrim-band` reads **374/766**, the ruling's own numbers. A per-`Text` predicate would have condemned correct typography twelve times. 🔴 **The defect belongs to the SHELL, not the Text**: gradient-hero and scrim-band are the same recipe differing by ONE node, and gradient-hero's uncapped `eyebrow` paints the full 1152px measure. Fires **exactly once** across 67 examples; door probed on both arms (defect 1, control 0); 18 tests, mostly named controls, with a mutation on the control. ⚠️ **Not in `catalog:examples`** — the one hit is VIB-002's recipe, owned by VIB-008 → **V45**. **AC4/AC5 unstarted.** Opened V43, V44, V45 |
| VIB-008 | The Members' Area, Redeemed | ➡️ **CARRIED to phase 82** | VIB-002/003/004/005 | 🔴 **Do not build this here.** Transferred 2026-08-31, same ruling as VIB-005. It is now **REL-002b** (V3 fail-open gating + V4 first-run state — *correctness*) and **REL-002c** (the redesign, V15/V29) in phase 82. **Its WORTHY ruling is 0.2.2's close condition**, so phase 82 owns the screenshot and Richard's look. Register rows V3, V4, V15 and V29's open half went with it |
| VIB-009 | The Site Builder, Redeemed | ⬜ | VIB-002/003/004/005 | — |
| VIB-010 | The Cold Proof | ⬜ | VIB-006, VIB-007, VIB-013 | — ⚠️ **VIB-006 is now met**; it waits on VIB-007 and VIB-013 |
| VIB-013 | **The Altitude** *(new 2026-08-31)* | ⬜ | VIB-004, VIB-006 | 🔴 **M4 a section expander + M5 a corpus generated from the compositions** (`VIB-013-THE-ALTITUDE.md`). The evidence is VIB-006's own generator: the worked page is good partly **because 129 nodes were not hand-written** — the compositions were declared once and composed, and every defect the renders caught was in the ~10% written OUTSIDE that vocabulary. ⚠️ **M5 retires 5 rows and none is still open** — it is prevention, not cleanup, so it goes last and must not delay M4 |
