...
| VIB-011 | The Stock Library | 🟡 | VIB-003 | **PASSABLE 2026-08-31** (`VIB-011-THE-STOCK-LIBRARY.md` §12), provisional until Richard looks. **44 CC0 photographs, 3.32 MB**, shipped in `STARTER_ASSETS` with `LICENCES.json` beside them; `get_style_vocabulary` gains an `imagery` block; doctrine §5 rewritten; `ui-split-hero` and `ui-testimonial-row` repointed at real pictures and the page re-rendered. 🔴 **The row's own licence table was wrong** — the collection restriction is in the *current* Unsplash Licence, not in the pre-2017 **CC0** grant, and `Category:Images from Unsplash` (31,004 files) is what fixed `people`. Closed V30, V33, V34; opened **V35** (the tool surface has 1 token left). 🔴 **WORTHY gap: the library ships and the page barely uses it** — one band of six carries a photograph and the headline is still on bare white → **VIB-006** |
