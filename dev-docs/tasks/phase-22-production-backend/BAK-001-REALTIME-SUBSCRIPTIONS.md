...
- Auth: the SSE connection carries the session token (query param — `EventSource` cannot set headers — accept and document it) or bearer token. Delivery-time permission checks: **BAK-003 already landed**, so this is no longer a placeholder to flip — import `resolvePrincipal(headers)` and `canReadRecord(principal, record)` from `packages/nodegx-backend/src/security/model.ts` and call `canReadRecord` per event per subscriber; gate subscription *creation* on the collection's `find` CLP (`checkClp`). `canReadRecord` is the JS twin of the SQL query filter, property-tested against it (`nodegx-backend/tests/security-model.test.ts`) so a record visible to a query is deliverable over realtime and vice-versa. Do **not** re-implement ACL logic here.
