...
| 4 | uploadable templates, ratings, categories, search | **FB-005** | 🟡 **T1+T2+T3 done (s40/s42/s43).** ~~no template picker exists in the product at all~~ — **there is one now**: a fourth wizard mode, a `community://` provider, and `templateRegistry.list()`'s first caller. ⚠️ **The shelf is empty and the platform is undeployed**, so the curated half has a mechanism and no content. T4 (categories+search), T5 (submissions), T6 (ratings, 🔒 needs a ruling) remain. ECO-002 stays gated; UNI-005's shelf stays org-scoped |
