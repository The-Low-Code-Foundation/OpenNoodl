...
- 🟡 **FB-005** — templates, **curated first** (L+) — **T1+T2 done, T3 next** — **R-templates ruled 08-22**: share files a
  ...
  **chain** rather than the registry. 🔴 **The root cause was a contract, not a bug**:
  ...
  applied migrations. ⚠️ **T3–T6 remain**; the shelf holds **no template yet** — content, not
  schema. T4 also waited on the search item — ✅ **UNBLOCKED**, see FB-024 (and note the item's premise was wrong: the AND was in a function nothing calls)
