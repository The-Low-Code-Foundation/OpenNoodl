...
- 🟢 **FB-014** — [search that survives renames](FB-014-SEARCH-THAT-SURVIVES-RENAMES.md) (M) — **DESIGN PHASE DONE 2026-08-25 (s38), [FB-014-DESIGN.md](FB-014-DESIGN.md); AC1+AC3 met, AC2 impossible.** 🔴 **The bench holds 3 posts** (2 threads, 1,369 chars, read off nexus-1) — *"a copy of real bench data"* does not exist, so the mechanism was measured against **real pre-rename product prose recovered from git** instead: the node catalog at `1f31d24f^`, 155 documents, and **29 real renames** mined from its own 98-commit history (`Success→Done` across ~15 nodes, `Logic Builder→Visual Function`). 🔴 **The finding is about the search we already ship**: with an old-vocabulary control holding everything else constant, the renames cost it **73% → 18%** recall on keyword queries; vectors recover it to **91% @10**, and **hybrid beats vector-only on the control** (17/22 vs 11/22 @1), so RRF is the answer rather than a hedge. ✅ **pgvector needs no third-party repo on prod** — `postgresql-16-pgvector` 0.6.0 is already a candidate from `noble/universe` on the Hetzner mirror; the whole DDL was **replayed on pgvector 0.5.1** to prove nothing postdates it. ✅ **`all-minilm` (384d, 45 MB) beat `nomic-embed-text` (768d) by 32 points** — but only after the harness was fixed to send nomic's **required task prefix**, without which I had measured my own omission and nearly filed it as "the bigger model is worse". 🔴 **Chunking is mandatory for a non-obvious reason**: whole-document embedding fails on 6/155 at **1070–2193 chars while a 6000-char document passes** — the window is spent in **tokens**, and two of the six were ground-truth documents, so dropping them silently would have scored the retriever on a corpus with the answers removed. 🔴 **Two of my own measurements were fiction before they were fixed** — a 100k-row latency table that Postgres hoisted into **100,000 copies of one vector** (`count(distinct)` = 1; the honest HNSW figure is **5.5× higher**, 2.93 ms median), and an index build that **exited 0 twice without building anything** while `EXPLAIN` said *Parallel Seq Scan*. ⛔ **Recommendation: build behind a flag, do not launch** — the trigger is content, not code. 🧭 **AC3 is with Richard**: local model vs hosted embeddings API, and it is **not a cost question** (single-digit dollars at 100k posts either way) — it is whether user-written posts go to a new external processor. Also found, unowned: **`websearch_to_tsquery` ANDs bare terms**, so a conversational query matches **2/22 even in perfect vocabulary**, and **`Set Record Properties→Update Record` collides** with `noodl.byob.UpdateRecord`.
