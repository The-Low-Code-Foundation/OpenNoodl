...
| PLAT-002 | Retire the jQuery islands | In progress (2026-07-24) | 6–8 wks | 547 → 262 `$(` calls (52 files). Dead code deleted; all DataTypes rows + composite widgets (align/resizing/margin-padding/stringlist/proplist) + font/image/identifier/file pickers converted to React (waves 1b–2e). Shared: ContentPicker, PickerTypeView popout plumbing, fontItems. Side catch: fixed pre-existing core-ui mount-commit bug that corrupted app undo whenever the property panel was open. Next: NOTES §8 handoff (wave 2f hosts — TypeView/Ports/propertyeditor; then canvas, PopupLayer) |
