...
| ~~**L12**~~ | ~~**The doctrine half of this task (§5) never shipped.**~~ ❌ **WRONG, and it was the recommended next move in two handovers.** It shipped in **`a4793530`** (2026-08-11 23:20:21 +0200) — `authoring.ts`'s AUTHORING CONTRACT (7 lines, the `submit_component` door) *and* `DECOMPOSITION_DOCTRINE_MD`'s **"Say why on the node"** bullet. The grep behind L12 looked in `AUTHORING_TRAPS` and `dev-docs/best-practices/`, which are the two places that commit **deliberately** did not touch | ❌ withdrawn 2026-08-12 |
| **L13** | 🔴 **The zero in L10 was measured with BOTH halves shipped, and the doctrine was at turn 1.** The run started `2026-08-11T22:12:31Z` — **52 minutes after** `a4793530`. The bullet is in the transcript verbatim at record 3, the `get_project_info` result of **turn 1**: the first thing the model read. So the field was declared in the schema, described in one clear sentence, *and* taught in prose on the **pushed** channel — the channel LAS-007 measured as the one that works — and it still produced **0 of 182**. **There is no cheap untried documentation lever.** What is genuinely absent is `AUTHORING_TRAPS` (excluded on purpose: every line there is a measured *silent failure*, and a missing comment fails nothing) and `dev-docs/best-practices/` — the **pull** channel, which LAS-007 measured at **zero** retrievals in 42 turns, so shipping there is predictably inert for this measurement | ✅ verified 2026-08-12 |
| **L14** | The two surviving hypotheses are L10's "further readings", and the **fixture** one is now the stronger: a label names what is in front of you, a comment needs a constraint the brief never stated, and the storefront brief states almost none. Salience is correspondingly weaker — the doctrine did not arrive buried in a 29,477-char schema, it arrived unasked at turn 1. **Before any further spend, the brief must contain external constraints**, or the fixture cannot measure the field at all | 📋 open |
