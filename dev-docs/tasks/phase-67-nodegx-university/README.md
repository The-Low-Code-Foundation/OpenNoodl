...
|---|---|---|---|
| **E1** | ✅ **UNI-001's issuer — BUILT 2026-08-19 (session 29).** Sessions are minted; GitHub OAuth in the browser; a device flow for the editor; sign-out revokes. ✅ **AC2's launcher card + chip BUILT 2026-08-18 (session 33)** and driven in a real editor — sign-out deletes the store file; a sign-in click reports the unreachable platform rather than hanging | task | ⚠️ **Code-complete, and NOBODY CAN SIGN IN YET** — the start route answers 503 without `GITHUB_OAUTH_CLIENT_ID`. **That is E10, and it is Richard's.** ✅ **AC4 SETTLED 2026-08-18 (session 34): the criterion was re-worded to the property it meant and is now a GATE** — `tests-unit/uni-001/session-readers.test.ts`, a recorded reader set walked from disk plus "no capability inside a session branch", proven to fail before it was believed. 🔴 **There is now NO editor code item left in E1** |
| **E2** | ✅ **BUILT 2026-08-18 (session 35), NOT YET RUN** — `nodegx-community/ops/`: `provision.sh` (Node, Postgres, role + database, systemd unit, **Caddy drop-in validated before reload and backed out if invalid**, daily `pg_dump` with 14-day retention) and `deploy.sh` (four secret refusals + an `https`-only `SITE_ORIGIN` check **on the laptop before a byte moves**, rsync, build on host, **migrate**, restart, verify, **all three neighbours curled before and after with a non-zero exit if any moves**). 🔴 **D20 amends D1's "Docker"** — systemd + apt Postgres, because nexus-1's Caddy is all-or-nothing and three live sites are not ours to break | ⚠️ **Owned by no task; built at the close** | ✅ **E5 answered: nexus-1.** `community.nodegx.io` already resolves to `49.12.102.195` — **no DNS step**. 🔴 **Uncovered a defect with nothing to do with deployment: the migrations could not be run twice** — 155 `create` statements, zero `if not exists`, so `applySchema` errors partway and **`resetSchema` succeeds and destroys every account**. Now a ledger with checksums, 9 specs, **both controls** |
...
| # | What | Blocks |
|---|---|---|
| **E5** | ✅ **ANSWERED 2026-08-18 — `nexus-1`, additively.** 🔴 Against the recommendation (a separate box), recorded as **D20** | ✅ unblocked E2, which was built the same session |
| **E6** | ✅ **ANSWERED + BUILT 2026-08-18 — Brevo.** Not a fresh choice: `nodegx.io`'s waitlist has run on that account for months **from this same host**, so the key exists and the source IP is already authorised. `src/lib/mail/brevo.ts` + 16 specs; `defaultTransport()` now has **three** states, and the third is the point — a secure origin with no key returns a transport that **throws**, so mail **waits** and is not silently marked sent. Migration `0012` moves the sending domain off the never-registered `mail.nodegx.dev` | ⚠️ **Richard still owes one thing: verify `mail.nodegx.io` in Brevo.** The envelope is checked by a Postgres trigger, so a mismatch refuses the row |
| **E7** | ✅ **ANSWERED 2026-08-18 — Hetzner Object Storage.** 🔴 **The thing it was blocking needs no code**: UNI-020's download button renders when `articles.project_url` is set, that column exists, and a public object URL can be written into it | The **capture upload path** is still owed and is genuinely blocked on S3 credentials only Richard can mint — **scoped into phase 67b**, not into this close |
