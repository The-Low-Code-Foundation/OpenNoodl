...
| ⚠️ **The MCP tool-surface budget** | UNI-010's `lesson` group spent 56 of LEG-001's 58 banked tokens and renegotiated the bar 8,200 → **8,280** ([`toolDisclosure.test.ts`](../../../packages/noodl-mcp/tests/toolDisclosure.test.ts)), whose note reads *"there should not be a third."* **Any further MCP surface this phase adds — a `derive_starter`, a lesson tool — is now competing with phase 69 for 57 tokens**, and phase 67 is the reason the margin is thin | **CN-009**, **CN-006** |
