...
| [UNI-010](UNI-010-A-TUTORIAL-YOUR-OWN-CLAUDE-CAN-WRITE.md) | the user's own Claude authors a verified lesson into the Learning folder — an experiment with pre-registered kill/keep criteria | editor + MCP | experiment | M | 🟡 **SLICE 1 BUILT** (08-15, `90776d8b`) — the **F1–F4 harness**, LEARN-009's own deliverable: a file-backed eval context (no editor), solution replay, §3.5's ghostwriting check, decoy injection, engine 2 as a port. **46 tests** in `test:main`. ✅ **Criterion 1 met for all four machine-detectable classes.** 🔴 The bundle format now carries its own **`solution/`** — without it three classes go unchecked, hence `installable` ≠ `ok`. 🔴 Building the caller found that the decoy test alone **fails every sound lesson**, so F3 is now error-vs-warning on whether a second candidate actually exists. ⚠️ **Not built:** the `create_lesson` MCP surface and the install that runs this scorecard — `LearningFolderModel.install()` still gates on **F1 alone** |
