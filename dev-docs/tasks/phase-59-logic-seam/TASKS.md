...
| LGC-008 | [LGC-008-A-PANE-NOT-A-TAKEOVER.md](LGC-008-A-PANE-NOT-A-TAKEOVER.md) | the workspace stops hiding the whole canvas; blocks beside the running app | ✅ **BUILT, MERGED AND DRIVEN 2026-08-12.** F3, F4 and the pane, in that order. **The acceptance criterion is met on a drive**: a splitter drag 847→1077 px left both Blockly workspace object ids identical — no remount — and the canvas re-measured 1036→1496 with it. **F4 proved on two mounted workspaces**: a background edit landed on its own node while the active node's `workspace` and `generatedCode` were untouched. 🔴 **Two things left:** the pane has **no minimum width** (at 288 px the rails alone are 304 and the workspace measures 0 — also answers LGC-004 #14), and **L30 keystroke ownership blocks shipping** (one Delete, two selections). §2's A/B still needs humans. Earlier: ✅ **F2 RULED: both panes on screen at once.** 🔴 The ruling took the expensive branch: **F3** (a hidden canvas measures 0×0 and nothing re-binds) becomes **every drag** and must be built first, and **F4**'s remount trap — `handleWorkspaceChange` writes to the *render's* `activeTab` — becomes a live program-eating bug the moment two workspaces are mounted at once |
