...
|---|---|---|---|
| LGC-001 ⭐ | [LGC-001-THE-LOGIC-TRIAD.md](LGC-001-THE-LOGIC-TRIAD.md) | **the flagship** — the picker explains Expression vs Function vs Visual Function, with examples, and answers `multiply` | ✅ **§1 + §4 DRIVEN 2026-08-12**: `multiply` returns exactly the three, in the specified order (Expression → Visual Function → Function), each with `tag · multiply`; **all three preview rows** match the chooser copy, Function's including D1's word `several`. §3/§5 confirmed in passing. Remaining: the jasmine suite, and the picker below 760 px. ✅ **L7 closed `144f1a0d`** — ruled: rename the sub-category, keep the rail label; `Logic & Utilities`' sub-category is now **`Conditions & Booleans`**. ✅ §5 confirmed on a drive: the rail reads **Logic** |
...
| LGC-004 | [LGC-004-INTERFACE-RAILS.md](LGC-004-INTERFACE-RAILS.md) | pin the signature to the workspace edges; the props panel edits blocks rather than shadowing them | 🔨 **§1 + §4 done 2026-08-12** — both rails built, rows derived from a new `detectInterface` projection of `detectIO`'s own traversal, **no second store, proved red**. 72 specs. 🔴 **§2/§3's panel NOT built** (a concurrent session owned `PortsTab/`) — the design is written and it **corrects §2's table in three places**, one of which decides the architecture (**L38**: a panel edit made while the block tab is open is silently destroyed). ✅ **DRIVEN 2026-08-12 — 11 of 15 run, 9 clean passes.** Rows/zoom/pan/rename/empty-copy/click-and-drag-create all pass, and **step 13's byte-identical check passes** (776 bytes either side). 🔴 **Three findings:** a row released **over the toolbox creates a block** (outside the pane correctly cancels); the dragged-out block is **not selected**; and the measured contrast **fails AA** — the 10 px type label scores **3.66** dark / **3.43** light, signal type **4.33** light, against 4.5. Steps 10 and 14 not run (14 needs LGC-008's splitter) |
...
| LGC-008 | [LGC-008-A-PANE-NOT-A-TAKEOVER.md](LGC-008-A-PANE-NOT-A-TAKEOVER.md) | the workspace stops hiding the whole canvas; blocks beside the running app | ⚰️ **SUPERSEDED BY LGC-010 on 2026-08-13.** Built, merged and driven 2026-08-12, and then *used*: on a 13" MacBook with the preview open the pane gets ~494 px, the two interface rails are **304**, and the blocks get ~190. Its own drive had already filed "no minimum width — at 288 px `.injectionDiv` measures **0**" and offered a clamp; a clamp cannot fix a surface whose minimum is most of the screen. 🔴 **Reverses ruling 1 of 2026-08-12 — do not re-litigate it back.** What it built that survives: F4 (an edit belongs to the tab that made it), the retired blanket `display: none`, L27's `z-index: 7`, and both re-measure seams. Implementation archived at [`dev-docs/archive/lgc-008-split-pane/`](../../archive/lgc-008-split-pane/) against a future multiple-canvas layout |
| LGC-010 | [LGC-010-A-WINDOW-NOT-A-PANE.md](LGC-010-A-WINDOW-NOT-A-PANE.md) | the Logic Builder floats **over** the document instead of taking a column of it | ✅ **BUILT AND DRIVEN 2026-08-13, every number measured over CDP.** `position: fixed` escapes the node-graph frame (window at x=123 against a frame at x=380), `.injectionDiv` **816 px vs the pane's ~190** (4.3×), the canvas is **not narrowed** (988 px, full width), an outside click reaches the preview and **does not dismiss** the window, drag and all eight resize handles work with Blockly's SVG tracking the container, and the floor clamps at **640 px with a 334 px workspace** — LGC-008's open finding closed by construction. 🔴 **L30 fixed and proved with a negative control** (node selected + focus inside → survives; focus outside → deleted), which unblocks shipping. Found by driving: **`overflow: hidden` clips hit-testing**, so the first handles were half unreachable. ⚠️ Still open: §2's A/B (needs humans), `detachedPreview`, and the two zoom clusters |
| LGC-009 | [LGC-009-A-HAT-FOR-THE-LANGUAGE.md](LGC-009-A-HAT-FOR-THE-LANGUAGE.md) | give block programs a stated beginning — *"when Run is received"* | ✅ **BUILT AND MERGED 2026-08-12**, and **both fixtures migrated** (dry-run first; 2 workspaces changed — exactly the ruled cost — `generatedCode` untouched). One hat type **parameterised by signal name**, so one hat or several. 🔴 **The `detectIO` clash is answered structurally, not by precedence**: the hat is a *mention* on the same single traversal, both projections key on the name, and — the reason it cannot land on **L39** — **a signal port has no type**, so there is no fact for two declarations to disagree about. Asserted in **both document orders**. ✅ criteria 1–3; 🔴 **criterion 4 NOT claimed** — nothing registers `disableOrphans` yet. 🔴 Two pre-existing defects found by measurement: `withBlockProbes` painted a chain-head declaration hollow on every run, and `classifyBlockForDoIt` refused **any** disabled block, which would have taken Do It out with `disableOrphans`. Earlier: ✅ **RULED: the hat is MANDATORY**, so `disableOrphans` becomes correct and **LGC-003 §2 is bought back**. The grammar has **9 statement / 6 value / 0 hat** blocks. 🔴 Do **not** re-derive the cost — the migration is **two fixtures, both ours**, not a fleet; that number was wrong once and nearly decided the ruling the other way. 🔴 Open and now urgent: a hat naming a signal is a **second declaration** of a port `noodl_define_signal_input` already declares, landing straight on **L39** (`detectIO` resolves clashes by *document order*). Settle that before writing the generator. One-hat-vs-several is unruled but does not block starting |
