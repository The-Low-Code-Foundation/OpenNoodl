...
| LGC-007 ⭐ | [LGC-007-MY-BLOCKS.md](LGC-007-MY-BLOCKS.md) | **the second flagship** — save a group of blocks, reuse it in any Visual Function | ✅ **§6 DRIVEN 2026-08-12: 2 of 3 pass.** Workspace stays live, console names the loop — 🔴 **but `generatedCode` is emptied to `""` and written to disk.** Same shape as the `disableOrphans` defect: a refusal to generate publishes its silence over the last-known-good code. 🚧 **engine built 2026-08-12, no UI** — format, two shelves, cycle guard, shape inference, inliner and export/import, 66 specs in a plain-Node runner. 🔴 **No save menu item, no dialog, no backpack, no §4 sweep, and nothing run in an editor.** 🔴 **Resized by LGC-006**: the two npm packages do **not** do most of it |
