...
| L7 | 🔴 **`Logic` now collides with an existing sub-category.** `Logic & Utilities` has a sub-category called `Logic` (Boolean To String, Switch, And, Or, Condition, Inverter), which is the *group heading* in browse mode. The picker therefore shows a rail entry and an unrelated group heading with the same word. One-line fix either way — revert the rail label, or rename the sub-category (`Booleans & Branching` is accurate for its six) | ✅ **RULED and FIXED 2026-08-12.** Richard ruled: **rename the sub-category, keep the rail label.** The rail names a choice a builder makes; the sub-category names six boolean and branching primitives, and is the one that can move without losing anything. It is now **`Conditions & Booleans`** in `nodelibraryexport.ts`. Display string only — no `category` field, nothing saved, not in `node-catalog.json` |
