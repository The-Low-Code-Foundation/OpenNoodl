...
| 🔴 **[FH-024](FH-024-THE-LOCAL-ADMIN-API-IS-CROSS-ORIGIN-READABLE.md) — any web page can read a developer's local backend admin API** | **Filed 2026-08-06, needs a decision, candidate alpha-blocker.** Three defaults compose: `devOpen` defaults **true** (`security/model.ts:202`), `devOpenActive` makes **every** admin gate return early with no token checked (`HttpServer.ts:1554`), and CORS defaults to `origins: ['*']` applied **before routing** (`ops/model.ts:176`, `HttpServer.ts:1149`). So `GET http://127.0.0.1:<port>/admin/*` is unauthenticated *and* answers with `Access-Control-Allow-Origin: *` — a simple cross-origin request whose body a page may read. **The deploy interlock is fine and a deployed backend is not affected**; the error is one layer up, treating `loopback` as "only the developer can reach this" when a browser reaches `127.0.0.1` on any page's behalf. **This repo already shipped this exact bug once — OBS-004, "the relay was readable by any web page".** ⚠️ **F10 made it far more reachable today**: the backend now starts automatically on project open, so the surface is live all session. Exposed: collection + function permissions, rate limits, timeouts, roles, users, ops config, execution history, data browser rows. CWF-009's secrets route is safe *only* because it returns names and never a value — do not read that as evidence the others are. ⚠️ **Confirmed by construction, NOT yet driven** — slice 0 is firing a real cross-origin `fetch` at a running backend, because a threat model read off source proves what the source says, not what the browser does. |
...
| 5 | [CWF-004](CWF-004-THE-TRANSFORM-STEP.md) — Transform + data steps | **Slice 1 SHIPPED 2026-08-06.** Rewritten 2026-08-05 on Richard's argument: without a workflow-level reshape, every function carries its caller's mess. The `transform` kind, its closed served vocabulary and its property row are in; the rest of the family (Validate, Filter, Split, Sort, Dedupe, Parse JSON) is a menu to pick from, and S6's "new function from this step" gesture is unbuilt. A free Function step remains advised against. |
