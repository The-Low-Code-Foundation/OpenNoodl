...
| 🆕 **The editor's own AI write path allocates node ids the way `noodl-mcp` used to** — ⚠️ **confirmed at the line, CONSEQUENCE UNVERIFIED** | Found by F12's agent while fixing the MCP side. [`candidate.ts:151`](../../../packages/noodl-editor/src/editor/src/models/AiAssistant/authoring/candidate.ts#L151) is `n.id ?? newId()` with no check against ids the project already uses — the same shape F12 closed. **What is NOT established, by anyone: whether it is a live defect.** The editor's apply path may rekey downstream — `rekeyAllIds()` is called by `import-engine/apply.ts:61`, `projectmodel.ts:341`, `RouterAdapter.ts:218` and `ComponentTemplates.ts` — and if the AI candidate reaches one of those, there is no bug here at all. **Establish that before fixing anything.** Deliberately written as a question rather than a defect: this register's most repeated failure is a row that claims more than was measured, and F12's own row claimed a fix that had reached a different spec. |
...
| **CWF-007** — streaming | Still a design doc to argue with, not a build — but **the trap that decided whether it is possible at all is now measured** (`390b880e`). It said to check early whether a cloud function can consume a *streaming* upstream API. It can: the trap is accurate about the isolate (`sandbox.isolate.js:101` hands over a finished string), but **`nodegx-backend` does not use the isolate** — zero references, and `@cloud-runtime` resolves to `noodl-viewer-cloud/src/index.ts`, which shadows no globals. Functions run in host Node 22 with the real `fetch`, whose `Response.body` *is* a `ReadableStream`. So layer 2 is a **vocabulary** decision, not an engine limit. ⚠️ The isolate path stays real for whatever deploys through it — a streaming node must refuse there loudly or the two paths differ silently. **Questions 1, 2 and 4 are still Richard's**, and Q2 is now explicitly tied to FH-024: a channel readable by anyone who knows its id would be the **third** instance of that class. |
| **CWF-016** — idempotency keys | ✅ **Slice 1 done 2026-08-06 (`c87c120f`) — the design is decided**, written into the doc before any code as its own slice order demanded. Explicit `Idempotency-Key` (body hash opt-in), key **and** response stored, 24h TTL on `ExecutionStore`'s existing sweep so there is **no fourth timer**, a sqlite table beside the execution store rather than an in-memory map, claim-then-run under a unique constraint, and a claim that is **provisional until the run succeeds** (a crashed claim is released — a key that can never be retried is the same defect with a longer fuse). ⚠️ **Its CWF-005 trap was BACKWARDS and the correction changes what the feature can promise.** A `call-function` step never reaches the HTTP endpoint — `invokeCloudFunction` → `WorkflowRunner.invokeFunction` → `cloudRunner.run`, in process, with literally `headers: {}` — so dedup cannot swallow a retry. The real limit is the mirror image and worse: **our own retries are the one duplicate path this feature cannot protect.** **Still unbuilt: slices 2–4** (the store, the endpoint, the editor door), concurrency test first. |
...
| 5 | [CWF-004](CWF-004-THE-TRANSFORM-STEP.md) — Transform + data steps | **Slices 1 and 2 SHIPPED 2026-08-06.** Rewritten 2026-08-05 on Richard's argument: without a workflow-level reshape, every function carries its caller's mess. The whole declarative data family is in — `transform`, `validate`, `filter`, `sort`, `deduplicate`, `split` — plus JSON parse/stringify as transform operations. **Aggregate is unbuilt and is Richard's decision** (it is the one candidate that computes); S6's "new function from this step" gesture is unbuilt. A free Function step remains advised against. |
