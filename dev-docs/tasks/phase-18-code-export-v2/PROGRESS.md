...
The three share one drive and one gate chain, and they turned out to share a shape: a port the
exporter had no rule for, on a node it reports as `translated`. **A node can be `translated` in the
ledger while dropping the parameter that decides what it looks like** — EXP-014's AC5 raised that as
an EXP-008 question and it is still open; the two ledger notes now at least say what is honoured.

Built: the background fold and `backdropBlur` in `computeNodeStyle` (transcribed from the runtime's
`_updateBackgroundLayers`); `authoredTag` in `component.ts` reading `CatalogIndex.enumValues`, new,
so no element list is hand-written; the runtime's two-declaration `body` rule plus a form-control
`font: inherit` in `scaffold.ts`. Two fixtures (`ground-desk`, `tag-desk`), three specs (78 rows),
**21 mutation arms, 21/21 killed**. The ledger's `Group` and `Text` notes and
`EXP-002-TARGET-OUTPUT.md`'s "Semantic HTML" row were corrected to match.

The drive is a **reverted-worktree pair**: `git worktree add --detach HEAD`, both arms exported from
the same installed project, both `npm install && tsc -b && vite build` exit 0 with no hand edits,
both served and measured in the same headless browser. Freelancer's hero **1.03:1 → 17.24:1**;
**0 → 1 `h1` / 5 `h2` / 5 `section` / 1 `main`**; body, heading and button `system-ui`/`system-ui`/
**`Arial`** → Inter, Inter, Inter. Every after-reading matches the **viewer** column the three task
files recorded before any of this was built, so three renderers now agree.

🔴 **Three things a task file said were wrong, and the measurement is why.** (1) EXP-015 said the
lost tag was "not refused, not noted" — it was noted **61 times** as `has no style/content mapping`,
in the notes, the report and 61 in-code markers; the defect was the *translation*, and the wording.
(2) EXP-016 said `document.fonts` reported four Inter faces **loaded** before the fix — measured
cold on the reverted arm they read **`unloaded`**: a browser does not fetch a face nothing uses, so
the app shipped four TTFs and never requested one. (3) A `var(--font-sans, …)` fallback was written
on the reasoning that 39 of 40 fixtures are token-less; they are not — `effectiveTokens` merges the
shipped defaults under every project, and the spec caught it. ✅ **a task file's §"what is true
today" is a measurement with a timestamp, not a premise** — re-take the ones your change depends on.

⚠️ **`&& false` is still not an arm.** M11 read `NO SUMMARY / 0 total` — the mutant did not compile,
which is not a kill. Re-armed at the value level (push the node's own id instead of the collapsed
Group's) and it died to three rows.

Gates: `nodegx-export` jest **83 files / 3126 rows exit 0** (80/3012 before — 78 of the +114 are the
new specs and the rest are the seven fixture-walking specs picking up two new fixtures, reconciled
against `ls tests/*.test.ts`) · package `tsc` 0 · editor `tsc` exit 0, empty log · `export-ledger:check`
OK 176/124 · **picker 117/127** · editor `test:ci` 2943 specs, 4 failures = the floor by name
(AIX-006 ×4, seed 61662, readout fresh 12:13).
