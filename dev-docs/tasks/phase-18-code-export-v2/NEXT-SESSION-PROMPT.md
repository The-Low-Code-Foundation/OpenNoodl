# Next session — the ranked row was blocked by a different gate, and the report was hiding a hole 73 wide

## Where the phase stands

Session 63 drove §33.6's top row — the **row-owned write** — before building against it, and the
reading moved twice. The recorded blocker is not the one that fires, and one screen further out the
export was dropping **every** deferred visual control out of the generated code with nothing where
it stood. That second thing is now fixed and under test. §34 has the long form.

- 🔴 **§5.4 is not this family's gate.** The dominant real graph's `Model2` has **no `prop-` input**,
  so *"a row written from inside the row is state the list owns"* never fires. What fires is **§5.3**
  — *"its Id output is consumed, and a repeater row has no id in the emitted app"*. Read off the
  disposition, not reasoned about.
- ✅ **Fixed: 73 of 73 → 70 marked.** Every deferred `range`/`options`/`checkbox`/Radio Button/Radio
  Button Group vanished from generated source. `markDroppedChild` now marks them where they stood.
- ✅ **Fixed: a sentence that claimed the opposite of the file.** *"the structure renders"* — it does
  not; the node is left out. Behaviour unchanged, wording corrected, and the rows that pinned it now
  assert the **file**.
- ⚠️ **3 still silent, and they are a different defect** — children under a component instance.

## 🔴 Read this before planning anything

1. 🔴 **Ask what the number would be if the defect WERE present.** `silenthole.ts`'s first run said
   **73 marked / 0 silent** — the exact inverse of the truth — because `app.files` includes
   `EXPORT-REPORT.md`, which names every deferred node id. A clean number, shaped like "no problem
   here", produced by measuring the wrong property.
2. ✅ **A zero needs a known-firing arm beside it.** "marked = 0" reads the same when the detector is
   broken. The control — **1077** ids named in markers over the same corpus, same detector — is what
   made the absence real.
3. 🔴 **A ticked AC can hold for one population only.** EXP-010 AC3 is *"nothing dropped silently"*,
   ✅, and it covered the child whose *type* was unidentifiable. The identified ones had the same
   hole, in the same function, for eight sessions.
4. 🔴 **A gate that asserts a sentence is not a gate on the artefact.** The two rows pinning "the
   structure renders" checked `result.notes` and never `result.files`. Grep the suite for the same
   shape — it is cheap and §34.7 expects more.
5. ⚠️ **A glob in a shell variable does not expand in zsh**, and the shell's cwd is reset between
   calls — s63 lost three calls to those two, and once wrote a file into the wrong `scripts/`.
...
../../node_modules/.bin/jest                  # 1161/1161, 46 suites, 46 files on disk; ~216 s
...
### 1. 🔴 The row-owned write, with the question §34 sharpened

Not *"can a row write to itself"*. The measured question is: **does the write need an id at all?**
...
⚠️ Derive before building, not after: `updateWhere` **replaces** the row object, so `item` goes stale
for anything later in the same handler (a write then a removal removes nothing). Owner **`NONE`** —
a design question. **1 independent graph exportable, 6 corpus-wide, 14 nodes, and 100% of every
`Set Object Properties` anybody has authored.**

### 2. 🔴 A deferred *logic* node's reason still reaches the report only by accident

§34.2: `model2ForeachGate`'s sentence is stored in `dispositions` with no `notes.push`, and surfaces
only when a wire path happens to quote it. The visual half is fixed; this is the same defect on the
logic half. One `notes.push` plus a decision about noise — every deferred logic node would now be
named, and some of them are already named twice.

### 3. Children authored inside a component instance — unowned, and the instrument needs fixing first
...
do. Cheap, and it puts the real path under the corpus audit permanently. *(Carried from s62 unstarted.)*
...
- Grep the suite for other `toContain` rows that assert a **note** where the claim is about a
  **file** (§34.7). §34.5 found two by accident.
...
## 🔴 What session 63 would tell you if it could only say three things

1. **Drive the ranked row's reading before you build against it.** The blocker on the top row was a
   different gate than three sessions of notes recorded, and finding that out cost one export and one
   disposition dump.
2. **Ask what your number would look like if the defect were present.** 73/0 and 0/73 came from the
   same corpus, the same code and the same probe — one line apart. The clean-looking one was wrong.
3. **A ✅ AC is a claim about the population somebody measured.** "Nothing dropped silently" was true
   of kits and false of every built-in control, and the two live in the same `if`.
...
**reconcile the suite count against disk** (`ls tests/*.test.ts | wc -l`). 46 files, 46 suites at the
end of s63.
ts-morph and Prettier stay uninstalled; `packages/nodegx-export` is not in root `test:packages`.

🔴 **Check for a peer's suite before running yours** — `ps` for `jest`/`vitest`. s63 waited on a
`noodl-mcp` run with an `until`-loop and lost nothing by doing the reading first.
...
(`$S/foo.log` with `S=/path/*/scratchpad` is a literal). 🔴 **`cmd | head; echo $?` reads `head`'s
exit code** — redirect to a file and echo `$?` on its own. ⚠️ **`timeout` is not on this machine**
(BSD), and **`uniq -w` is not either**.

## Instruments

s63 scratchpad `da3e8895-…/scratchpad`:
- **`silenthole.ts`** — the §34.3 table, **with its control arm**. Copy into
  `packages/nodegx-export/scripts/` to run (the relative imports need to live there), point it at
  s62's `v2only`. 🔴 Its header records the `EXPORT-REPORT.md` trap that inverted its first reading.
- **`instkids.ts`** — the component-instance subtree count. ⚠️ Header records why its split is not
  yet a finding.
- `dump_row.py` — dumps one component's nodes, params and wires from disk (takes a project dir and a
  component-name fragment). This is what produced §34.1's graph.
- `cn019-out/`, `def015-out/` — two full exports; `snap-src-pre` is `src` before §34's change.
- `tsc.log`, `jest.log`, `picker.log`, `check.log`.

s62 `898de7ef-…` — **five reusable from-disk sweeps**, all sharing one loader, all taking any number
of base directories: `rowvalue_sweep.py`, `declared_sweep.py`, `disagree_sweep.py`,
`outsink_sweep.py`, `setmodel_sweep.py`, plus `foreach_sweep_fixed.py` (s61's census with the
non-dict-`children` guard). 🔴 **`v2only/` and `classiconly/`** — symlink farms splitting the corpus by
format; point any sweep at these and every number arrives pre-split.
...
2. Mutate **by asserting the anchor's uniqueness before replacing it** — a python
   `assert s.count(old) == 1` that fails leaves the source untouched. ✅ s63 used exactly this for all
   four of its edits.
...
5. ✅ **Read what the mutant does to your own rows.**
...
- ✅ **For a corpus-wide question, call `parseProject` + `emitApp` in-process** rather than shelling
  `emit-app.ts` 60 times — s63's sweeps do this and finish in one run.
...
the field 48 of 62 real projects carry and `resolveSourcePortKind` reads first), **or a child
authored inside a component instance** (§34.5).
