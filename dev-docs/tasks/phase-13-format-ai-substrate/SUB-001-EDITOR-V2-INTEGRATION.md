...
**Deferred (documented, not blocking):** true on-demand lazy loading (needs a whole-project-consumer audit — see Risks); performance pass + crash testing on a *large real project* in the running app; wiring `reloadComponentFromDisk` to a live file watcher (collab / SUB-007). The feature flag ships **off**, so none of this is on the default path. **DEBT-002 (2026-07-24) attempted the performance/crash pass and could not complete it**: the 176-component corpus is React-17-legacy and now hard-gated read-only behind the migration wizard, and a synthetic 155-component project opened fine but the session was ended by the editor's HMR full-reload bug (phase-14.5 PROGRESS, DEBT-002 findings #1) before save-latency / kill-mid-save could run. Still owed; needs one clean run in a quiet tree.
