...
**Status:** ✅ **done** 2026-08-08 (session 3) · **Track 2 (surface)** · ⭐ · closes audit **F5** and **F9** · §5 descoped with its blocker named (**F22**) — ✅ **F22 resolved 2026-08-10**, §5's webview half shipped in phase 57 (BLD-014)
...
| 5. The editor client | 🟡 **unblocked and half-built.** F22 resolved 2026-08-10 (`@nodegx/render-measure`); phase 57's BLD-014 shipped the **webview** client, which now returns findings as well as a picture. The CDP-from-the-editor client (arbitrary URL, chosen viewport) is still unbuilt. |
...
| F22 | **§5 (the editor client) descoped, and the blocker is not the one the task expected.** It guessed "the sandbox's webview plumbing". That plumbing is fine: `SandboxPreview.tsx:86` holds an `Electron.WebviewTag` ref and `PreviewTokenInjector` already reaches into it on `dom-ready`, so `webview.executeJavaScript(measureExpression(...))` is a handful of lines. The real blocker was **where the shared substrate lives**: the pure half was plain CJS under `scripts/` precisely so the CLI runs in a fresh checkout with no build step, and the editor bundle could not import it without dragging `child_process`/`http`/`net`/`ws` into the renderer. ✅ **Decided by Richard 2026-08-10 (phase 57, session 15): option 2 — a new no-build package.** `@nodegx/render-measure` now holds `measureExpression`, `summarise`, the thresholds, the finding vocabulary and `placeholderStringsFromCatalog`; `render-report.js` requires it and re-exports every name it used to own, so `noodl-mcp` and `measure-from-disk.js` are untouched (its 41 recorded-measurement specs stayed green across the move). ⚠️ **It is plain CJS with a hand-written `index.d.ts`, unlike this repo's seven other no-build packages, which point `main` at `.ts`** — because `measure-from-disk.js` is run by bare `node`, and bare `node` cannot `require` a `.ts` file. The two rejected options and why: a dependency-free file under `scripts/` would bundle (webpack's loaders exclude only `node_modules`) but the editor's `tsconfig` has no `allowJs` and its `include` is limited to `src/*`, so `typecheck:editor` could not see 600 lines of measurement code — an unwatched typecheck is the `test:main` shape; and moving it into the editor with a build step for the CLI destroys *clone and measure*, the property that made the harness useful in this phase's own audit, and points `noodl-mcp` at the editor package. **Verified end to end:** the identical expression evaluated inside a live sandbox `<webview>` and summarised by the same module returned `0 errors, 1 warning (empty-decorated-box). preview 800×150px, 2 texts, 2 on screen, 0 images.` | ✅ **resolved** |
