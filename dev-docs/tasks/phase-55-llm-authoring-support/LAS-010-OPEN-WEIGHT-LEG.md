...
| F37 | **Our own write-mode MCP surface does not fit in a 32k-context client.** 89 tools → 100,512 chars of function schemas resent every turn, plus 2,855 chars of server instructions. **Measured on the wire, not estimated: turn one of the open-weight replay billed 27,322 prompt tokens** — tools + instructions + the 1,235-char brief. Against `Qwen2.5-Coder-32B`'s 32,768-token window that leaves ~5,400 tokens for the entire build, so F36's model could not have worked even with tool support. **60 of the 89 tools and 63% of the schema bytes are backend admin** a storefront brief never touches. There is no progressive disclosure on the server — and the two baselines never met this, because the `claude` CLI defers tools behind `ToolSearch` (haiku's session-1 transcript opens with **11** `ToolSearch` calls). So "the same surface" is not the same surface, and every small-context MCP client is locked out today | 🔴 **OPEN** — measured, not fixed; candidate LAS-012 |
