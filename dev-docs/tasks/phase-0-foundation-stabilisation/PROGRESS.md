...
4. ~~**Dashboard routing error** noted in the prior PROGRESS.md ("`ERR_FILE_NOT_FOUND` for `file:///dashboard/projects`", attributed to Phase 3 TASK-001B) was not re-verified in this audit — it's out of scope for Phase 0 (the prior doc itself says it belongs to Phase 3) and is left for whoever audits that phase folder.~~ **Resolved 2026-07-26 by PLAT-005.** It did reproduce, and the Phase 3 / TASK-001B attribution this note repeats was wrong — the cause was `noodl-core-ui`'s `Launcher.tsx` assigning `url.pathname = '/dashboard/<tab>'` onto the renderer's host-less `file:` URL and pushing the result into session history, so the next reload asked the filesystem root for it. Nothing to do with the Electron store migration. See `dev-docs/tasks/phase-3-editor-ux-overhaul/TASK-001B-launcher-fixes/ISSUE-routing-error.md` and `dev-docs/tasks/phase-14-editor-platform-health/PLAT-005-NOTES.md` §4. Live confirmation in a running editor is still owed.
