...
What is genuinely left is a single class of work: **things that can only be seen by running the editor** — LIB-005's QA-3/QA-4/QA-6, LIB-001's Criterion 5, LIB-002's visual restyle pass, and LIB-003's per-module exercise on both React pairings (0 of 29 done). None of it is blocked on code.
