...
| 6 | LIB-006 | Legacy project import — best effort, honest report, AI repair | Not started | 1–1.5 wks | Added 2026-07-30 as the counterpart obligation to [`COMPATIBILITY-POLICY.md`](../../reference/COMPATIBILITY-POLICY.md). The policy says legacy Noodl projects are not a design constraint and the fallback is "the user's AI assistant fixes it, or they rebuild"; this task makes that fallback real — outcome taxonomy, a committed machine+human import report, visible placeholder nodes that the semantic validator errors on, the cheap mechanical conversions, and a wired hand-off to AIX-002's diff review. **Step 1 is a committed inventory table, not code**, and it is also the scope boundary against this quietly becoming a fidelity project. |
