...
| NDA-002 Reactivity contract | 1 | ✅ **Built** `bd6632ca`…`825da393` | R1–R9 green unmarked; all suites green. **`items === arr` does NOT break** — `Collection.get`/`create` return a memoised Proxy, so identity holds and the raw array is simply unreachable. Mutating methods intercepted in the `get` trap (traps alone storm 3–4 changes per `splice`). Perf: only indexed reads regress (~0.3 µs/read Proxy floor; digit-leading fast path recovers 28%). ⚠️ `packages/noodl-editor/src/external/*` + `nodegx-backend/deploy/artifact/` embed stale `collection.ts` copies until rebuilt — **and both are under `packages/noodl-editor/src/external`, which `.gitignore:187` ignores in its entirety, so this is untracked build output rather than committed drift**. ✅ **Criterion 3 discharged 2026-08-01 — all four criteria now met.** No cyclic-loop warnings: `runtime/cyclic-loop` appears **0 times** in `.logs/dev.log` across the whole session, and the churn fixture (`tier1-tails`: Repeater `refresh`, in-place mutation, Collection resync) shows **no warnings chip at all** — `warningsAmount === 0`. The QA fixture shows `0 errors · 2 warnings`, both `Unknown node type "Markdown"`, neither cyclic. ⚠️ **The Problems panel was the wrong instrument for this and would have read clean either way** — it lists *semantic-validator* findings, while a cyclic loop raises `runtime/cyclic-loop` through the runtime error channel into the *warnings service* behind the toolbar chip (FINDINGS `TT-iv`). Screenshot corpus re-run: **30 surfaces × both themes, zero blank frames, editor and canvas render correctly in dark and light** (`corpus/captures/2026-08-01-22-16/`, up from the 07-26 baseline's 28 because the menu-walk self-discovered four panels added since). ⚠️ **That capture is a *new baseline*, not a comparison**: the 07-26 set was captured against *Shine Phase 2*, a fixture that no longer exists, with six days of intervening UI phases — so "no render regressions" is asserted from the live reading, not from a pixel diff, and could not have been otherwise |
...
| NDA-014 Type dead ends | 2 | 🔄 §1+§2 done | Decision at [`PORT-TYPE-CONTRACT.md`](../../reference/PORT-TYPE-CONTRACT.md) (A now, C direction). Table changed (`object`/`array`/`color` → `string`), JSON mirror added in `setInputValue`, catalog + register regenerated, runtime jest green (1,026), editor suite green (1,885 specs incl. validator/catalog-index). ⚠️ **Criterion 1 was NOT met, and the live check on 2026-08-01 is what found it: a Function `object` output wired to a Text node rendered `[object Object]`.** The JSON mirror had been written into `setInputValue` and **could never fire on any graph**. Three things must hold and only the first did: the *table* permits the cast (✅, and that is all the 07-29 pass verified — it checked the editor's typecast table, not the DOM); the *receiving* port must be recognisable as `string` (❌ — `nodedefinition.ts:47` copied a declared type onto the instance only for `color`/`textStyle`/`array`/`object`, so **every declared `string` port in the library was untyped at runtime**); and the *sending* port must be recognisable as `object` (❌ — the Function node registers author-declared outputs via `registerOutputIfNeeded`, which passed **no type**, though the author's choice sits in the `outtype-<label>` parameter the editor-side port list already reads). ⚠️ **There was no corpus row for any of it — not a row passing for the wrong reason, no row at all.** Fixed on all three counts and **live-verified**: the Text now renders `{"user":"Ada","roles":["admin"]}`, and an `array` output renders `["one","two","three"]` rather than the bare join `one,two,three`. **9 new rows** (`nda-014-outbound-string-cast.test.ts`); revert-measured — reverting the port-type half reddens exactly 3 of the first 8, reverting the Function-node half reddens **only** the 9th. ⚠️ **The first 8 rows would have missed the defect entirely**, because their emitter declares `type: 'object'` the ordinary way while the real Function node does not — the same lesson as `DV-xvii` from the other side. ⚠️ **The cast is narrowed from the contract's literal wording**: phrased over the *value's* runtime shape it also fired for a direct `setInputValue` and for a `*`-typed source, breaking `net.noodl.TextAccumulator`'s object-chunk refusal (NDA-004 B1, 5 rows) and the `Object` node's dereference of a plain object wired to `Id` (NDA-012 C3) — **both caught by the existing suite, neither by reading the contract**. It is now a property of a wire between two declared ports, pinned by two `NDA-014 scope:` rows; **PORT-TYPE-CONTRACT.md carries the amendment and Richard should confirm the wording**. FINDINGS `TT-i`/`TT-ii`. Gates after: runtime **1,767**, viewer **579**, editor jasmine **2007/0**, four catalog gates exit 0, typecheck unchanged (18 pre-existing `TS2307`). Outstanding: the two `E1` cells (`array` casting only to `collection` for Dropdown/Repeater), and the remaining `object` outputs beyond the one criterion 1 names |
...
| NDA-017 Signal input freshness | 2 | ✅ **DONE — §0 `d6db6f39`, §1 decided, §2 built and live-verified** (`213cf337`, `b6dc078a`, `21eda2b0`, `78435c25`, stamped 2026-08-01). **All six criteria met as of 2026-08-01** — criterion 6's semantic-validator half landed as the `signal-driven-stale-input` rule (`warning`, on by default, 7 rows). ⚠️ **Narrowed from the criterion's literal wording on purpose**: *"a `Run` driven by something other than its inputs' producers"* flags a Button driving `Run` while values come from Text Inputs, which is the canonical **correct** graph and the reporter's own working one. The rule instead requires the producer to be *asynchronous* (it publishes a completion signal) and the signal not to be reachable backwards from that completion. **Measured on the false-positive corpus: 12 firings, all in one legacy project, 0 in the other eight, 0 new errors** — and every firing is the report's own shape. 3 of the 7 rows are controls, each a graph the literal reading would flag | Community report, **now reproduced** — 10 rows, 4 `test.failing`, including row 4 against the **Function** node, so the reporter's workaround is measured not to be one. The spec's mechanism survived §0 (unlike NDA-008/016). **One correction**: the seed also reaches the graph with *no* `Run` at all — with the signal connected the node never evaluates at boot, yet `connectInput` pushes a confident `0` downstream from `result`'s getter, so §1's option A as written closes three of four doors. Reusable: `update()` is synchronous and `settle()` yields, so a row written with `settle()` reports this whole defect class as absent. The confirmed mechanism: with `Run` connected the value setters go passive and `Run` evaluates whatever is in scope, so an async producer that hasn't landed leaves the previous cycle's value, and `On True`/`On False` pulse downstream while `Result` stays quiet. **Twelve node families**, not one. ~~**§1 is now the blocker and it is a decision for Richard**~~ — **§1 was decided on 2026-08-01 and with none of the four options offered.** Richard's answer is a per-input **"Run on value change"** affordance in the node panel, because the defect is one level deeper than the spec framed it: connecting `Run` *silently changes what every other port does*. `Run` becomes purely additive, all inputs tick by default, several ticked inputs changing in one frame coalesce to one run, and a node that has never evaluated reports `null` rather than the confident `0` its getter pushes today. See Decisions §7 **§2 built 2026-08-01 across fifteen families — the twelve in the spec's table plus three it missed** (Array, Variable-viewer, Array Filter; one of them the code already calls a twin of a node that *is* in the table). Mechanism in `run-on-value-change.ts`: one `boolean` `runOnChange-<input>` port per governed input, `shouldRunOnValueChange()` on `Node.prototype`, and a `runOnValueChange` field on the node definition. ⚠️ **The table's "same idiom" claim does not survive reading it** — only five families suppress a value *setter*; the rest suppress a *subscription* (`onModelChangedCallback` opens with `if (isInputConnected('fetch')) return;`), which is the same trap with no port to hang a checkbox on. Hence `sources` alongside `inputs`. ⚠️ **One site keeps the old guard deliberately**: the *definition* port (`expression`/`functionScript`) is set at load on every node in the project, and dropping its guard would run every `Run`-driven script once at load, including the ones that POST. ⚠️ **Two defects the fix created and the corpus caught**: Variable would have reverted to a stale value on the next `Set` (it recorded `latestValue` only on the branch that did not store), and Condition would have double-fired `On True` (it had no coalescing, which was free while wiring `Evaluate` silenced the setter). ⚠️ **And one the fix exposed rather than caused**: seeding inputs `undefined` made `a.missing.deeper` *throw at load*, raising `expression/threw` before anything had arrived — exactly what the Failure Contract forbids. The old `0` hid it by making the boot evaluation succeed on a value nobody supplied. **Caught by the NDA-004 §2 corpus, not by NDA-017's own rows.** **Live QA found three more** (`78435c25`): the checkboxes rendered *unticked* on a node running ticked, because the panel reads a declared `default` and there wasn't one — so `default: true` and absent-reads-as-ticked are both declared, and they are needed together *because* A-D1 is true. Unticking a box **broke the Expression node outright**: a saved parameter is applied before the port it governs exists, so it reached `registerInputIfNeeded`, landed in the expression *scope*, and `_compileFunction` then built a `Function` with a parameter called `runOnChange-a`, threw, and evaluated to `0` for the session. ⚠️ **The corpus could not have caught that and the reason generalises: a test sets a parameter with `setInputValue` on a graph that is already built, so the port always exists first. Only a saved project applies the parameter first.** Fixing it centrally exposed **two older `defineNode` bugs** — `Object.create` left every `methods:`-declared function frozen on the prototype (so `registerNumberedInput` and `makeNodeInert` worked only on nodes that declared none), and the normalisation loop mutated `opts` in place so `defineNode` was not idempotent |
...
| NDA-012 Per-node audit | 3 | ✅ **Audit COMPLETE — 17 of 17 categories, 136 of 151 nodes, twelve checks each** (Visual closed 2026-08-01). 🔄 **Remediation: 32 of Visual's 47 in-scope ⚠️ cells closed 2026-08-01** across four streams — A in six fix items (`a1aaa73a`…`bbfd6340`), B in three (`d6f483c9`, `34ec5660`, `19da638a`, corrected by `f2bc6d0a`), C in two (`e673c770`, `f100d2ce`), all live-QA'd except D. **Stream D closed 9 more: the four `D1` bare-string contracts, three `B2` cells, `Radio Button`'s `A1` and `Page Router`'s `A2`.** ✅ **`A1`, `A2`, `A3`, `D1`, `G1` and `H1` are now all closed for the whole category.** **15 cells remain and every one of them is owned elsewhere or blocked**: 11 are `ERG-001`'s (7 `B3`, 4 `B1`), 2 are `NDA-014`'s (both `E1` — `array` casts only to `collection`, which is the typecast table's property and not the nodes'), `Page`'s `B2` is blocked on Richard (its mechanism is `DV-vii`, the dead `Title`/`Url Path` ports, and fixing it means deciding whether the Page or the Router owns the document title), and `Radio Button`'s `F1` is **half closed** — class F's *report* half is built, its *nameable target* half needs a name→group registry like `Component Stack`'s. **There is no remaining code change in this category that this phase owns and can make unilaterally.** ⚠️ **Stream D found three more mis-filed mechanisms, one of which would have produced a fix that changed nothing**: `Page Router`'s `A2` was filed as identity-versus-value, but the router held a *reference* to the index entry it was comparing against, so identity and value agreed and only a **snapshot** could see an in-place edit; `Columns`' `D1` described a `NaN` column that NDA-006 had already turned into a silent *drop*; and `Page Router`'s `D1` double-decode is really an **uncaught `URIError`** — a page parameter containing a `%` took the router's whole match down. ⚠️ **`Drag`'s `B2` closed with no code change**: stream B's `G1` fix had already put its diagnosis on the runtime bus, and the cell had simply not been re-read. **Four defects were found by writing the rows and none had a cell** — the Repeater kept using the *previous* compiled `templateScript` after a syntax error; `Group`'s `scrollToElement` threw a `ReferenceError` in any runtime without a DOM, on a node declaring SSR `safe`; `RadioButtonContext`'s truthy default rendered a groupless Radio Button **permanently checked**; and `Scroll To Element` aimed outside its Group silently scrolled a *different* container. ✅ **Live QA for streams C and D done 2026-08-01 — all eight claims hold**, each measured by something that distinguishes the fix from its absence. It also found **`DV-xvii`**, which only this category's own two fixes could have exposed: `Scroll To Element` resolved its target with the pre-React-19 `getRef()`, so since `RUN-001` it could not resolve an ordinary target at all and reported *"it may not be mounted"* about an element that had rendered. ⚠️ **The corpus rows for that contract were green and always had been** — their fake supplied `getRef` returning `{current: el}`, a shape no real node has. Fixed and live-verified (`scrollTop` 0 → 399). ⚠️ **Stream C corrected the worksheet twice.** The `A3` pair was filed as two defects and is one: `Group`'s cell called `Scroll To Index` the careful version because it tests the ref inside `scheduleAfterInputsHaveUpdated`, and `Video`'s cell said Group's actions "at least defer" — but the ref commits after the graph update either way, so both dropped the action and making the two *agree* would have closed the cell and fixed nothing. And `Video`'s `E1` changes no behaviour at all: `valueChangedToTrue` already replaces `type` with the signal type, so the `boolean` was read only by the editor panel — which is exactly where the wrong affordance was drawn. `G1` and `H1` are closed for the whole category. The largest remaining block is **DV-viii**, which is `ERG-001`'s, not this task's. ⚠️ **Stream B's live QA disproved one of its own claims**: the third `DV-ii` instance (`Text Input`'s placeholder opacity) was fixed, measured, and **reverted** — `assets/style.css` already carried the rule, so it was Button's case and not Icon's. The operational rule that would have caught it from the source is now in FINDINGS `DV-ii` | **Navigation added 2026-07-30 — all 8 nodes, 15 defects, 8 of 8 nodes affected, 1.88/node: the highest rate of any category and the reversal of the Cloud Services dip.** Headline is **NV-i**: `Page Inputs` had *no connectable ports at all* and had not since the initial commit — its whole `setup` was commented out, so the node that exists to expose a page's own parameters could be added from the picker and wired to nothing. CS-i one step on (there the node could not be added; here it does nothing). Also **NV-ii**, which corrects this file: reordering `Signal To Index` changes nothing observable, because the *visibility* of signal-before-value depends on whether the paired value port was non-`undefined` at connect time — so the class is two situations, not one, and "a test catches it" is not the test. **NV-iii** is a fourth cross-category shape (one-shot state that latches, in Close Popup and Pop Component Stack). **NV-v** is where the rest probably are: 6 of the 15 live in a module's `setup`, which `graph-harness` does not run. 6 fixed, 9 filed. Variables (4/4) as the worked example; **Logic, Math, Events, String Manipulation, Interpolation, Javascript and Sensors added 2026-07-30**, run batched with NDA-005's C1 so each node is read once. **31 new defects in 22 nodes and no find-rate decline.** Two shapes recurred *across* categories, which the per-node reads could not have shown: **signal-before-value** (`Receive Event`, `Signal To Index` — in both, carrying data with a signal is the node's whole purpose) and **a value port whose setter emits**, announcing a change at page load once authored (`Switch`'s `State`, `Counter`'s `Start Value`). Two nodes are clean and both for a recorded reason. **Cloud Services added 2026-07-30 — all 22 nodes, 25 new defects, the largest single-category haul in the phase**, and it corrected this file's own dismissal of two nodes as picker-exempt: **`Sign In With` and `Request Magic Link` cannot be added to a graph at all**, while four deprecated nodes in the same picker group can (FINDINGS **CS-i**). The other headline is **CS-ii**: `Aggregate Records` carried both defects NDA-004 §2 and PLAT-003 had already swept for, because it is the one Cloud Services node in `noodl-viewer-cloud` and both sweeps covered two packages. Fixed; everything else is a verdict. **The per-node rate dipped for the first time (1.41 → 1.14) and the cause is five sibling nodes, not exhaustion** — read the find-rate table before treating it as a stop signal. A **third** cross-category shape joined the two: a create-on-read lookup fed by a value that can be absent. **The small remainder added 2026-07-30 — Component Utilities, Utilities, CustomCode, Animation and Cloud, 27 of 28 nodes, 39 defects in 21 of them, 1.44 per node.** Two categories set records: **Animation 2.50** and **CustomCode 2.00**, both with every node affected. Headlines: **SR-v**, `Date To String`'s `Invalid Date` had never fired because `flagOutputDirty` on a signal output sends a *value* of `undefined` rather than a pulse — and PLAT-003 NOTES §25 had written the line down and correctly left it alone; **SR-iv**, `Script`'s External File mode had **no failure path at all**, so an unreachable URL left the node permanently and silently inert (`isWaitingForExternalFileToLoad` never cleared, `update()` skipping `Node.prototype.update` for ever) — the same class NDA-004 fixed on `Expression` and `Function` and never carried to the other two script hosts; **SR-vi**, a timer leak shared by all four Animation nodes, where the node that gets it right (`Delay`) is filed under a *different category* so no per-category read would compare them; and **SR-iii**, a seventh cross-category shape — structured names concatenated into flat port strings and decoded with `split('-')`, found independently in `Component Object` (fixed) and `States` (filed: the name is genuinely ambiguous and needs an encoding decision). **SR-i** is the counting lesson: Component Utilities scored **0.63**, the lowest in the phase, because four of its eight nodes had already been remediated twice — a low rate with a cause, not a low rate. **`Logic Builder` audited 2026-07-30 — CustomCode is 5/5 and the category's rate rose from 2.00 to 2.80**, because the one node nine handovers had deferred as blocked held **six** defects, more than any other node in it. Three findings reach outside it. **SR-viii**: the compile-failure silence was on *four* of **five** script hosts, not the four SR-iv claimed — running the grep SR-iv had described rhetorically turned up **`REST` in Data**, which swallows the `SyntaxError` and keeps fetching with the *previous* script; four passes found one defect four times, each citing the last, and a class named by its exemplars had silently inherited their category. **SR-ix**: the reserved-name collision NDA-004 §3 predicted as a cost of adding a completion signal was **already live** here, silently, against the node's existing `error` output and `run` input — check whether a stated cost is already being paid. **SR-x**: `__triggerSignal__` was assembled on every execution, documented in a comment, and never passed to the compiled function, so it read `undefined` inside every block program ever run. 5 fixed, 1 filed (a port cannot change kind on a live node; `deregisterInput` throws on a connected port). ~~🛑 Data is ON HOLD as of 2026-07-31~~ — **the hold lifted 2026-08-01 when phase 34 landed, and Data is now CLOSED at 37/37.** The inputs were re-derived rather than inherited, and doing so corrected the scope twice: **Data is 37 nodes / 433 ports, not the 42 / 471 recorded here on 2026-07-31** (phase 34 deleted the five `noodl.byob.*` types, so that figure was already stale when written). Richard dropped the four deprecated ones (`Collection`, `Model`, `REST2`, `Variable`), all at 0%, so the denominator changed and nothing earned was lost. Settling scope also corrected the catalog's `inNodePicker`, which had reported five deprecated types as pickable because it mirrored the curated *listing* and not the picker's creatability filter — **measured live: index lists 126, picker offers 121, and the 5 dropped are exactly those types**. That also corrects **CS-i**. FINDINGS **DA-i**. **Data closed 2026-08-01 by a four-worker parallel batch — 29 nodes read, 26 new defects, 13 fixed and 13 filed, and the category went 12.7% → 96.8% documented.** The seam was four disjoint file sets and it produced **zero merge conflicts**. Headlines: **A-D1**, `Global Store`, `Subscribe to Store` and `State History` **did nothing at all until an author touched an input** — `registerInput` writes a declared `default` straight into `_inputValues` and `NodeScope` queues only the keys the *model* carries, so **a default never runs its setter**, and all three nodes did their whole job from a setter side effect; invisible from the canvas, because the panel shows the default. **C-OB-i**, the `Object` node silently lost every `prop-…` value that arrived a frame *earlier* than its Id — measured `{}` where `{name:'Ada'}` was expected, and NDA-004 §2 had read that file and written a comment asserting the opposite. **C-OB-ii**, an empty Id (`null` or `''`) binds a *process-wide shared* record and reports success — one shape at four sites, two of them still open in the Record family. **B**, `Variable` wrote a key literally named `undefined` and then fired `Changed` for a write its own getter refuses to read back, in the file next door to the one NDA-004 §2 fixed for exactly that. ⚠️ **Two instrument defects matter more than any single node**: `worksheets.js:55` derives the `B1` pre-fill by matching `/fail\|error/i` against output **names**, so a string port called `Error` satisfies it — **fifteen categories have now been audited from that column** — and `Model.get(undefined)` turns out to be **unreachable over a wire** (`sendValue` drops `undefined`, `node.ts:635`), so the reachable empty values are `null` and `''` and a sweep hunting the `undefined` variant would find nothing. ⚠️ **`Run Tasks` is Data's 37th and is not covered by this batch**: audited under NDA-009, never given a worksheet entry or C1, and the only in-scope Data node still at 0%. **Only Visual (20 in scope) remains.** See the find-rate table |
...
| Findings live-verified in the running editor | Tier 1 + NDA-014 | 2026-07-29 pass: preview runs the new collection semantics (push/index/length notify once each, synchronous, `items` identity holds, `set` = one change); zero renderer exceptions; no cyclic warnings; editor's live typecast table carries the three new casts. Catalog page empty = expected (DB query, no local data, no repeater on it). ⚠️ **This row and the NDA-002/013/014 task rows disagreed for four sessions and the register could not say which was stale. Resolved 2026-08-01: both were right about different things.** The 07-29 pass *named* NDA-002/003/013/014 in its header, but every claim it recorded was about collection semantics and the editor's typecast **table** — nothing drove `refresh()`, and nothing wired an `object` output into the **DOM**. It listed exactly those two as residuals; the header is what got read afterwards. **Both are now discharged** (`tier1-tails` fixture, 2026-08-01), and the DOM half found a defect the table check structurally could not — FINDINGS `TT-i` |
...
6. ✅ ~~🛑 **NDA-012/NDA-005 Data is HELD**~~ — **lifted and discharged 2026-08-01.** Phase 34 landed,
   the inputs were re-derived rather than inherited (catalog regenerated, `audit/data.md` deleted and
   regenerated, everything re-measured before planning), and Data closed at 37/37 the same day.
   **The hold was correct and it paid**: re-deriving is what caught that the recorded scope of
   *42 nodes / 471 ports* was already stale — phase 34 had deleted the five `noodl.byob.*` types, so
   the real figure was **37 / 433**. A number written down to hand work over goes stale exactly as
   fast as the work it describes.

   ⚠️ **The instruction to "read the merge diff as an audit input" was followed and it did not pay
   the way this entry predicted.** The three recurring shapes were expected in the *new backend
   plumbing*; the batch's headline defects were in the **oldest** code (the Object family's dirty-value
   flush, `Variable`'s `undefined` key) and in **AIX-005's agentic nodes**, which no pass had ever
   read. New code scored the phase's *lowest* find rate. The prediction was reasonable and the
   evidence went the other way — recorded because the next category's plan should not inherit it.
...
**Richard answered the remaining five open questions on 2026-08-01.** Two of them overrode the
recommendation and both are better for it; three turned into build work too large for this phase and
are now [phase 35](../phase-35-authoring-ergonomics/README.md).

7. ✅ **NDA-017 §1 — DECIDED, and not with any of the spec's four options.** The spec offered
   never-arrived detection (A), an upstream-pending concept (B), documentation (C) or nothing (D), and
   recommended A + C. **Richard's answer is a fifth option: a per-input "Run on value change"
   affordance in the node's config panel** — tick which of the declared inputs should trigger a run
   when their value changes.

   ⚠️ **The recommendation had the defect one level too shallow.** A and C treat *staleness* as the
   problem. Richard identified the actual one: **connecting `Run` silently changes what every other
   port does** — the node flips from "recalculate on any input change" to "never recalculate", with
   nothing in the panel saying so. That is why authors hand-build a `Value Changed` node behind `Run`:
   they are restoring a behaviour that wiring an unrelated port took away. His fix removes the trap;
   A only reports it.

   **Three constraints the build must hold, or it reintroduces the same confusion:**
   - **`Run` becomes purely additive.** The checkboxes are the *only* thing governing auto-run.
     Wiring `Run` adds a trigger; it never un-ticks anything. If connecting `Run` auto-clears the
     boxes, the trap has been made visible rather than removed.
   - **Default is all inputs ticked** — today's behaviour for a node with no `Run` wired, so nothing
     changes until an author deliberately unticks.
   - **Several ticked inputs changing in one frame produce one run, not three.**

   ✅ **And §0's open companion question is answered by the same message:** a node that has never
   evaluated reports **`null`**, not the confident `0` its getter pushes today. Richard's words —
   *"a null value if nothing has ever come in"*.

   ⚠️ **Keep the `Run` port.** An async re-fetch returning an *identical* value fires no change, so a
   node that must re-run per fetch still wires `Run` to the producer's completion signal. That is
   option C's correct dataflow and it now coexists with the checkboxes instead of fighting them.

   **§2 remains in this phase.** Scope is the whole twelve-family table, not Expression alone.

8. ✅ **The duplicate-insert question became a library-wide contract, and it left this phase.**
   Richard's reply named the real cost: *"some workflows are wired up as a series of input and output
   signals, and the workflow breaks at the point that one of the nodes doesn't output anything because
   of a duplicate you didn't plan for, where you'd actually like it to carry on running regardless."*
   That is not one node. It is the same shape as NDA-004 §3's mute nodes, `Run Tasks`' `Done` on one
   of four terminal paths, and **DV-viii — seven of eight Visual nodes with action inputs cannot tell
   a graph the action finished.**

   Settled as a sixth contract: [`OUTCOME-CONTRACT.md`](../../reference/OUTCOME-CONTRACT.md).
   `Done` / `Unchanged` / `Failure` — exactly one, always — plus a universal `Completed` that fires
   after any of them. Built as phase 35 `ERG-001`. `Insert Object Into Array` reports **`Unchanged`**:
   `Done` is a lie, `Failure` overstates a state the author asked for *and* breaks the chain.

   ⚠️ **Richard proposed config as the general remedy and it is deliberately bounded** — the rule is
   *prefer a port over a setting whenever both would work*, with `Treat Unchanged as` as the
   project-policy escape hatch, on NDA-003's `Treat empty as` precedent. The three costs of
   config-first are in the contract, and the second is this phase's own finding: **a default is a
   hidden behaviour** — `Global Store`, `Subscribe to Store` and `State History` did nothing at all
   until an author touched an input, with the panel showing the default the whole time (**A-D1**).

9. ✅ **The `Repeater`'s `null` semantics — DECIDED: it clears.** Empty array, `null`, anything falsy.
   Richard: *"a repeater should absolutely clear itself… We're not catering to existing projects
   anymore."* This is §3.3 of the handover and the one Visual fix that was held for a behaviour call.
   Remediation stays in this phase.

10. ✅ **`shortDesc` — DECIDED: delete it.** Fifty declarations, and re-checked on 2026-08-01: nothing
    in the editor's own interface reads it either, only bundled build output mentions it. Wiring it up
    would create a second source for a sentence enrichment already supplies, against the
    `description`-is-canonical rule settled the same day.

11. ✅ **The deprecated set — DECIDED: no revivals, and the one missed capability is solved properly
    instead.** Richard: *"The only deprecated node I'm thinking about is the Script Downloader. I don't
    want to revive it."* So: no deprecated node is fixed and none is brought back. All **30** are
    already unpickable (`inNodePicker: false` on every one, measured), so their defects are reachable
    only from a project that already contains them, and legacy projects are not a compatibility
    target.

    ⚠️ **`Script Downloader`'s replacement did not exist, and this file's earlier claim that it did was
    wrong.** The Script node's *External File* mode fetches a URL and parses it as **that node's own
    body**, scanning it for the node's declared ports (`javascript.ts:692-694` →
    `JavascriptNodeParser.createFromURL`). It cannot load a third-party library. The capability is
    rebuilt as app config in phase 35 `ERG-002` — where the finding is that
    **`noodl_modules/` already does the whole job and nothing surfaces it.**

    ⚠️ **Four capabilities have no live equivalent at all** and are recorded in phase 35's README so
    the decision stays deliberate: multi-stop number blending (`Number Blend` — Interpolation ships
    `Color Blend` and nothing for numbers), "which of N signals fired" (`Signal To Index`), a real
    HTML form with a `Submit` signal and field grouping (`Form`, `Field Set`), and the gyroscope
    (`Device Orientation`). Nothing depends on the answer.
...
26 defects (A 14, B 8, C 3, plus D's WD-1 across the six Record verbs). ⚠️ Counting note, per the
Cloud Services convention: this reports **distinct** defects. The per-node verdicts in the worksheet
sum higher — A's 14 live at **34 sites**, because `globalstore.ts` and `statehistory.ts` each back
three nodes.

⚠️ **The low rate is prior work plus newness, and the two pull in opposite directions.** Half the
category had been remediated twice (NDA-004 §2's `_getACL` mixin, the relation validators, the Array
mutators' shared helper) — Component Utilities' 0.63 for the same reason. But the *other* half, the
fifteen agentic/streaming nodes, had **never been read by any pass** and still scored only 0.93, the
lowest per-node figure in the phase, with two nodes completely clean. That is the first evidence in
this protocol that **recently-written code carries fewer of these defects than old code** — the
opposite of the "unread means unaudited, not clean" warning, and it does not generalise: the same
worker found `Global Store`, `Subscribe to Store` and `State History` did *nothing at all* until an
author touched an input.

⚠️ **Three of D's four filings are not node defects and are not in this table** — stale generator
prose, a third undeclared port-documentation channel, and an ACL silently dropped in
`RestDataAdapter`. They are in FINDINGS under **WD-2/3/4** and they are the kind of finding a
per-node table cannot hold.
...
- **2026-08-01 (ninth session — the Tier-1 live-QA tails, and the typecast that had never once
  fired).** The register's last unresolved bookkeeping item is closed, and closing it honestly meant
  re-deriving it rather than picking a side: the NDA-002/013/014 task rows said *"live QA pending"*
  while the Audit-coverage row said the 07-29 pass had verified Tier 1 live. **Both were right.** The
  07-29 entry *named* four tasks in its header but every claim it recorded was about collection
  semantics and the editor's typecast **table**; it listed two residuals — the screenshot-corpus run
  and "a live wiring demo of an `object` output to a Text node (table verified live, DOM demo not
  performed)" — and the header is what later readers took. **Three criteria were genuinely open and
  all three are now discharged**: NDA-013 criterion 5, NDA-014 criterion 1, NDA-002 criterion 3.
  ⚠️ **The QA fixture could not serve any of them.** Criterion 3 and NDA-013's criterion 4 both name
  it, and it was replaced (NodeGX QA Fixture superseded *Shine Phase 2*) by a project with **no
  `For Each` node at all**, DB-backed Collection consumers with no local data, and two Function nodes
  wired to nothing. All three would have gone green having exercised none of the named code. A new
  fixture generator carries them instead (`scripts/nda-live-qa/make-tier1-tails-fixture.js`), and its
  header table says which claim each part is for and what each would read under the old code.
  **NDA-013 held**: the Repeater holds the same array instance the graph bound, an in-place `push`
  moved the array to 3 and left the DOM at 2 rows (the A1 defect), and `Refresh` took it to 3 rows
  with the *new* model id — so it re-read the source rather than re-rendering. **NDA-014 did not.**
  Wiring a Function `object` output to a Text node rendered `[object Object]`, and the JSON mirror
  written in NDA-014 §2 turns out to have been **unable to fire on any graph since it was written**:
  a declared `string` port carried no type at runtime, and the Function node registered its
  author-declared outputs with no type either. **There was no corpus row for it — not one passing for
  the wrong reason, none at all.** Fixed on both counts plus the cast itself, live-verified
  (`{"user":"Ada","roles":["admin"]}` in the DOM), and covered by 9 rows. ⚠️ **Eight of those nine
  would still have missed it**, because their emitter declares its type the ordinary way and the real
  Function node does not — `DV-xvii`'s lesson reached from the opposite direction. ⚠️ **The cast is
  deliberately narrower than the contract's own wording**, which is phrased over the value's runtime
  shape and, built that way, broke two behaviours earlier tasks had pinned (`TextAccumulator`'s
  object-chunk refusal, the `Object` node's `Id` dereference) — **both caught by the existing suite,
  neither by reading the contract**. `PORT-TYPE-CONTRACT.md` carries the amendment for Richard to
  confirm. Also recorded: the "stale embedded `collection.ts` copies" caveat on NDA-002 is about a
  path `.gitignore` ignores wholesale, and the Problems panel is the **wrong instrument** for
  cyclic-loop warnings. FINDINGS `TT-i`…`TT-iv`. Gates: runtime **1,767** (1,758 + 9), viewer **579**,
  editor jasmine **2007 / 0**, four catalog gates exit 0, typecheck unchanged at 18 pre-existing
  `TS2307`. Screenshot corpus re-run: 30 surfaces × 2 themes, zero blank frames, both themes correct
- **2026-08-01 (eighth session — live QA for Visual streams C and D; all eight claims hold, and the
  pass found the defect that stream C's own fix had been hiding).** The largest owed item in the
  phase is discharged: two sessions of viewer changes had never been driven in the editor. One
  fixture (`scripts/nda-live-qa/make-visual-cd-fixture.js`) carries all of it. **Every claim
  verified**, and each by a measurement that distinguishes the fix from its absence rather than
  merely coinciding with it:
  - **`Columns` `D1`** — `'1 a 1'` renders **two** equal columns (526px each) and the warnings panel
    reads *`"a" is not a positive number and is dropped, so 3 columns were authored and 2 are
    rendered`*. ⚠️ Authored as a **saved parameter**, so this also exercises the apply-before-the-
    port-exists ordering a corpus row structurally cannot produce.
  - **`Radio Button` `A1`** — clicking the second button moved **both** counters (1→2 and 0→1): the
    deselected button fires `Changed` too. Setting the *group's* `Value` from the graph moved the
    selection and left **both** counters where they were. The same state transition, driven two
    ways, with opposite results — which is what makes it a measurement and not a coincidence.
  - **`Radio Button` `F1`** — `radio-button/no-group` in the panel, and the button renders
    unchecked. ⚠️ **The first reading of this proved nothing**: the fixture gave the lone button a
    `Value`, and the old defect needed `undefined === undefined`. Re-measured with the `Value`
    cleared, which is the configuration that used to render **permanently checked**.
  - **`Group` `B2`** — `Scroll To Index did nothing: there is no child at index 99 — the Group has 2`.
  - **`Page Router` `D1`** — a page parameter of `50% off & more` routes. Hash reads
    `#/detail/50%25%20off%20%26%20more`, the page renders `50% off & more`, no `URIError` anywhere.
  - **`Page Router` `A2`** — editing the Detail page's `title` in place and firing `Reset` rebuilt
    the page and re-ran `setTitle` (`document.title` → `Detail EDITED`). Under the old identity
    comparison both sides were the same mutated object and `Reset` was a no-op.
  - **Stream C `A3`, `Video`** — `wantToPlay: true` on the inner component after a `Play` fired in
    the mount frame of a page being navigated to. ⚠️ **The obvious witness was broken and was
    replaced**: the fixture's `src` is unplayable, and `onPlaybackFailure` fires from the element's
    own `error` event as well as from a rejected `play()` — *and* `play()` only reaches
    `startPlayback` once `canplay` fires, which an unplayable source never does. The failure counter
    would have read `1` whether or not the queue flushed. `wantToPlay` is set by `play()`
    unconditionally and by nothing else.
  - **Stream C `A3`, `Group`** — the queue flushes, and **that turned out not to be enough**: see
    below.

  ⚠️ **`DV-xvii` — `Scroll To Element` has been unable to resolve an ordinary target since the React
  19 migration, and a green corpus said otherwise.** `Group.tsx` resolved its target with
  `noodlChild.getRef()`, which returns the pre-React-19 `reactComponentRef`; since `RUN-001` a node
  reports its root through `setDOMElement`/`getDOMElement` and `reactComponentRef` stays empty. So
  the action arrived on time, exactly as stream C intended, and then reported *"no rendered DOM
  element — it may not be mounted"* about an element that had been on screen the whole time.
  **It took both of this category's own fixes landing before the defect could produce any symptom at
  all** — `withInnerComponent` to stop it being dropped, and stream D's `B2` diagnosis to stop it
  failing silently — and the symptom it then produced named the wrong cause. Fixed by reading
  `getDOMElement`, with the SSR guard moved into that accessor so every caller gets it. Live-verified
  end to end: `scrollTop` **0 → 399** on a `Page.didMount → Scroll To Element`, which is exactly
  `scrollHeight − clientHeight`.
  ⚠️ **The corpus rows for this exact contract were green and had always been green.** They handed
  ...
  perfectly and the resolution not at all. Generalisable, and the sharpest version of it in the
  phase: *a fake is a claim about the shape of the real collaborator, and nothing checks that claim.*
  The corrected fake keeps `getRef` **present and empty**, so a regression reddens instead of
  silently passing again.
  ⚠️ **A stable wrong answer is not a timing bug.** The reason string sent the first read of this
  straight at mount ordering; what settled it was asking the target for `getRef()` after the page had
  been idle for minutes and getting `{current: undefined}` still.

  Two reverts, two correct predictions (3 rows, then 1). Gates: viewer jest **579** (was 577, +2 new
  rows), runtime **1,758 / 13 skipped** unchanged, all four catalog gates exit 0, viewer typecheck
  clean, root typecheck **18 errors — all `TS2307`, all `@noodl-versioning`**, the documented
  pre-existing baseline.

  **NDA-017 criterion 6 closed in the same session, which closes NDA-017 entirely.** The
  semantic-validator half is the `signal-driven-stale-input` rule — `warning`, on by default, 7 rows
  in `rules.test.ts`. ⚠️ **It is deliberately not the rule the criterion asks for.** The criterion
  says *"a `Run` driven by something other than its inputs' producers"*, and built literally that
  flags a Button driving `Run` while the values come from Text Inputs — the single most common
  **correct** graph in the library, and the reporter's own working one. This module treats false
  positives as its primary risk, so the rule narrows to the condition that actually makes a `Run`
  unsafe: the producer is asynchronous (it publishes a completion signal such as `Success`), and no
  connected signal input is reachable backwards over signal wires from that completion. `Failure` is
  not an async marker — most of the library reports one.
  **Measured on the false-positive corpus before being written up: 12 firings, all in one legacy
  project, 0 in the other eight, 0 new errors**, and every firing is the report's own shape — a
  `Condition` reading `User`'s value with `eval` driven by something that does not wait for
  `fetched`. Three of the seven rows are controls, each a graph the literal reading would flag.
  ⚠️ **The handover's editor-jasmine reasoning was wrong and the number was stale.** It said 1,894,
  last measured 2026-07-30, and "still correct not to re-run: nothing since has touched
  `packages/noodl-editor/src`". A one-line `git log` on that path refutes it — NDA-017 §2 itself, the
  whole BCN batch and the `shortDesc` deletion all landed there. Re-run this session: **2007 specs,
  0 failures**, of which 7 are new. No harm done, because the suite was green; the *reasoning* was
  unsound, and "nothing has touched X" is a claim about the tree that costs one command to check.

- **2026-08-01 (seventh session — Visual remediation stream D; nine cells closed, and the category
  runs out of work this phase owns).** In-scope Visual ⚠️ cells **24 → 15**, and all 15 are
  `ERG-001`'s (11), `NDA-014`'s (2), blocked on Richard (1) or half closed by design (1). Closed:
  the four `D1` bare-string contracts (`Columns` layout string, `Page`'s derived `urlPath`,
  `Page Router`'s double decode, `Repeater`'s template), three `B2` deployed-diagnosis cells
  (`Group`, `Repeater`, and `Drag` **with no code change**), `Radio Button`'s missing `Changed`
  output, and `Page Router`'s in-place page edit. `A1`, `A2`, `A3`, `D1`, `G1` and `H1` are now
  closed for the whole category.
  ⚠️ **Three of the nine cells named the wrong mechanism** and one of those would have produced a
  fix that changed nothing — `Page Router` `A2` was filed as identity-versus-value, but
  `getPageInfoForComponent` returns the live `routerIndex.pages` entry and `currentPage` was
  assigned *that same object*, so an in-place edit mutates both sides at once and a value
  comparison would have closed the cell and fixed nothing. Second time this category has produced
  that shape, after `Group`'s `A3` in stream C. `Columns` `D1` described a `NaN` column NDA-006
  had already converted to a silent drop; `Page Router` `D1`'s "decoded twice" is an **uncaught
  `URIError`** on any parameter containing a `%`.
  ⚠️ **`Drag`'s `B2` was stale** — stream B's own `G1` fix had already put its diagnosis on the
  runtime bus. A cell can be closed by a previous session and stay ⚠️ because nobody re-read it.
  **Four defects found by writing the rows, none with a cell**: the Repeater kept the *previous*
  compiled `templateScript` after a syntax error; `Group`'s `scrollToElement` threw a
  `ReferenceError` wherever there is no DOM, on a node declaring SSR `safe`;
  `RadioButtonContext`'s default was a **truthy object**, so a groupless Radio Button with no
  `Value` rendered permanently checked and could not be unchecked; and `Scroll To Element` aimed
  outside its Group silently scrolled a *different* container, because `scrollIntoView` walks to
  the nearest scrollable ancestor.
  **Gates:** viewer jest **577** (was 519; +58 rows in 4 new corpus files), runtime **1,758 / 13
  skipped** (unchanged), all four catalog gates exit 0, Visual C1 **1228/1228** (denominator +1 for
  the new `Changed` port). ⚠️ **The root typecheck reports 18 `@noodl-versioning` module-resolution
  errors — identical at `HEAD`, so pre-existing and environmental, but the previous handover called
  both typechecks clean.** Editor jasmine deliberately not re-run: nothing here touches
  `packages/noodl-editor/src`.
  ⚠️ **Seven reverts, seven correct predictions** (1, 2, 3, 1, 2, 2, 3) — including one row that
  correctly stayed *green* under revert because it guards an implementation choice (deny-list vs
  allow-list URL sanitising) rather than the defect.
  ⚠️ **This file disagreed with itself**: stream C updated the NDA-012 task-table row and never
  added a Log entry, so the table was current to stream C while the Log stopped at stream B. Both
  halves are updated here. **Live QA owed for streams C and D**, and no Visual fix reaches a
  deployed app until `npm run build --prefix packages/noodl-viewer-react`.
- **2026-08-01 (fifth session — Visual remediation stream B; three fix items, 7 cells closed, and one
  fix reverted after live QA).** Commits `d6f483c9` (`Drag` + `Slider` value ports — `DV-x`),
  `34ec5660` (`Text Input` — `DV-xi`), `19da638a` (`Radio Button` + `Page` render bodies — `DV-xii`),
  `f2bc6d0a` (the revert and its rows). **In-scope Visual ⚠️ cells 34 → 27; nodes with a ⚠️ cell
  13 → 12** (`Slider` joins the clean list on cells, though `DV-vi` is still open in its prose).
  **`G1` and `H1` are now closed for the entire category**; of the 27 remaining, **7 are `B3` and
  belong to `ERG-001`**, so 20 are per-node work.

  Gates: viewer jest **506** (was 487 at the start of the session), viewer typecheck clean,
  `catalog:check`, `catalog:merge:check`, `catalog:examples` and `cloud-library:check` all exit 0. No
  runtime or editor source was touched, so those suites were not re-run.

  - ⚠️ **The most useful result was a fix being wrong, and it was wrong the way this phase has already
    written down.** `Text Input`'s `placeHolderOpacity` was recorded as a third `DV-ii` instance:
    declares `0.5`, DB-ii blocks its setter, no `inputProp` route, therefore renders the browser
    default. A mirror went into `initialize` on that reading and was **committed**. Live, with both
    injected per-instance rules deleted from a running preview, the computed opacity is **still 0.5** —
    `assets/style.css` carries `.ndl-controls-textinput::placeholder { opacity: 0.5 }`, exactly as it
    carries Button's padding. **Button's case, not Icon's.** The rule *"a mechanism defect and its
    consequence are two different claims"* was written into `DV-ii` on 2026-08-01, in the same
    category, and the next instance was still diagnosed from the source — because the rule as written
    is epistemic and unactionable. Its operational form is now in the finding: **before filing "the
    declared default does not render", grep `assets/style.css` for a class rule carrying the same
    value.** One query, available both times, run neither time.
  - **The `EMPTY-VALUE-CONTRACT` has two answers for an empty arrival and which one applies is a
    property of the port's type, not a judgement.** `Drag`'s positions and `Slider`'s `Value` **abstain**
    (no representable empty state — the `Radio Button Group` precedent from stream A); `Text Input`'s
    `startValue` **clears to `''`** (a string has one, per the contract's `E1`/`E2` row). Both landed in
    the same session and reading them as an inconsistency would be the mistake.
  - **`G1` closed to zero across the category, and the seven answers were seven different wrong ones**:
    a crash, a throw from the guard itself, a truthiness test, `|| 0`, a raw store, an identity compare,
    and a pass-through into the DOM. One question — *what does this port do when handed nothing* — asked
    of seven nodes by seven authors. **A check that closes to zero across a category is worth reading as
    a class before it is filed as a list**, which is `DV-viii` arriving at the same place from the other
    end.
  - ⚠️ **`Page`'s A3 fix had to be half a fix, and the test suite would not have said so.** SSR renders
    with `renderToString` and **effects never run**, while `injectSeo` builds the served `<head>` out of
    the buffer `setMeta` fills — so moving the render-body call into `useEffect` outright empties the
    meta tags of every SSR and SSG page, with `ssr-inject-seo.test.js` still **green** because it tests
    the string transform and not the producer. Generalisable: **"move the side effect out of render" is
    a browser rule, and SSR is the environment where the render body is the only place a side effect
    can happen.** Three of that item's six rows are the SSR-preserving controls, green at baseline on
    purpose.
  - ⚠️ **Two rows nearly proved nothing, in two new ways.** `Slider`'s `initialize` seeds
    `props.value = props.min`, so on a handle that has never moved "abstained" and "clamped to `Min`"
    are **the same reading** — FINDINGS `B-x` generalised from *a control where the code never ran* to
    *a control where both outcomes coincide*. And `Text Input` has **two modes** switched by whether
    `Set` is connected; `Clear`'s defect lives in one and `startValue`'s in the other, and one fixture
    measured a single mode while reporting on both, reading three genuine controls as failures.
    **Where a port's behaviour is switched by whether another port is connected, connectedness is a
    fixture parameter.**
  - ⚠️ **Both revised cell counts in the worksheet's by-check table were wrong when first written**, in
    the same direction. The fix is that the summary is now derived from the worksheet by one shell
    command rather than maintained beside it — the same failure the unreproducible "31 defects / 41
    cells" figure records, one revision on.
  - Live QA ran on a new committed fixture, `scripts/nda-live-qa/make-controls-fixture.js`, whose nodes
    carry only what an author would have typed — necessary, because every claim in the batch is about
    what an *untouched* port does. Zero renderer exceptions across the session.

- **2026-08-01 (fourth session — Visual remediation begins; six fix items, 13 defect cells closed).**
  Stream A of the post-decision split: every item already measured and pinned, no design work left,
  and no decision inside it. Commits `a1aaa73a`, `05d7bfe6`, `3d0b72ba`, `0ec494ad`, `67e84363`,
  `bbfd6340` — one per item, pathspec-scoped. **In-scope Visual ⚠️ cells 47 → 34; nodes with ≥1
  defect 15 → 13** (`Icon` and `Radio Button Group` are clean).

  **What closed.** `Page Router`'s `resetAsync` (RT-1/RT-2/RT-3 — the throw on an unconfigured node
  and the permanently blank router, four cells); `Dropdown`'s `items` setter (one rewrite, three
  defects, and B1/B2 dissolve because the crash *was* the failure surface); the `Repeater` clearing
  on an empty `Items`; `shortDesc` deleted; `Icon`/`Button` padding (DV-ii); `Component Stack`'s
  `Clip Content` (DV-iii); `Radio Button Group` G1; `Slider` A3. **Four new corpus files, 30 rows,
  and every one was run against the old code first** — 3 to 6 red per file with the controls green,
  which is the only thing that makes a passing row mean anything.

  ⚠️ **Two fixes to the same finding class went in opposite directions, deliberately.** DV-ii
  *deleted* Icon's and Button's padding declarations, because nothing had ever honoured them and
  adding 5px now would move every existing Icon on every canvas. DV-iii *made* Component Stack's
  `Clip Content` real, because the declaration is the documented promise and the rendering was the
  thing that was wrong. **"A declared default that never applies" is not one defect with one remedy**
  — the question is which of the declaration and the behaviour is the one to believe, and that has to
  be asked per port.

  ⚠️ **Live QA earned its place three times over.** It confirmed DV-ii moved no pixels (`Icon` 0px,
  `Button` 5px 20px, both measured on author-untouched nodes), confirmed DV-iii's `overflow: hidden`
  now reaches the element, and produced the session's best single piece of evidence: an unconfigured
  Page Router that previously threw a `TypeError` now puts *"This Router has no Pages configured…"*
  in the editor's warnings panel with node provenance — the runtime error bus, its editor subscriber
  and the panel all working end to end. ⚠️ **It also found a defect the corpus could not: DV-ix**,
  `Component Stack`'s `resetAsync` dereferencing `pages[0].id` behind a `length === 0` guard, which
  is RT-3's exact shape in the sibling node. Its `G1` had been marked ✅ against the case the code
  handles (empty) rather than the case it does not (malformed).

  ⚠️ **Three diagnoses of that one exception were wrong before the right one.** It was attributed
  first to the new Page Router code, then to the editor's compilation traverser, and only resolved by
  finding the throwing line **in the served bundle** and walking backwards to the enclosing node
  definition. **Two nodes in this family have a `resetAsync` and a port called `Pages`, with
  incompatible shapes, and a stack trace naming `resetAsync` does not say which.** The `_b.forEach`
  variant of the same fixture bug surfaced separately in the editor.

  ⚠️ **`shortDesc`'s recorded justification was slightly wrong and the conclusion survived it.** The
  claim was that it reaches the catalog for *no* core node; it reached it for exactly one,
  `Component Children`, from a hardcoded literal in `nodelibraryexport.ts` rather than any node
  definition. `ContextBuilder`'s `enriched?.summary ?? node.shortDesc` still could not fire, because
  that node has an enrichment summary too. **Verified before deleting: all 151 catalog nodes have an
  enrichment `summary` or `description`**, so no readable prose was lost. 57 sites removed, and the
  Data-pass row that pinned the corrected sentence is rewritten to pin the deletion plus a source-scan
  ratchet.

  ⚠️ **Citations drift inside a single phase.** The Page Router pair was cited at `:272`/`:284` and
  was at `:280`/`:292` when the fix was written; `Radio Button Group`'s G1 at `:82` was at `:85`;
  `Slider`'s A3 at `:205` was at `:220`. Same code in every case. **Re-verify a citation against the
  file before working from it, even one written last week by this phase.**

  ⚠️ **The recorded "31 defects / 41 ⚠️ cells" is not reproducible from the worksheet** — counting
  in-scope ⚠️ cells excluding `C1` gives 47. The 47 → 34 pair is derived by one method over both
  revisions and is internally comparable; it should not be compared with the 31.

  **Gates, all eight, before and after:** runtime jest 93/94 suites 1750 → **1751 passing**;
  viewer-react 39 suites/437 → **42 suites/470 passing**; both typechecks clean; `catalog:check`,
  `catalog:merge:check`, `cloud-library:check` clean (all three regenerated — the first alone would
  have passed while the others went stale); editor `test:ci` **2000 specs, 0 failures**. No new
  console noise: the `router-handler.ts:52` deferred-navigate trace was measured at **2 occurrences
  on both sides** by stashing the change, so it is pre-existing.

  ⚠️ **Not done, deliberately:** DV-viii's seven completion signals (`ERG-001` owns the collision
  sweep that names the ports), `Slider`'s own G1 (`newValue || 0` conflating `null` with `0` — pinned
  as-is), `Page Router`'s A2 and D1, and the Repeater's Dynamic-template surface.
...
- **2026-08-01 (NDA-012 Data — CLOSED at 37/37, by a four-worker parallel batch).** The previous
  ...
  and the enrichment layer". That is true, and it is not a reason not to parallelise — it is a reason
  to **take those three files away from the workers entirely**. Four disjoint file sets, the catalog
  regenerated **once** by the orchestrator after all merges, worksheet rows written into per-worker
  notes files and merged centrally. **Zero merge conflicts across four merges.**

  **29 nodes read, 26 new defects, 13 fixed and 13 filed. Data is 37/37 and only Visual (20 in scope)
  remains.** Coverage 12.7% → **96.8%**.

  **Every gate was measured before the batch and again after, and all eight are green**: runtime
  1683 → **1734** passing (93 suites, 0 failed), viewer-react 382 → **403** (36 suites, 0 failed),
  both typechecks clean, all three catalog gates clean, and the **editor suite 2000 specs / 0
  failures both times** — the gate the previous session skipped, so it is evidence now rather than
  an expectation.

  ⚠️ **Two instrument defects outrank every individual node finding.** `worksheets.js:55` derives the
  `B1` pre-fill by matching `/fail|error/i` against output **names**, so a *string* port called
  `Error` satisfies it — and **fifteen categories have now been audited from that column**. And
  `Model.get(undefined)` is **unreachable over a wire**: `sendValue` returns early on `undefined`
  (`node.ts:635`), so the reachable empty values are `null` and `''`. The phase's third recurring
  shape has been described as "the key is missing" throughout, and the `undefined` spelling of it
  cannot arrive by connection at all.

  ⚠️ **A capability hole found by pushing on a worker's stale-premise report rather than filing it.**
  Worker C noted that `net.noodl.HTTP` is `availableIn: ['browser']` while the deprecated `REST2` is
  `['browser','cloud']`, and called it a second reason REST cannot be deleted. Measured across all 57
  cloud-available types, it is worse: `REST2` is also `inNodePicker: false`, and the only other
  candidates are `Sign File URL` and `noodl.cloud.request` — which is the function's **incoming**
  trigger, not a client. **A cloud function has no pickable way to make an outgoing HTTP request at
  all.** The hole predates DA-i: the editor has always filtered deprecated nodes out of the picker
  (`componentmodel.ts:292-295`), and DA-i only made the catalog report it honestly. So DA-i did not
  cause it — DA-i is what makes it discoverable. **NDA-011 decided "deprecated, not deleted" without
  this fact.**

  ⚠️ **The batch's own test run grew a leak, and 0 failures did not catch it.** The runtime suite
  began reporting *"a worker process has failed to exit gracefully"* with 0 failures. Bisected to the
  agent corpus' B1 block: `Optimistic Update — an Apply that works` leaves the transaction open, and
  an open transaction owns a rollback `setTimeout` until something commits, rolls back or deletes the
  node. **The product code is correct** — `_onNodeDeleted` clears it — so this was test teardown, not
  a shipped defect. Fixed by deleting the node, which is also the H1 contract the family is audited
  against, so the cleanup doubles as its control. **A new warning beside a green count is a signal;
  the bar is 0 failures *and* no new noise.**

  **The TSFixme ratchet was held at exactly its inherited level.** Baseline re-measured at `any` **+35**
  (not the +33 the handover carried — the extra two were last session's own corpus file, so that figure
  was stale by the work that wrote it). The four workers took it to +42; all seven were in one file and
  none needed to be an `any` — `const Model = ModelImport as any` was discarding types `model.ts`
  already exports, and six `(graph.node(id) as any)._internal` casts are the exact anti-pattern
  `agent/node-instances.d.ts` was written to replace. Typed, and the ratchet is back to +35/+1: still
  RED, still unowned, **not re-baselined**.

  Per-worker headlines are in the NDA-012 cell; full notes in `WORKER-{A,B,C,D}-NOTES.md`, which are
  committed and carry the "could not verify" lists.

  ⚠️ **Owed, and not covered by this batch: `Run Tasks`.** It is Data's 37th node, audited under
  NDA-009, and it has never been given a worksheet entry or a single port description — the only
  in-scope Data node still at 0%, and `audit/data.md` still shows it `⬜`.
...
- **2026-08-01 (NDA-012 Data — the hold is lifted, and the first family cost the schema).** Phase 34
  ...
  remaining are **Data** and **Visual (20 in scope)**. The error is traceable: BCN-010-NOTES §12, written to
...
- **2026-07-31 (Data HELD — Richard is merging the backends into these nodes).** Told to the session
  ...
  Data nodes**. The audit is held rather than descoped. **Visual (20 in scope) is now the next NDA-012 work**
