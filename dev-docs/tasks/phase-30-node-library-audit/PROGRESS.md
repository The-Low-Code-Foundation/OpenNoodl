...
| Nodes whose implementation has been read | **87** | 8 named by Richard + 5 from his second list + Boolean/Color, then NDA-012's fifteen categories |
| Nodes fully audited against the 12 checks | **87** | 15 of 17 categories **complete**: Variables, Logic, Math, Events, String Manipulation, Interpolation, Javascript, Sensors, Cloud Services, Navigation, Component Utilities, Utilities, **CustomCode (5 of 5)**, Animation, Cloud. **Data is 7 of 37, in progress** (the hold lifted 2026-08-01 when phase 34 landed; re-derived, and it is **37 in scope, not 42** — phase 34 deleted the five `noodl.byob.*` types). Remaining: the rest of **Data (30)** and all of **Visual (29)** |
...
| Animation | 4 / 4 | **4** | **10** |
| Cloud | 3 / 3 | **3** | 4 |
| Data (in progress) | 7 / 37 | 6 | 6 |
| **15 complete + Data started** | **87 / 151** | **73** | **122** |

**Data's 6 defects in 7 nodes is 0.86 per node — but do not read it as a rate yet.** Seven nodes is
not a category, and four of the six live in code NDA-004 §2 had already remediated (`_getACL`'s
mixin, the relation validators, the Array mutators' shared helper), which is Component Utilities'
0.63 for the same stated reason: **prior work, not exhaustion.** The untouched half of the category
— the sixteen agentic/streaming nodes — has never been read by any pass. ⚠️ Counting note, per the
Cloud Services convention: this reports **distinct** defects. `Update Record` is affected by DA-iii
but owns none of its own, so it is not in the "nodes with ≥1" column; its worksheet says so.
