...
| REL-011a | The site builder's fifty-three controls, and the three that were styled | ⬜ | — | [REL-011](REL-011-THE-SITE-BUILDER-SHIPS.md). 🔴 **The shipped artefact carries 53 `net.noodl.controls.*` nodes and styles 3 — and 0 of its 25 text inputs.** Rendered, every field in the product is a label over blank ground: public contact form and all eleven admin screens. Under the bar Richard set this one row is the whole SHITTY→PASSABLE difference. The census spec **must be written to fail at HEAD (3/53) and the failure recorded**, or it is a gate with a hole shaped like the defect |
