...
| REL-004 | The cut — `0.2.2` | 🟢 **TAGGED 2026-09-06** | REL-003, REL-005 | **AC1 met s11** (`5c805978`). Notes + runbook drafted. ✅ **PUSHED and TAGGED 2026-09-06.** `cline-dev` went up in two pushes (`c8180ef1..91b942df`, then `91b942df..10a6c147`), **0 unpushed after**; annotated tag `v0.2.2` at `10a6c147`, pushed as a new tag. Pre-tag checks per runbook §3, all run: no existing tag locally or on the remote, and `HEAD:packages/noodl-editor/package.json` reads **`0.2.2`** — checked at the SHA rather than trusted from the tip. **851 commits since `v0.2.0`.** 🔴 **ORDER HELD: the shelf row was confirmed live BEFORE the tag**, which is what §6 asks for. ⏳ **Not verified by this session: the release workflow and its 15 assets** — `gh` and `api.github.com` are both blocked by the harness here, so the ~16-minute run and `verify-release-assets` are Richard's to read. Superseded: 🔴 **Blocked: `cline-dev` unpushed — 580 at the start of s13** |
