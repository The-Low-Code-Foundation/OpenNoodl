...
🔴 **READ [`TESTING-PASS-2026-09-04.md`](TESTING-PASS-2026-09-04.md) — RICHARD DROVE A PRE-0.2.2
BUILD AND REPORTED NINETEEN THINGS IN ONE AFTERNOON.** Nine were fixed the same day (uncommitted at
the time of writing); four became rows **REL-012 – REL-015**; the rest carry an owner or the literal
word `NONE` in its §2. ⚠️ **Two of the fixes were wrong before they were right**, and §1.1 records
both — including a near-miss that would have made *every blank project* fail to create while the
template shelf looked correct. 🔴 **§1.2 corrects D1's premise**: *"holding costs no action"* was
false about the editor, and the site-builder row below has been amended.

🔴 **READ [`RICHARD-RULINGS-2026-09-04.md`](RICHARD-RULINGS-2026-09-04.md) FIRST — HE RULED ALL
TWENTY-FIVE SCREENS ON 2026-09-04 AND TWENTY-FOUR CAME BACK BELOW THEIR BAR.** The members' area is
**14 PASSABLE, 1 SHITTY, 0 WORTHY**; the site builder is **9 SHITTY, 1 PASSABLE** against a bar that
was already relaxed to PASSABLE for it. **REL-002c, REL-010 and REL-011c all STAY OPEN**, and README
§4's close condition 2 is **not met**. All four §7.3 judgements are answered — three are buildable
now, and **judgement 3 REVERSES D39**, which `tpl001Template.test.ts` §5 exists to catch. ✅ **BOTH DECISIONS
ANSWERED THE SAME DAY (§4 of that file): the site builder goes BACK ON HOLD (owner phase 77, and
holding costs no action), and the members' area PUBLISHES AS V1 AT PASSABLE** — README §4's close
condition 2 is amended in writing, with the one SHITTY page fixed first so nothing ships below the
bar. ✅ **And the seam is named**: *"it still looks like a standard Wordpress bootstrap template,
there's nothing modern CSS about it"* — the V2 brief, and the same sentence he opened the phase-81
baseline with. ⚠️ **He declined three candidate readings a session offered (stock photography, one
typeface, the narrow left column) — do not build them.**
...
|---|---|---|---|---|
| REL-001 | The shelf's first row — publish the association template | 🟢 **PUBLISHED 2026-09-06 11:33:18Z — Richard ran it.** ✅ **Verified on the LIVE shelf, not assumed**: `GET https://community.nodegx.io/api/v1/community/templates` returns `total: 1` and one row — `members-area` / *Members' area* / `starter` / **`fileCount: 100`**, which is the same figure `tpl001Template.test.ts` §1 asserts about the bundle on disk, so the row is of the artefact we gated. ⚠️ The credential was on NEITHER laptop path — `provision.sh` generates it on the box and it points at the server's own loopback, so no pasted URL could ever have worked; [publish-members-area.sh](../release-0.2.2/publish-members-area.sh) tunnels instead. Superseded status follows: 🟡 **CLEARED TO PUBLISH — D2's CONDITION IS MET. Richard runs the command; nothing else blocks it.** ✅ **s45 (09-05) corrected this row: the second look WAS given** — [rulings](RICHARD-RULINGS-2026-09-04.md) §6.5, *"Unsubscribe is passable now"*, so the members' area reads **15 PASSABLE, 0 SHITTY** and the amended close condition 2 is satisfied. This row had said *"BUILT but NOT YET RE-RULED"* for a day after the ruling landed. ✅ **Preconditions re-derived at HEAD 09-05, all green**: four top-level entries, **100 files** (`32×3+4`, the figure `tpl001Template.test.ts` §1's control asserts — the submission file's "94" is the 09-01 number), no `.mcp.json`/`CLAUDE.md`/`.gitignore`, no `.env`/`*.key`/`*.pem`, no absolute developer path or credential-shaped literal, and the artefact **byte-identical to a fresh generate** (`tpl001Template.test.ts` **82/82, EXIT=0**). 🔴 **ORDER: publish BEFORE the tag** — `list()` ships **empty** (both embedded templates held), so a 0.2.2 tagged first has a create wizard and a Templates tab with no rows, under notes that lead on templates ([runbook](../release-0.2.2/PUBLISH-0.2.2.md) §6, corrected s45). ~~the way back is in and driven, and the ~190px void it was meant to close is still ~220px~~ — ruled acceptable with the measurement in hand ([rulings](RICHARD-RULINGS-2026-09-04.md) §6.4, pictures in `verdicts/rel-002c-unsubscribe/2026-09-04/`). ~~judgement 3 lands first.~~ Richard: publish as V1 at PASSABLE; README §4 close condition 2 amended in writing. ~~🔴 `/unsubscribe` is SHITTY and is the condition~~ — **superseded 09-04 by §6.5.** Run the command in [REL-001-SUBMISSION.md](REL-001-SUBMISSION.md). Preconditions re-checked 09-04: four entries, 100 files, no `.mcp.json`/`CLAUDE.md`/`.gitignore` block. ~~BLOCKED ON D2~~ — the look ruled **PASSABLE, not WORTHY** (14 of 15 pages), so the standing decision *do not publish before REL-002 rules* now bites. Richard chooses: publish as V1 at PASSABLE (amending close condition 2 in writing) or another round. [Rulings](RICHARD-RULINGS-2026-09-04.md) §4 | REL-002 | P78 **T5**. Richard drives the publish. **No code blocker**: T6's three fixes are in at HEAD (README §1.3), and the publish path was driven end to end locally by P80/DEF-007 s41. Needs no app release (README §1.2) | ✅ **s25 cleared the one finding s24 handed this row**: the artefact now ships a document outline and landmarks (REL-002c §8), so the thing Richard publishes is no longer a pile of `<div>`s
...
| REL-004 | The cut — `0.2.2` | 🔴 **THE 09-06 CUT FAILED AND WAS ROLLED BACK — AWAITING A RE-CUT** | REL-003, REL-005 | **AC1 met s11** (`5c805978`). Notes + runbook drafted. ✅ **PUSHED and TAGGED 2026-09-06.** `cline-dev` went up in two pushes (`c8180ef1..91b942df`, then `91b942df..10a6c147`), **0 unpushed after**; annotated tag `v0.2.2` at `10a6c147`, pushed as a new tag. Pre-tag checks per runbook §3, all run: no existing tag locally or on the remote, and `HEAD:packages/noodl-editor/package.json` reads **`0.2.2`** — checked at the SHA rather than trusted from the tip. **852 commits since `v0.2.0`** (s58: 851 was the reading at `91b942df`, the commit BEFORE the one tagged). 🔴 **ORDER HELD: the shelf row was confirmed live BEFORE the tag**, which is what §6 asks for. 🔴 **s58 READ THE RUN: it FAILED, and the draft holds 6 of its 15 assets.** Run `34030632269` — linux ✅, win32 ✅, **darwin-arm64 ✗ and darwin-x64 ✗**, `merge mac update feed` ✗, `verify draft release is complete` ✗. Both mac legs died in `webpack.renderer.production.js` with `FATAL ERROR: Reached heap limit` — an OOM, not a signing failure. **0.2.2 CANNOT SHIP AS TAGGED**; the fix is REL-018 and the recovery is runbook §3 (delete the release, then the tag, then re-cut). ⚠️ `gh` and `api.github.com` were NOT blocked in s58 — that harness claim did not reproduce and should be re-measured, not inherited.** ✅ **ROLLED BACK s58, on Richard's instruction, in runbook §3 order**: the draft release deleted, then the remote tag, then the local tag — **all three verified ABSENT afterwards, not inferred from three exit codes**. `cline-dev` is at **`134777d3`**, `package.json` reads **`0.2.2`**, and the REL-018 fix is at the tip. ⬜ **What is left is the re-cut, and it is Richard's**: `git tag v0.2.2 <sha> && git push origin v0.2.2`, then **READ THE RUN** — 15 assets, and `latest-mac.yml` listing all four mac files under one `version: 0.2.2`. ⚠️ **REL-001 does NOT need redoing** — the shelf row is independent of the tag and was confirmed still serving `fileCount: 100` after the failure. Superseded: 🔴 **Blocked: `cline-dev` unpushed — 580 at the start of s13** |
...
| REL-011c | The three site-builder surfaces reach PASSABLE | 🟢 **CLOSED 2026-09-05 — AC3 RULED PASSABLE.** Richard, shown the two seams he named built and photographed against a ONE-VARIABLE before-arm (REL-011 §12 — the 09-03 pictures he first ruled on carried the harness's own blue and gradient rectangles, so comparing against them would have credited the seams with the instrument fix): *"It's passable, let's include it in the 0.2.2 release, add it to the templates menu."* **D1 CLOSED**; `site-builder` is out of `HELD_TEMPLATE_IDS` and the create wizard and Templates tab draw it. 🔴 **Four gates asserted the hold and only ONE could see the change** — `sb-007`'s, read on the SHIPPED provider; the two `fb-005` arms are built on `new EmbeddedTemplateProvider(new Set())` and were green either way, which is right for a vocabulary rule and is also the D1 defect's own shape one level up. All re-aimed, and the hold MECHANISM is now graded directly because nothing on the shipped shelf exercises the `continue` any more. `test:main` **425 suites / 7144, 1 red and it is a peer's untracked `project-examples/lessons/spine.json`, written 19:14 mid-run**. 🟢 **THE DRIVE IS TAKEN** — predicted first, then measured: both surfaces draw `All (1) ✓` / `Site (1)` and one **Site Builder** row, and the wizard's `Next` goes **disabled → enabled** on selecting it (`aria-pressed="true"`). 🔴 **The transition is the discriminating half** — the pre-ruling drive read `Next` disabled on an EMPTY shelf, so the disabled state alone is identical in both worlds. Neither empty-state sentence rendered. ⚠️ The launch was blocked ~4 min by a PEER'S in-flight `nodegx-export/plan.ts` `TS2339` — `reactMounted:false`, the *window-opens-renders-nothing* mode, from another lane's uncommitted edit; their next save cleared it and nothing in their package was touched. Pictures in `notes/rel-011c-unhold-2026-09-05/`. See [REL-011 §13](REL-011-THE-SITE-BUILDER-SHIPS.md). Superseded hold record follows: ⏸️ **HELD FOR 0.2.2 (D1, 2026-09-04) — owner phase 77.** 🔴 **s40 (2026-09-05): THE 9-SHITTY RULING WAS TAKEN ON THE HARNESS'S PALETTE, MEASURED IN PIXELS.** `vib001-site.look.ts:173` seeded a `Theme` row with four invented values, and `#1f6feb` — §3 seam 1's *"the framework's default blue … the one colour a form library emits before anyone has chosen anything"* — is that seed's `colorPrimary`, **byte for byte**, on 9 of the 10 ruled screens. It is not the shipped default (`#2563eb`), not Studio's floor (`#1e4d8c`), and not any of the three chips the theme editor offers. ✅ **The DOOR arm is the control and was right throughout** — no `Theme` row, Studio's navy and ground exactly. Seam 4 is the same story: the "gradient rectangles where photographs belong" are `SWATCH`, a hand-written `data:` URI SVG, copied from `sbr005-sections.look.ts` where per-slot swatches are CORRECT because a probe must tell tiles apart — *in a look harness the picture IS the reading*. `judge()` has been placing all 44 `starter-imagery` photographs in the served dir the whole time (`helpers/judge.ts:229`). **Fixed**: the seed now imports `SITE_THEME_PRESETS.press` (a chip a person can press, and NOT `studio`, which is also the floor — seeding it would stop this arm proving the record beats the default) and four real `work-*`/`texture-*` photographs. 🔴 **The door arm is BYTE-IDENTICAL across the change** (32 PNGs, combined `md5=6d90f842`), which is the isolation. ⚠️ **This does NOT overturn his ruling** — he disliked ten screens and that stands; it changes what a fourth round aims at · 🟢 **§3 SEAM 3 WAS THE PRODUCT, SURVIVED THE INSTRUMENT FIX, AND IS BUILT — in TWO halves, and the first candidate fix was INERT.** `/admin/pages` drew ~260px and left ~640px of **`#ffffff`** — the unpainted `<body>`, no token. `/Pages/Site` has stated `minHeight: 100vh` since SBR-004; `Admin frame`, which `ADMIN_FILL_EXEMPTIONS` calls *"meant to be the whole viewport"*, never got the line. Four arms, sampled in pixels: `#ffffff` → **`+minHeight`** `#f5efe6` (ground fixed, rail unchanged) → **`+alignItems:'stretch'`** `#f5efe6` (**no movement at all**) → **`+rail sizeMode:'contentHeight'`** **`#fffdf9`** (`--surface`, rail runs the window). 🔴 **Why arm 3 was inert**: `addDimensions` defaults `height` to **100%** and `Layout.size` assigns it under `explicit`, so the rail carried a specified cross size — and `stretch` only applies to an item whose cross size is `auto`. **A three-arm run would have certified `stretch` as the fix.** ⚠️ `out-railSizeMode` and its wire are **deleted, not left constant**; §A1 rewritten to the new mechanism with `Admin content` as the control. 🔴 **And no instrument could have caught this**: every admin shot reads `unreachable=0` and is RIGHT — the void is BELOW the content. `/Pages/ThemeEditor` is the one admin screen whose content exceeds the viewport (1126 vs 900), so it fills by CONTENT and shows no void — **and it is the one screen he graded PASSABLE.** §3 read that as composition; it is content length. Gates: `noodl-mcp` **93 suites / 1261, EXIT=0** twice, deterministic; the exemption **mutant reddens exactly one spec**; three regenerations each diffed to exactly the intended nodes. ✅ **`test:main` 423 suites / 7086, EXIT=0** — the baseline exactly, re-taken after a peer's `p18-row8` merge landed; the first attempt read `EXIT=1` / **7075** purely because `nodegx-export/src/analyze/plan.ts` was `UU` and would not compile (**2 failed suites but only 1 failed test — the missing COUNT is what named it**). **Uncommitted**, not asked for. ✅ **s41 (2026-09-05): THE UNATTRIBUTED +3 ARE NAMED, and no suite was run to do it** — they are a PEER'S COMMIT landed BETWEEN the two arms: `5f407849` (the members-area lane, 08:54) adds exactly three `it()` blocks to `tpl001Template.test.ts` (76→79 literal, 82 reported via `it.each(DETAIL)`), and its own commit message records the whole-suite reading it produced — **93 suites / 1260**. So **1257 + 3 + 1 (`rel011cAdminSurfaces` 28→29) = 1261**, and the delta is 3 rather than 4 because that commit's other two specs (`rel002b-fail-closed`, `tpl001-refused-query`) are in `nodegx-backend/tests`, OUTSIDE the mcp population — a count that moved by the 'wrong' amount is what makes the attribution checkable. 🔴 **This is §8's own trap one level up**: the four LOOK arms were flagged as artefact-varied-not-commit-controlled and the two SUITE arms had the identical defect unnoticed, because a test count feels like a property of the tree. ✅ **`git log --since=<the first reading> -- <the package>` before attributing a count delta to your own edit** — two greps here, against a second 93-suite run and the disk that killed the first one · 🟢 **s41: THE FIVE SEAMS ARE AUDITED TO THE END — TWO are the product, and the list is not five.** §8 measured seams 1/3/4 and stopped; **2 and 5 had never been re-read against the corrected instrument**. Read off the 09-05 living shots beside the 09-03 shots he RULED on, then confirmed in the artefact: **seam 2 (ruled rows + outline-secondary pills) is the PRODUCT** — `/Admin/PageRow`'s `row-2` states `borderBottomStyle/Width/Color`, `pill-2` is a Group on `--accent`, `editButton`/`menuButton` are controls on `--surface`; all tokens, so the seed fix REPAINTED the idiom and changed none of it. **Seam 5 (one column, one rhythm) is the PRODUCT and is two numbers**: `shell` is the ONLY node stating a width (`maxWidth: var(--site-measure)`, nothing full-bleed anywhere), and every section of all five kinds is drawn through the single `/Site/SectionView` whose root states `space-4` top and bottom, with `space-8` rowGap above. ⚠️ **Naming where the work would land is NOT a licence to build it** — the template is HELD, phase 77 owns it, §4 says nothing about the look is built until he names the seam. 🔴 **A fourth round aimed at §3 as written would spend two of five items on the harness; what is left to re-show him is seam 2 and seam 5.** See [REL-011 §8–§9](REL-011-THE-SITE-BUILDER-SHIPS.md) · 🟢 **s41: `<h2>` ORDER IS CHECKED AT LAST — the residual this lane registered TWICE with owner `NONE` (§5 item 2, §6 item 3), and it is GREEN AT HEAD: 20 pages across BOTH artefacts, **0 skipped levels, every page opening at `h1`**. A checker over the corpus that exists, and the corpus passes — stated, not buried. 🔴 **A page's headings are mostly NOT in the page**: `/Pages/Site`'s own tree holds ONE, the other five arrive through `/Site/SectionView`, which is not a child of anything — it is the `template` PARAMETER of a `For Each`. **A walker following only children would report every site-builder page as a lone `h1` and call it well-formed** — the answer the gate wants, arrived at by seeing nothing — so a spec pins the cardinality (own subtree 1, resolved 6, from >1 component). Instrument graded BEFORE it was aimed: the skip, the page opening at `h2`, ⚠️ the ascent that must stay LEGAL (`h1 h2 h3 h2 h3` is a second section), and ⚠️ `fault([]) === null` kept as a control because *no headings is not a level fault*. **Mutant on the REAL artefacts** (one `h2`→`h3` in each): **3 red of 29 that all ran**, naming both ends of the step; both files restored byte-identical (`95a666c3` = §8.5's fixed arm, `d8fb0b7a`). One reading, two adapters — the templates ship in genuinely different shapes and `documentOutline.ts`'s header already argues that split. Gates: **29/29 EXIT=0**, `tsc -p noodl-mcp` **0**. ✅ **Full mcp suite TAKEN once the peer's stack came down: 94 suites / 1290, EXIT=0** — written down as a PREDICTION first, then measured. 🔴 **And it closes §8's register row a SECOND way**: 93→94 suites is this file, 1261→1290 is **+29 = exactly this file's specs**, so NOTHING ELSE MOVED and the 1261 baseline the +3 attribution rests on is confirmed by a run that did not exist when the attribution was made. Two files added, none modified. See [REL-011 §10](REL-011-THE-SITE-BUILDER-SHIPS.md)** The built work STAYS: A1/A2/A4–A7, the outline and the two residuals are operability fixes phase 77 inherits, with their gates. 🔴 **AC3 RULED 2026-09-04 — 9 SHITTY, 1 PASSABLE (`/admin/theme`) against the RELAXED PASSABLE bar. THE SITE BUILDER CANNOT SHIP IN 0.2.2 ON THIS RULING**; ⏳ D1 (back on hold?) is Richard’s, [rulings](RICHARD-RULINGS-2026-09-04.md) §4. 🔴 **A1/A2/A4–A7 were all real and none of them is why** — they were operability, which is the floor, not a grade; §3 reads the five seams the nine share and the one PASSABLE screen does not · AC1 + AC2 MET s22 — the SIX product findings it owned are BUILT; AC3 is Richard's.** 🟢 **s28: the ADMIN half of the outline is now rendered-graded** — `sbr010` §7 (3 screens × 2 states) + `sbr009ThemeEditorDrive` §4, both with reverted arms stripping 13/13 tags, on one shared instrument (`documentOutline.ts`, 11 own specs). 🔴 s28 also found **two stale gates**: `sbr010`'s D42 pinned `#pick`, which `3f95a804` silently renamed to `pick-2` (project-wide id uniqueness) — FIXED, matches the id base; and **`ac2-page-editor-drag-drive` is RED at HEAD, 23 of 23**, stale since `bc012147` (2026-09-01) — owner `NONE`. See [REL-011 §6](REL-011-THE-SITE-BUILDER-SHIPS.md) · 🟢 **s29: THAT RED IS REPAIRED AND THE SIXTH SCREEN CAME WITH IT — `ac2-page-editor-drag-drive` reads 28/28, EXIT=0, from 23 failed of 23.** The drive was stale in TWO places and the template was right in both: `runOnChange-in-image` moved to a new `absorb` node (SBR-005, for a stated reason), and the card's kind is now a LABEL — `c.text.split(' ')[0]` returned `Rich` for `richText`, so `sectionIds[-1]` was `undefined` and the expected map came out keyed **`{"undefined": 0, …}`** while the product did exactly the right thing. 🔴 **Neither was repaired with a fresh literal**: `restoreD31` DERIVES its population from the graph (the one `SetDbModelProperties`, the `JavaScriptFunction`s wired into it, the value inputs actually wired to each) and restores **9 keys across 3 nodes** where the typed version restored 3 across 1; `kindOf` INVERTS `SECTION_KIND_LABELS`, imported not copied, and asserts the match is unique and total. Arms still disagree: shipped `rowWrites:0 landed:false`, defect `landed:true rowWrites:137+`. **§8 grades `/Pages/PageEditor`'s rendered outline — HEAD `1/1/1` mains/h1s/h1sInMain, reverted (13 `as` stripped) `0/0/0`** — so **five of six admin screens are now graded at render time**. ⚠️ `mains:0` is also what a BLANK SCREEN reads and the usual `<nav>` control was NOT available (the stripper takes `/Admin/Shell`'s `as: 'nav'` too), so the three section cards are the control instead. ⚠️ **Registered, owner `NONE`**: `/Pages/Setup` still has no drive that loads it; and `typecheck:backend-tests` is unrunnable here because of **one import** — `noodl-mcp/src/server` via `helpers/site-drive.ts` — bisected with single-file `tsc` programs, with the UNEDITED drive OOMing identically as the control. See [REL-011 §7](REL-011-THE-SITE-BUILDER-SHIPS.md) | REL-011a, REL-011b | [REL-011](REL-011-THE-SITE-BUILDER-SHIPS.md) §REL-011c. Re-photograph the three SHITTY states at four widths on the FIXED artefact — 🔴 the existing pictures show **D40 as well as the design** (the Judge's harness never applied the template's `bodyScroll`; run record in `phase-81/verdicts/sbr-005/2026-09-03/`). **A session cannot close this: Richard rules** · 🟢 **s22 — A1, A2, A4, A5, A6, A7 BUILT, plus A8 and A9 found by looking at the after-arm.** Gate `rel011cAdminSurfaces.test.ts` **28/28 over the SHIPPED `site-builder.content.json`**, written to red and MEASURED red: the same file with HEAD's artefact copied over the fixed one reads **26 failed, 2 passed**, and the two survivors are the two preconditions that exist to survive. 🔴 **A2's mechanism was ONE node's EMPTY parameter bag** — `/Admin/SectionRow`'s `Image preview` stated no size, `Image` defaults to `contentSize`, so it rendered at its SOURCE's intrinsic width (1200px here, a phone photograph is 3×) and `layout.ts:82` left it `flexShrink: 0`. That made the editor's content column 1216px wide at every viewport; `min-width: auto` is `min(own stated width, content minimum)`, so above 1280 the rail came out of the content and at or below it `main` stopped shrinking and **pushed the rail off the right-hand edge**. The fields narrowed and `Save page` did not because they are two different boxes and only one was clamped. 🔴 **A1: the shell had NO breakpoint of any kind**, and `Columns`'s `smallLayout` — the runtime's only one — is the wrong tool because it would turn the fixed 240px rail into a PROPORTION and grow it to ~380px at 1900. `Screen Resolution` + one function moves four ports together below 760. 🔴 **A5's photograph showed a FIXTURE defect that was hiding the opposite PRODUCT defect**: `absorb` never writes `image` on a gallery, so the seeded `{image}` gallery was unreachable through the product — and a REAL gallery showed no picture at all beside a count saying how many. ⚠️ **The render caught a regression the first build shipped**: `flexWrap: 'wrap'`, authored only to make `rowGap` authorable, put Title above Slug and the live preview under the fields at **1900 as well as 390** — `flexWrap` now moves with the direction, and an arm pins it. Gates: `template:site-builder` exit 0; noodl-mcp **92 suites, 1224 passed, exit 0**; `typecheck:mcp` 0; both look harnesses exit 0. 🟢 **All sixteen admin shots now read `unreachable=0` with a named heading**, from `admin-page phone 2379`, `admin-theme phone 362` and `headings: []` on all twelve. Pictures: `phase-81/verdicts/vib-001/2026-09-03/` + `sbr-005/2026-09-03/`; the before-arm is committed at `d4d3400c`. ⏳ **AC3 — his ruling — is untouched, and A3 stays phase 81's** · 🟢 **s23 — the TWO RESIDUALS this row registered are BUILT**, one string and one node: the theme editor's preview no longer says *"follow the fields on the left"* (false at 390 since A9 stacked them above it), and `/Site/ContactForm`'s fixed `h2` is gone — 🔴 **the defect was a heading the AUTHOR CANNOT EDIT, not two headings**: `/Site/ContactSection` already carries one, wired to the record and mounted on `showHeading`, so a person who typed "Get in touch" into the panel read it twice on `/contact-only`. Pinned by a shared `headingsOf` predicate; **two arms redden on the reverted source, measured**. Gates: `typecheck:mcp` 0, `template:site-builder` exit 0, noodl-mcp **92 suites / 1226**, `sb007Template` byte-identical. 🔴 **And FIVE literal gates were RED AT HEAD, four of them not this phase's** — `sb017` acceptance 6 (38 vs 35, s22's), `sb-007` disjoint node ids (401 vs 393, s19's then s22's), `sb-018 (3)` naming `/Site/SectionView body` after SBR-005 deleted it, `sb-018 (1)` missing P77 AC2's `DropAt`/`DropIndex` and three wires (`12cc718a`, 2026-08-30), and `aib-007`'s table missing `noodl.cloud.listusersinrole` (P80/DEF-005 `ab677258`, phase CLOSED). All five fixed, each decomposition measured at its commit; baseline re-measured on the committed artefact first. **`template:site-builder`, the full noodl-mcp suite and both look harnesses are green with all five red — the two runners that see them are the ones a template session does not reach for** · 🟢 **s27 — THE DOCUMENT OUTLINE, registered by s26's handoff with owner `NONE` and built.** Measured at HEAD first: **7 of 7 pages carried exactly one `h1`, 0 of 7 carried a `main`, and 0 of the artefact's 26 `as` tags were held by any assertion anywhere in the repo.** 🔴 **A near-complete outline is harder to see than a missing one** — the members' area read *0 semantic tags in 100 files* and every instrument agreed; here a heading census reads perfect and the missing thing is the region a reader jumps to in order to skip the rail. Fix: `as: 'main'` on the six admin screens' existing content column, plus a NEW NODE (`siteMain`) on `/Pages/Site`, whose `shell` also holds the nav band and the colophon — and `as: 'nav'` on `/Admin/Shell`'s rail, which is the **control** as much as the markup. **400 → 401 nodes, 553 connections both sides, 26 → 34 tags.** Gate: **§12 of `sb007Template.test.ts`, nine specs** — two censuses, a containment walk, a nav-outside-main sweep with its five placing pages named, a stray-`as` sweep, two predicate controls (the live decoy is `/Admin/Shell`'s Group whose **id is literally `main`** and which carries no `as`) and **two mutants**: the artefact exactly as it shipped (§12.1/§12.5 and the swallow sweep stay GREEN, which is why nothing caught it) and the shortcut fix that moves the landmark onto `shell` (passes every census; §12.4 alone sees it). 🔴 **And the browser half — a parameter is an intention.** `sb008-public-site-drive.test.ts` gained **§6** on the four page loads it already takes against a real backend: **0 of 4 loads had a `<main>` on the reverted arm, 4 of 4 at HEAD; 0 → 4 with the `<h1>` inside it**, beside a `<nav>`-in-document control that reads 4 of 4 in BOTH arms. Readings: `sb007Template` **71/71**, noodl-mcp **92 suites / 1242**, `typecheck:mcp` **0**, `sb008` drive **24/24**, `vib001-site.look.ts` **exit 0, 40 shots — 192 of 200 PNGs and 96 of 100 text dumps byte-identical**, the eight that moved being `/admin/messages`'s printed clock (`20:15` → `23:07`). ⚠️ **`artefactMd5` is NOT the pin**: it hashes `nodegx.project.json` alone and moved on the living run (a random backend port is written into that file) while staying identical on the door run whose graphs changed just as much — what excludes a stale artefact is that `authorSiteTemplate` writes `SB005_COMPONENTS`/`SB006_COMPONENTS` through the door **at run time**. ⚠️ **Registered, owner `NONE`**: no drive grades the six ADMIN screens' rendered outline (§6's four loads are all the public catch-all; the mechanism carries, the per-page authoring is §12's job), `<h2>` order is unchecked in both templates, and neither outline gate is general — the next template on the shelf starts at zero. See [REL-011 §5](REL-011-THE-SITE-BUILDER-SHIPS.md) |
| — | **The site builder** — unheld, and on the shelf | 🟢 **UNHELD 2026-09-05 — it ships in 0.2.2** | — | Richard ruled it **PASSABLE** on the honest pair and asked for it in the release and in the templates menu. `HELD_TEMPLATE_IDS` keeps `hello-world` alone, for the separate and unchanged reason that it **is** the blank project. Release notes and `PUBLISH-0.2.2.md` §6 both moved — §6's *"`list()` returns zero rows"* reading is **quoted and marked superseded, not deleted**, because it is what D1 was taken on. **Publish-before-tag survives with a narrower reason**: the shelf is no longer bare, but the notes headline the members' area by name. Superseded hold record follows: ⏸️ **HOLD REINSTATED 2026-09-04** | 🔴 **The 09-03 lift is REVERSED.** It was granted with the bar relaxed to `PASSABLE` so this template could ship in 0.2.2; Richard then ruled its ten screens **9 SHITTY, 1 PASSABLE** and put it back on hold (D1). README §3's hold list is live again, owner **phase 77**. 🔴 **"Holding costs no action" was FALSE, and it was false about the EDITOR** — the template sat in `EmbeddedTemplateProvider`'s map and `list()` returns the whole map, so every 0.2.2 user would have been offered the held template in the create wizard. The ruling was right about `templates/` and the community shelf and wrong about the shipped app. ✅ **Holding now costs exactly one line** — `HELD_TEMPLATE_IDS`, added 2026-09-04, gated by `template-needs-backend.test.ts`; phase 77 unholds it by deleting one string. See [TESTING-PASS-2026-09-04](TESTING-PASS-2026-09-04.md) §1.2. ⚠️ **The built work stays** — the hold is about shipping, not about the code. Superseded lift record follows: 🔴 **Richard reversed the 2026-08-31 hold** (*"I want to publish the association page template but not the site builder yet"*) and set the bar at **literally `PASSABLE`** — a deliberate relaxation of `phase-81/README.md` §83 (*"PASSABLE is recorded progress, never a close"*), put to him before it was written down. **It applies to REL-011 only; REL-002c and REL-010 keep the WORTHY bar.** ⚠️ **README §3/§4 still describe the hold as live** — they were not edited because `README.md` carries a peer's uncommitted paragraph and a pathspec commit would sweep it. Whoever lands that must strike §3's site-builder rows and add REL-011 to §4 |
| REL-012 | The lessons nobody receives | 🟢 **ALL 5 ACs GRADED 09-05** | — | [REL-012](REL-012-THE-LESSONS-NOBODY-RECEIVES.md). The Learning tab reads a **per-user install register** that nothing seeds, and `project-examples/lessons` is in **neither `files` nor `extraResources`** — so both built, gated, driven bundles ship in **no artefact a user receives** and a fresh install opens Learning empty. The community channel does not rescue it: production answers `total: 0`. ⚠️ **Packaging is only half** — the tab reads the register, so AC1 without AC2 reports success against a still-empty tab. Richard: *"package bundles, absolutely critical… even just 3 right now for 0.2.2 would be fine"* **Built 09-04, GRADED 09-05**: AC2/3/4 by `jest tests-unit/rel-012` (**30/30, EXIT=0**); **AC1 on a real packaged artefact** — `Contents/Resources/lessons`, 4 bundles, 122 files, source 124 − 2 `.mcp.json` reconciled, against the **Aug-20 build on disk that has no `lessons` at all**; **AC5 DRIVEN both arms** — clean profile shows 3 lessons and *Log a thing* opens, the reverted arm lands on *Your path* with no register file. **Three bundles ship, so his number is met.** |
| REL-013 | The Templates tab | 🟢 **BUILT + COMMITTED `14c9bd90`** (board was stale ⬜ — corrected 09-05 off the commit and the specs on disk) | REL-001 (for content) | [REL-013](REL-013-THE-TEMPLATES-TAB.md). `Templates.tsx` renders one hardcoded sentence and imports no registry. ✅ **Half of Richard's "two birds" is already done** — the create modal reads `templateRegistry` through `useProjectTemplates`. The work is the **layering**: the tab lives in `noodl-core-ui` and cannot import the editor hook, so rows must arrive as props from `ProjectsPage`, and the launcher must report its active tab so the community fetch stays **off cold start**. 🔴 **AC4: the shipped empty state is the real state** until REL-001 publishes |
| REL-014 | The value the field destroys | 🟢 **BUILT + COMMITTED `9d58c505`** (board was stale ⬜ — corrected 09-05) | — | [REL-014](REL-014-THE-VALUE-THE-FIELD-DESTROYS.md). `parseFloat("var(--space-4)")` → NaN → `setParameter(name, undefined)` in `Dimension.ts` **and its twin** `NumberWithUnits.ts`. Two gestures reach it — the reset dot, or touching one character in the field — and **it cannot be typed back**, because non-numeric text is discarded silently. Colour fields survive, which is why it reads as arbitrary. ⚠️ **The editor authored those values** (`ElementConfigRegistry.applyDefaults` stamps `var(--…)` on every new node). 🔴 **Scope is the DEFECT only** — the picker, the panel and the whole editing surface are [FIX-015](../phase-66-0.1.7-bug-fixes/FIX-015-THE-TOKENS-NOBODY-CAN-EDIT.md)'s ruled scope and its green-lit successor phase |
| REL-015 | The shelves you can fill | 🟢 ⏳ | — | [REL-015](REL-015-THE-SHELVES-YOU-CAN-FILL.md). People, replays and tutorials all have real tables and working reads; **every gap is on the write side**, and for two of three the only writer in existence is a hardcoded array in `seed.mjs`, which is destructive to the whole DB. 🔴 **People reverses ruling D8** (*"approval-first is a queue only one person can clear"*) — record the reversal quoting it, and amend the two code comments that assert "never approval-first". 🔴 **Do NOT redefine `profile_meets_bar`** — four other callers, one with an AC saying it is gated by that "and nothing else". ✅ The replay model is **already YouTube-aware** (`splitVideoUrl`); Richard's two links need a ~30-line publisher. ⚠️ **Chat is NOT here** — that is FB-013, phase 75. 🟢 **s33 — PLATFORM MEASURED AND THE EDITOR BUTTON BUILT.** `0025` applied for the first time: **15/15 `EXIT=0`**, plus 411 across the 14 suites the change reaches, plus a **reverted arm** (2+1+3 red, restored byte-identical). 🔴 **Richard ruled twice** ([§8](RICHARD-RULINGS-2026-09-04.md)): D3 **removed `profile_meets_bar` from `listDirectory`** (approval alone lists you; the FUNCTION is untouched, AC3 holds) and D4 **gated `/u/<handle>` on approval too** — so a public profile that never asked to be listed now has **no page**, and `AccountForm`'s copy was rewritten because it said the opposite. ✅ **`POST /api/v1/me/listing` was NOT built and would have been a defect** — the write already existed on `/me/profile`; only the **GET** was missing. ⏳ **AC9/AC11 — RICHARD SUPPLIED THE LINKS 09-06** (`youtu.be/fqmHH36ndc0` + the channel): **one video, not two**, and a **channel URL is not a video** — `splitVideoUrl` would store a provider id that renders a broken player, so AC9 closes on one row or on a second link he gives. The channel has **no slot on the platform** (measured: nothing in `src/app` references one) and is linked from the release notes and the changelog instead. Commands ready in [REL-015 §6.8](REL-015-THE-SHELVES-YOU-CAN-FILL.md); **not run — both write to the deployed database and nothing is deployed**. AC11 still needs **prose, which a session must not invent** (the 200-char floor exists because UNI-020 shipped `Placeholder body.` three times) |
| REL-016 | The tokens nobody can edit | 🟡 **BUILT + DRIVEN, gate is Richard's** | — | [REL-016](REL-016-THE-TOKENS-NOBODY-CAN-EDIT.md). FIX-015 slice 1, built in place s39: the Design Tokens panel's rows are editable (blur-commit, undoable, resettable). **Driven 09-05 — all five pre-ungate checks PASS**, so the "expect a bug list" ruling did not hold. 🔴 **The gate is not what the row thought**: `if (config.devMode)` at `router.setup.ts:435` is dead in **every** build — `config-dev.js` is required by nothing — so `design-tokens`, `file-explorer` and `undo-queue` register **nowhere**, dev included, measured as an A/B on the experimental-panels list. ⚠️ And un-gating is **two** gates: `experimental: true` keeps it behind a per-user Settings toggle that is off by default. **Richard's call** whether 0.2.2 removes the dead flag (which exposes two undriven panels too) |
| REL-017 | **The second row on the shelf — TPL-003, the landing pages** | 🟢 **EMBEDDED 2026-09-05** — ruled in (*"add it to the template picker and ship with 0.2.2"*), then *"make it one of the packages templates like the members area and site builder"*: `embedded://landing-pages` beside the site builder, no publish. Editor specs 172/172, MCP gates 81/81, both typechecks 0. 🟢 **COMMITTED 2026-09-06 `9da450b0`** with the site-builder unhold (87 files); on the settled tree afterwards `typecheck:editor`/`:runtime`/`:mcp` all **EXIT=0** and `test:main` read **432 suites / 7218 tests EXIT=0** | REL-001 (same act, second slug) | Richard's ask the same day: *"one more template to ship with 0.2.2 … no backend … small business or freelancer landing page, presentation and a contact form … a few different pages within the same template for different typical 'top 3' landing page types."* [TPL-003](../phase-78-the-templates/TPL-003-THE-LANDING-PAGES.md): `templates/landing-pages/`, 21 components, 3 pages (`/`, `/business`, `/launch`), a `mailto:` contact form driven in a real browser, 82 `EDIT —` marks in a generated `START-HERE.md`. ✅ **His first look the same day: Freelancer and Business "good", Launch "too much like the freelancer one … lazy" — REBUILT** (product window beside the promise, alternating text-and-window rows, big numbers on ink, a dark plan; no card grid) and re-photographed, 78/78 after. `tpl003Template.test.ts` 39/39 · appearance+DEF-038 39/39 · `tpl003-landing.look.ts` 2/2 · `typecheck:mcp` 0. 🔴 **RIDES 0.2.2, does not gate it** (REL-008 precedent). ⬜ Category: `site` proposed, `starter` is what he ruled for the first row — ONE ruling. ⬜ Copy is §E-ii instruction-shaped, not a fictional business — his call. Pictures: `phase-81/verdicts/tpl-003/2026-09-05/` |
| REL-018 | **The macOS release legs run out of heap** | 🟢 **FIXED, GATED AND REPRODUCED s58** | REL-004 | 🔴 **This is what stopped 0.2.2.** Both mac legs of the release OOM in the renderer production webpack: V8's default old-space is derived from system RAM, the `macos-26-arm64` runners give node **2048 MB** (the crash dump's `1993.3 (2096.8) MB` is exactly `--max-old-space-size=2048`), the 16 GB linux/windows runners give ~4 GB, and the renderer bundle grew past 2 GB during 0.2.x. ✅ **Excluded: signing/notarisation** — the crash is in a step that runs BEFORE either, and win32 (which signs) passed. ✅ **Excluded: the mac toolchain** — that morning's nightly built both mac legs green on the same runner image, from `main`, which is **1823 commits behind**. ✅ Fixed in [`webpackHeapCeiling.ts`](../../../packages/noodl-editor/scripts/webpackHeapCeiling.ts) — 4096 MB, chosen because it is what the legs that SUCCEEDED already had, not as a guess; applied in `build.ts` so the nightly and a macOS laptop get it too, not just the release workflow. Gated by `tests-unit/rel-004/webpackHeapCeiling.test.ts` (**10/10**), red on four arms. ✅ **PROVEN ON RICHARD'S OWN MACHINE (macOS arm64), as a control pair on the real webpack config**: `--max-old-space-size=2048` → **EXIT=134, `FATAL ERROR: Reached heap limit`** (the CI failure reproduced); `=4096` → **EXIT=0, compiled in 57 s, peak RSS 3404 MB**. The laptop could not have found this on its own — 16 GB gives node a 4144 MB default, so the bug is invisible locally unless the heap is forced down, which is exactly why it reached a tag. ⏳ Still ungraded: a full mac CI leg WITH signing and notarisation — but that half was never implicated (win32 signs and passed, and the crash precedes both steps) |
| REL-019 | **The tabs left behind — Community and Learning** | 🟢 **CLOSED s61 — ALL FOUR ITEMS DONE.** Editor half built/gated/driven s59; platform half deployed s60; the *First look* tutorial PUBLISHED s61 from Richard's own prose and verified live (`/api/v1/community/home` 1 replay + 1 article, page 200, 0 iframes cold, listed on `/tutorials`) | REL-015 (the People route it deploys) | [REL-019](REL-019-THE-TABS-LEFT-BEHIND.md). Richard, 09-06, third time of asking: *"the community and learning tabs are sad, they've been left behind"*. 🔴 **Chat could not be written to from the launcher because `ProjectsPage` never passed `composer`/`reply`** — the hook composed them (`66b5f1df`), the view drew them (`e39393c1`), no gate on the wire. Wired, and a source gate now sits on the wire. 🔴 **"Add yourself as a person" fails because production is the 28 Aug build** (`deployed.json` → `91d8b0c`); `/api/v1/me/listing` 404s there and the card says *unreachable*. 🔴 **The shelf led with spine lesson 8** — `list()` sorts newest-install-first and nothing read `spine.json`; now `models/lessonchain.ts` reads it and the shelf is numbered, in order, with *Next on the spine* and five filters. 🔴 **Path steps were inert**; a step whose slug is on the shelf now opens it, and *Explain this for me* is withdrawn from every step once the community answers `unavailable` (no projector on nexus-1). 🔴 `/articles/<slug>` → `/tutorials/<slug>` — the old URL never existed on the web. Head is an identity card with a **Sign in** door (it used to say *"sign in below"* over nothing); readout is tiles with D21's words intact; replay/video rows carry a **drawn** poster, never a fetched thumbnail. `rel-019` **6 files / 51 specs**, neighbours 26 suites / 487, `test:main` **439 / 7287 EXIT=0**, two mutants red + restored by md5, driven via CDP signed in. ⏳ **LEFT, all in the community repo (edits there were DENIED by the session's classifier)**: §4.1 flip the eight shipped lessons to `ready` + revise the two tests written to fail that day · §4.2 `video_url` on `articles` so the *First look* tutorial has a home · §4.3 `ops/deploy.sh` (0025+0026 run for the first time; count public profiles first, D4) · §4.4 publish the meet-up replay and the tutorial through the tunnel — commands with the YouTube facts already in the file; **the tutorial body is prose Richard writes**, not a session |
...
### REL-017 — The second row on the shelf (TPL-003, the landing pages)

**Opened and built 2026-09-05 (s56)**, on Richard's ask that day. The whole record — what it is,
the ACs with their readings, the publish command, and an eight-row register with owners — is in
[`TPL-003-THE-LANDING-PAGES.md`](../phase-78-the-templates/TPL-003-THE-LANDING-PAGES.md) under
phase 78, where templates live. This row exists so the launch can see it.

- 🔴 **Rides, does not gate** — the REL-008 shape. If the cut is ready before he has looked, it
  waits for 0.2.3 and nothing is undone.
- ✅ **Ruled in 2026-09-05** on the rebuilt launch page, then **embedded** on his second ask. Category
  `site` on the template object. The copy question was not raised and ships as built (§E-ii).
  ⚠️ He described the members' area as a packages template; it is not — it is the REL-001 shelf row
  and still needs his publish. Said in the reply and in TPL-003.
- ⚠️ **Three findings for the product, owner `NONE`**, in the task file's register: a component
  instance has no `this` output and the door does not say so (L1); `gridAutoFit`'s 280px minimum
  is four columns at the measure, so every row of three is three-of-four (L3); the Judge's
  `unreachablePx` reads 180 on every phone shot from mobile emulation's `innerHeight`, not the page
  (L4).
- ⚠️ **Nothing committed by the session** — Richard commits. Pathspecs: `templates/landing-pages/`,
  the three `landing-pages.*` files under `noodl-editor/.../template/templates/`,
  `EmbeddedTemplateProvider.ts` (⚠️ carries a PEER's uncommitted unhold edit too — commit it whole),
  `tests-unit/tpl-003/`, `tests-unit/sbr-001/template-needs-backend.test.ts` (same caveat),
  `packages/noodl-mcp/tests/tpl003*.ts`, `packages/noodl-mcp/tests/templatePins.ts`,
  `packages/noodl-mcp/tests/templateAppearance.test.ts`,
  `packages/noodl-mcp/tests/def038SettledTemplates.test.ts`,
  `packages/nodegx-backend/tests/tpl003-landing.look.ts`, `scripts/generate-landing-template.ts`,
  the verdict PNGs, and the docs files. ⚠️ The `template:landing` line in `package.json` was
  swept into a peer's commit `3e0eaa0c` (SYL-003) — already on the branch, nothing to do.
