...
now, and **judgement 3 REVERSES D39**, which `tpl001Template.test.ts` §5 exists to catch. ⏳ **Two
decisions are HIS and OPEN (§4 of that file): does the site builder come off 0.2.2, and does the
members' area publish at PASSABLE?** Do not build look work until they are answered.
...
|---|---|---|---|---|
| REL-001 | The shelf's first row — publish the association template | 🔴 **BLOCKED ON D2, 2026-09-04** — the look ruled **PASSABLE, not WORTHY** (14 of 15 pages), so the standing decision *do not publish before REL-002 rules* now bites. Richard chooses: publish as V1 at PASSABLE (amending close condition 2 in writing) or another round. [Rulings](RICHARD-RULINGS-2026-09-04.md) §4 | REL-002 | P78 **T5**. Richard drives the publish. **No code blocker**: T6's three fixes are in at HEAD (README §1.3), and the publish path was driven end to end locally by P80/DEF-007 s41. Needs no app release (README §1.2) | ✅ **s25 cleared the one finding s24 handed this row**: the artefact now ships a document outline and landmarks (REL-002c §8), so the thing Richard publishes is no longer a pile of `<div>`s
