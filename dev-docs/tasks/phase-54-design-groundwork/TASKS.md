...
| DSG-008 | [DSG-008-THE-ICONS-NOBODY-COULD-SEE.md](DSG-008-THE-ICONS-NOBODY-COULD-SEE.md) | nine dark-on-dark glyphs, one mechanism, and an instrument so the next one is found by measuring | ✅ **FIXED AND MEASURED 2026-08-13.** Six hosts set `fill` (or nothing, or `color: inherit` with nothing above) where the rule is `color`; worst was `SearchInput`'s magnifier at **1.16:1** — literally black. 🔴 The four seen most were `IconButton:disabled` painting its glyph **`--theme-color-bg-4`, a background token**, which is undo/redo and the trail's back/forward — **invisible on every fresh session**, and WCAG-exempt, which is why nothing caught it. `BasePanel` now sets `color` beside its `background-color` as a backstop. **12 panels × dark, 5 × light: 0 below 3:1.** Instrument kept: `scripts/devtools/icon-contrast.js` — ⚠️ this class **cannot be found by reading**, since a losing rule looks exactly like a correct one |
