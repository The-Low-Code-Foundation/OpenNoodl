...
| F52 | ⚠️ **`render:report` reads at scrollTop 0**, where stuck and not-stuck are pixel-identical. It would have reported *"Rendered clean"* for **four dead or illegible** sticky variants. Any scroll-dependent recipe needs a scrolled read; the ~140-line probe is worth promoting to `scripts/devtools/` | ✅ **closed** — `scripts/devtools/scroll-probe.js`, `npm run render:scroll`. Variants B/D/F rebuilt and proven red (all three still *"Rendered clean"* to `render:report`), shipped recipe green on both viewports. ⚠️ **F52 is also the number of a different, still-open finding** — see F64 in [NOTES-SCROLL-PROBE.md](NOTES-SCROLL-PROBE.md) |
| F53 | 🔴 **`zIndex` on a sticky band is structural, not styling.** Without it the geometry is perfect, every rect-based check passes, and the band is unreadable because sibling bands paint through it. Only `elementFromPoint` catches it | ✅ **detectable** — `sticky-occluded` fires at 3 of 3 sampled points on variant F and names the blocker. The finding stands; it is no longer invisible |
...
3. ~~**The scrolled-read probe (F52)** promoted into `scripts/devtools/`.~~ ✅ done —
   `npm run render:scroll`, [NOTES-SCROLL-PROBE.md](NOTES-SCROLL-PROBE.md). ⚠️ Two things it changed:
   the boot half of `render-report.js` is now `withRenderedPage()` and both tools share it (41/41 MCP
   module specs green, `render:report` byte-identical), and **the probe reports its own blind spots** —
   a page that does not scroll and a page with no sticky element both say *"nothing was tested"*
   rather than passing. Written the obvious way it would have reported clean on both.
