...
| DEF-035 | ⬜ open | **P77 [D13](../phase-77-the-site-builder-rescue/DEFECTS-THE-SITE-BUILDER-FOUND.md#d13)** second half | **Which ports *exist* still depends on whether the editor window got focus with the backend up.** `SchemaHandler` (`utils/schemahandler.ts:72`) fetches the backend schema on **`window-focused`** and writes `dbCollections`; `resolveSchemaPortContext` reads it and `recordFieldPorts` mints one port per column. 🔴 **DEF-028 does NOT close this** — it makes the export read a *settled verdict about the ports that exist at that moment*, not a port set that is a function of the project. A build taken with the schema cold still differs from one taken warm. Measured by s17 of P77: the census moved **19 unhealthy → 4** on its own, with no edit, between 14:18:32 and 14:22:57 | every **author who exports before the editor has spoken to their backend** |
