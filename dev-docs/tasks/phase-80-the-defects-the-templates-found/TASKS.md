...
## 🟢 46 of 46 ROWS IN THIS TABLE ARE ✅ — DEF-042 and DEF-047 built s45 on Richard's ruling; DEF-044/045/046 s44, 2026-09-03

🔴 **`DEF-043` is a CLAIMED id with no row here** — phase 82's, and the count above does not include it.

⚠️ **The phase was closed at 37 on 2026-08-31 and then grew.** DEF-038 was promoted out of
[UNOWNED-ROWS-TO-MEASURE.md §10](UNOWNED-ROWS-TO-MEASURE.md) — a row that had been **measured and
left unowned** — and built the same session. **s43 did it again with DEF-039** (§7), and that row's
third bullet *reversed* on measurement, and **DEF-040** (§8), whose second bullet found a worse
failure than the row it belongs to, and **DEF-041** (the drag door), which measured **DISPROVED as
a live defect**. **DEF-042 (§2) is MEASURED and left 🧭 OPEN** — its fix needs a ruling, and the
row's own recommended fix measured **wrong**. 🔴 **A closed phase is not a sealed one**: **s44 did it a fourth time with DEF-044** (§9, the no-home template), which needed no ruling — Richard's
2026-08-31 words already covered the door nobody had built on. **Four** of that file's eleven rows are still
unowned, and the next free id is now `DEF-048`. **s45 did it a FIFTH time with DEF-047** — the
stale-function double render, owner `NONE`, registered as *“trivially fixed”* and left; the reason
it was left is that a one-line filter would have been **ungradeable**, which is also why it
survived being screenshotted. 🔴 **`DEF-043` IS CLAIMED BY PHASE 82 AND IS NOT A ROW HERE
YET** — the deprecated `DbCollection`'s copy of the failed-fetch defect, measured but unwritten (see
[§ the header of UNOWNED-ROWS-TO-MEASURE.md](UNOWNED-ROWS-TO-MEASURE.md)). **s44 took `DEF-044` rather than
`DEF-043` because a peer claimed the id in this repo while s44 was building in another one** — check the
claim line, not just the highest number in this table.

DEF-036 closed on Richard's ruling that part 3 means **the canvas** (already shipping, driven s39);
DEF-007 closed on **AC2 driven** against a locally-run instance of the real community platform.

🔴 **Closed is not fully measured.** Several rows carry boundaries that survive them — DEF-036 §10's
four, DEF-007 §8.1's finding, and the ⬜ items further down this file. **Say them whenever a row is
quoted**, and re-derive this count from the status column rather than trusting this banner:
...
| DEF-005 | ✅ **BUILT s27** | [Membership is a category the graph cannot express](DEF-005-MEMBERSHIP-IS-UNEXPRESSIBLE.md) — **both halves; AC1–4 met, AC5 (TPL-001's member list) outstanding and never the acceptance** | P78 D2, D3 | every **membership app** |
...
| DEF-007 | ✅ **CLOSED s41 — AC2 DRIVEN, the last open AC on the row.** A curated template was published to a LOCAL instance of the real `nodegx-community` platform (publishing is a **DB-credential** act, not a session-token one — the s38 blocker was a wrong assumption, and nothing went to the live service), drawn by the picker over real HTTP badged **Community**, installed, and the editor opened on **`App`** — the home, at **index 3 of 4** and alphabetically last. 🔴 **The CONTROL is what makes it a measurement**: the identical template with `rootNodeId` deleted lands on `/#__page__/…`, index **0**; only that field was varied. See §8. 🔴 **The control also found a real hole — a curated template with NO home publishes, lists and installs SILENTLY** (`publish-project-template.ts` has no gate; s38's refusal guards only the PERSON's door in `shareAsTemplate.ts`; no install-side rescue) — registered as **UNOWNED row 9, owner `NONE`**, and part of that fix lives in the **nodegx-community repo**. ⚠️ Earlier: **BOTH RULED ITEMS BUILT + DRIVEN s38** (refuse at publish; scream on deleting a home page — see §7). ✅ **AC2 IS NOW CLOSED (s41)** — the picker CAN reach a curated template; the claim that it could not was an assumption about which service had to serve it. 🔴 s38 found that **undo does NOT restore the home** (row 8 of UNOWNED-ROWS). ✅ **AC4 CLOSED s37** (seam table moved into `projectLoadSeam.ts` beside `applyPatches`, **plus the test that fails on an unregistered `fromJSON` call site** — mutant-proved). ⚠️ **The two ruled items below read as unbuilt because this cell was written the moment they were ruled — §7 records BOTH built and driven in s38.** 🧭 **RULED 2026-08-31: refuse at publish.** A project with no home page must not publish as a template — *"it's a very basic requirement"*. 🧭 **AC4 RULED: move the seam table INTO the codebase (beside `applyPatches` / `dev-docs/reference/`) AND add a test that fails when a new path reaches `fromJSON` without `applyPatches`** — the table already answers AC4 (**4 of 6 load paths skip the upgrade**); its problem is that it lives in a task file nobody will maintain. 🔴 The test is the point — same decaying-hand-list shape that let Checkbox be missed. 🆕 **And a NEW item on no row before today: make the app scream when someone tries to DELETE a home page.** The existing preview error is after the fact; this catches the moment of deletion. ⚠️ **Two separate items** — publish catches a project that never had a home, deletion catches one that had a home and lost it | [A project means one thing on disk and another once loaded](DEF-007-DISK-AND-LOAD-DISAGREE.md) — **§6 seam named · §3.2 BUILT s31, AC3 GREEN (65 → 0, §6.2) · §3.3 struck** · ⚠️ left: **AC2** (curated template through the picker, needs publishing) and **AC4** (the seam's doc names each path). ⚠️ The pin is on the **site-builder generator only**; `tpl001Template.ts` writes a **v2 directory**, needs its graph in legacy shape first, and is **unmeasured**. 🧭 **Richard: the pin arms P77 D33's six `/Pages/ThemeEditor` rows in the editor** — no change to the deployed site, which already ran them | P78 D9 residual · P77 D5 · **P77 D11** · **P77 D33** | the **next template** |
...
| DEF-038 | ✅ **BUILT s42 (2026-09-03)** | [The other template generator means one thing on disk and another once loaded](DEF-038-THE-OTHER-GENERATOR-DISAGREES.md) — **DEF-007 §3.2 settled `toTemplateContent` (the site builder) and `prepareArtefact` (TPL-001) was never touched, so the members-area artefact disagreed with its own editor load in 57 stored parameters.** Re-measured at HEAD before building — still **57 writes / 107 family nodes**, against the control's **0 / 97**; both arms non-zero on `familyNodes`, which is what makes the 0 an absence rather than a broken instrument. Fixed with `pinRunOnValueChangeDefaultsInDirectory` (a v2-directory settling beside the legacy one, writing by node id **with the cardinality asserted** — a plan entry matching two files or none throws), one call in `prepareArtefact`, and the artefact regenerated: **57 insertions, 0 deletions, 18 files, nothing else moved.** 🔴 **Gated across BOTH generators by a sweep that DERIVES its population** (`templates/*/nodegx.project.json` + the editor's `*.content.json`) rather than listing it — §10's own instruction, because *"this row recurs with a third template"*. 2 mutants: restoring the pre-fix artefact reddens exactly the two members arms (2 failed, 3 passed), and removing the single call regenerates an artefact **byte-identical to pre-fix HEAD** — which is what proves the fix is in the generator and not a hand-edited file. 🔴 **The extraction of `readAsLegacyProject` into `templateArtefact.ts` is part of the fix**: its old home imports the whole site-builder authoring surface at module scope, and the gate's first run died `Tests: 0 total` on a peer's half-saved edit to an unrelated file. ⚠️ **Known hole, stated**: a third template written as a `.ts` module (like `hello-world.template.ts`) escapes the sweep. ⚠️ The 81 authored `false` values were **not** audited — the pin never touches an answered key, and whether each is what its author wanted is a different question nobody has asked. Gates: def038 5/5, tpl001 72/72, sb007 62/62, `typecheck:mcp` exit 0 | **P80 UNOWNED §10** | every **person who installs the members' area and opens it** — a page that fetched on load stops fetching, and they changed nothing |
| DEF-039 | ✅ **BUILT s43 (2026-09-03)** | [A `connections.json` the editor cannot read is one it destroys](DEF-039-A-FILE-THE-EDITOR-CANNOT-READ-IS-A-FILE-IT-DESTROYS.md) — **a component whose wires carried field names the v2 loader does not know loaded with no error, drew, and was written back to disk as `{}`×8.** Measured with a control that behaves: arm A destroyed, arm B (correct names) byte-identical in the same run, arm C (an id naming no node) lossless. 🔴 **Bullet 3 FLIPPED.** `getConnectionHealth`'s two callers hand it **opposite shapes** — `exportComponent` sends ids and no nodes, the canvas sends nodes and no ids — so `c.sourceId ? c.sourceId : c.sourceNode.id` dereferenced `undefined` in **both** directions for **different inputs**. Arm C is safe on the export path and lands on it from the canvas, where `createFromModel` throws even earlier in `resolvePorts()` and **takes every later wire in the component with it**. That half is reachable by a bad merge, so **the row's rank went up**. Bullet 1: `SchemaValidator` has **one call site in the entire editor** (AI-assistant authoring) and is not on the load path — a missing call site, not a coverage hole, and the shipped schema already produces exactly the right 8 messages. Bullet 2: the `{}` write is a **save of a model that was already empty at load**. 🔴 **The obvious fix was checked before it was written and is NOT available**: refusing the component drops it from the project, and a component missing from the project is `removeDirRecursive`d by the next save — the rescue would lose the whole directory instead of eight wires. Built: an end guard (**extracted to `connectionEnds.ts` so it can be graded at all** — `NodeGraphModel` reads `platform.getUserDataPath()` at module scope), a canvas skip, preserve-verbatim on **both** ends of the round trip, and the load warnings surfaced to the person instead of `console.warn`. 3 mutants, each killed by its own arm — including one that reverts only the exporter half, because either end alone would have read as fixed. Gates: def-039 8/8, `--findRelatedTests` over all 7 changed files **37 suites / 517 tests**, `tests/io/*` **190 passed** incl. `roundtrip-fidelity`. 🔴 **`test:ci` OWED** — the canvas skip is renderer code jest cannot reach. ⚠️ The canvas half is **source-derived, not driven**. | **P80 UNOWNED §7** | anyone whose v2 project was **written by something other than this editor** — an agent, a generator, a merge |
| DEF-040 | ✅ **BUILT s43 (2026-09-03)** | [Undo brings the node back and not the home](DEF-040-UNDO-BRINGS-THE-NODE-BACK-BUT-NOT-THE-HOME.md) — **delete the home node, press *"Delete it anyway"*, undo: the `Group` came back and `getRootNode()` was still `null`** (driven, DEF-007 s38). The home pointer is cleared by a module-scope `Model.nodeRemoved` listener — **a side effect of an event, not part of the undoable act** — so nothing in the undo group knew to reverse it. 🔴 **Not tidiness: DEF-007 §7.2 shipped a confirm rather than a refusal on the argument that undo exists.** It did not. Bullet 1: **the same hole on `removeComponent`**, and `import-engine/apply.ts` reaches it unrefused. 🔴 Bullet 2: **the listener is NOT reachable for a nested node, and that IS the defect** — `removeNode` drops every descendant from `nodeMap` and notifies only for the top, so a home inside a deleted Group left a **dangling root rather than a null one**: `getRootNode()` answers a node no component contains and `toJSON` writes its id into `rootNodeId`. The row guessed this *"may be worse"* — it is. Bullet 3: **neither shape** — a globally undoable `setRootNode` would put a second writer on a pointer four other paths set deliberately, so each remover restores the home in the undo action it already pushes. Built: a subtree-aware listener, the home restored in **both** removers' undo (read before the removal, set after the re-add), and the predicate **extracted to `rootNodeRemoval.ts` so the two call sites share one definition** — they must agree exactly, and neither host module can be imported outside Electron. 2 mutants: identity-only reddens the nested arm, always-true reddens **the CONTROL** — which is there because `home === null` is also what the defect produced, so the positive arms alone cannot tell a fix from a coincidence. Gates: def-040 6/6, `--findRelatedTests` **12 suites / 146 tests**, `typecheck:editor-tests` and `typecheck:editor` both exit 0. 🔴 **`test:ci` OWED** — `tests/models/RootNodeUndo.test.ts` is registered and **has never run**; it can only run inside Electron. | **P80 UNOWNED §8** | anyone who **deletes and immediately regrets it** — and every import that replaces the component holding the home |
| DEF-041 | ✅ **DRIVEN s43 (2026-09-03)** — 🔴 **row DISPROVED as a live defect** | [The drag door and STYLE-002's defaults](DEF-041-THE-DRAG-DOOR-AND-THE-DEFAULTS.md) — the row claimed *"the same control is accessible or not depending on which gesture created it"*. **The call-site count re-derives exactly at HEAD; the sentence about a person does not.** 🔴 **A built-in node type cannot be dragged onto the canvas at all**: `onDrop` is fed only by `PopupLayer.dragItem`, whose sole canvas-bound source is the Components panel carrying a **project component** — and `ElementConfigRegistry.has('/Site/Nav')` is `false` (measured live), while `DragItem.nodeType` is declared, read by `getDragItemComponent`, and **assigned by nothing**. 🔴 **The control read ZERO first.** Both arms created nothing until the drag used 120 steps — `dispatchDrag`'s default 12 jump the pointer off the item before the panel's 5px threshold — so **a zero control would have confirmed this row by accident**. Armed: Checkbox card drag → **nothing**; `Nav` drag → `/Site/Nav`; Checkbox card **click** → all 8 defaults incl. DEF-001's `borderColor`. Built anyway, one line, as a **latent** trap: `applyDefaults` on the drag door, before the add, mirroring the picker. The row's own candidate fix (fold into `seedNewNode`) was **refused** — its own warning about undo ordering is correct and the restructure risks a visible regression to close a hole nothing can reach. 🔴 **The post-fix drive caught the fix not being live** — HMR replaced the module but the held `NodeOperations` instance kept the old prototype; only after a renderer reload did the presence control (checkbox → 8 defaults) pass, beside a no-regression control (`/Site/Nav` → `{}`). Without it this would have shipped as working on a green source sweep. Gate **derives** its population (mutant 2 = a fabricated third door, killed). Gates: def-041 4/4, 2 mutants. | **P80 UNOWNED (drag door)** | **nobody today** — the tripwire test is what keeps it that way |
| DEF-042 | ✅ **BUILT s45 (2026-09-03)** — measured s43, ruled and built the same day | [A throwing listener kills the whole backend](DEF-042-A-THROWING-LISTENER-KILLS-THE-BACKEND.md) — **reproduced at HEAD** against the committed `dist/cli.js`: two PUTs declaring the same cloud component, PUT 1 → **200**, PUT 2 → **no response**, status → **refused**, `EXIT=1` on `Duplicate component name`. 🔴 **The row's central recommendation is WRONG and the measurement is what shows it.** It called *"catch the rejection so the PUT answers 400"* small, clearly right and ruling-free — that try/catch **has been in `loadWorkflow` since WFA-001** (verified in the BUILT bundle) and **never runs**: no `Failed to load workflow` line precedes the crash. `EventSender.emit` is **async**; `GraphModel.addComponent` calls it **without awaiting**; the listener's throw rejects a promise **nobody holds**, so it never joins the chain the catch watches and Node exits instead. ⚠️ The async stack trace ends `at async WorkflowRunner.loadWorkflow`, which reads as *inside the guard* — that frame is where the chain was **created**, not where the rejection was **delivered**. ⚠️ **`graphmodel.ts` has 15 un-awaited `this.emit(...)`**; `componentAdded` is only the one a person has met. 🧭 **The row's proposed split does not exist**: awaiting the emit makes `addComponent` async and ripples into **`noodl-runtime`, shared with the viewer**; a backend-local `unhandledRejection` guard alone returns `{success:true}` for a half-written registry — a quiet failure in place of a loud one, on the one path designed so *"only a complete success swaps it in"*. **Recommended: await the emit and let the existing catch do its job; the blast radius is the thing to weigh.** Gate pins the **mechanism**, not the crash (3/3) — and its third arm was rewritten after the first version **measured the harness**: jest installs its own `unhandledRejection` handler. ✅ **BUILT s45 on Richard's ruling, and 🔴 THE BLAST RADIUS THAT MADE IT NEED ONE WAS NEVER COUNTED.** Counted before writing: `graphModel.addComponent` has **exactly ONE caller** in the runtime, both viewers and the backend — twelve lines above it — and that caller (`importComponentFromEditorData`) was **already `async`**, as was every link above it, **already awaited**: `loadWorkflow` → `CloudRunner.load` → `setData` → `importEditorData`. So the guard was **starved by two missing `await`s in one file**, and the feared "ripple through the shared runtime" was one `await` on one line. Built: `addComponent` async + the emit awaited + its caller awaits it. ⚠️ **NOT built, both named in the code**: the other **15** emits stay un-awaited (`_onNodeAdded`'s among them, from a `forEach` inside `addComponent`) — the class is **narrowed, not closed**, and the gate's last arm MEASURES that so the row and the code cannot drift; and `editormodeleventshandler.ts:225` still detaches on the editor's live-edit path. ⚠️ The PUT answers **500** not the row's 400 — `updateWorkflow` maps every failure to 500 and covers disk errors too. Gates: **drive against a FRESH build, both arms** — before A 200 / B **no response** / status **ECONNREFUSED** / exit 1 with **no guard line**; after A 200 / B **500 with the real error** / status **200** / **alive**, and the *"previous version left in place"* line 🟢 **present** — the catch dead since WFA-001 now runs. New runtime spec **4 red of 5 → 5/5**; runtime suite **146/2566** (5 red = SQLite floor); viewer-react **87/1138**; viewer-cloud **13/219**; 4 × `tsc` 0; editor `test:ci` **2943 specs, 5 failures at pinned seed 13203 — identical to the pre-change baseline**. 🔴 The absent guard line was **re-measured** after the instrument changed: `safeLog` writes stdout, so the first stderr-only reading would have been an artefact. Drive committed at `scripts/devtools/drive-def042-duplicate-component.js`. ⚠️ **The semantics question is untouched** — whether two projects on one backend should collide at all. | **P80 UNOWNED §2** | anyone with **two projects on one backend** — the product advertises the sharing |
| DEF-044 | ✅ **BUILT s44 (2026-09-03)** | [A curated template with no home reaches the shelf, and installs silently](DEF-044-A-TEMPLATE-WITH-NO-HOME-REACHES-THE-SHELF.md) — promoted out of [§9](UNOWNED-ROWS-TO-MEASURE.md), where Richard’s **2026-08-31 ruling already covered it** (*“A project with no home page must not publish as a template”*, unqualified) and only the **builder’s** door was built on it. Re-measured at HEAD first: `rootNodeId` appears **0 times** in the curator’s `publish-project-template.ts`, **0** in the queue’s promote path, and **0** in this repo’s `createFromTemplate.ts` — no gate at either end and no install-side rescue. 🔴 **§9’s open question is answered: the queue does NOT inherit the editor’s gate** — the submissions route is an ordinary authenticated write and the editor is one client of three. Fixed at `publishProjectTemplate`, **the one seam both curated doors pass through** (the curator’s script, and `promoteTemplateSubmission` inside its transaction). ⚠️ **A SECOND COPY of `templateHomeStatus` in a second repository with no import path between them** — the drift is recorded, not solved. 🔴 **Every publish fixture in the community suite had no home** (6 files, 8 payloads, 0 home pages). Gates: 96/96 + 92/92 + tsc 0, and the **reverted arm exits 1 with exactly the 4 refusal specs red and every control green**. ✅ **REL-001 is not blocked — checked**: `members-area` carries `rootNodeId: app_root`. ⚠️ **The fix is in the `nodegx-community` repo** | **§9**, this phase | a **curator**, and everybody who installs what they publish |
| DEF-045 | ✅ **BUILT s44 (2026-09-03)** — 🔴 **the row as recorded was WRONG** | [A cloud function in a folder had two addresses and only one worked](DEF-045-A-CLOUD-FUNCTION-IN-A-FOLDER-HAS-TWO-ADDRESSES.md) — the register said *"declared, listed, ticked — and **unreachable over HTTP**"*. **It was reachable.** Driven at HEAD against a real `BackendService`: `POST /functions/site%2FpublishPage` → **200**, and every in-product caller percent-encodes (`newFunctionFromStep.ts` had measured that half in SB-003 and neither row knew of the other). What actually 404’d is the **RAW** address `POST /functions/site/publishPage` — what a person, `curl` or a third-party webhook types — with a body indistinguishable from *"no such function"*. ✅ **DEF-015’s card was right to tick it.** Fixed by `functions/:name` → **`functions/*name`** plus a final-segment rest-capture; 1 route of 92 uses it, and a control proves exact-length matching is untouched everywhere else. 🔴 **The two consumers of the pattern STRING** (the access-log label, the rate-limit class) were updated and **their own suites cannot run on this box** — `better-sqlite3` is absent and Node is 20.11.1, so all six route-table suites die before any assertion (**59 failed / 40 passed, every failure that one error**); both assertions are re-made in the new spec, which runs. Gates: **before exit 1 (1 red of 4), after exit 0 (6/6)**, backend `tsc` 0. ⚠️ **`tsconfig.tests.json` cannot complete here** — OOM at 4 GB (134), SIGTERM at 9 GB (143); CI runs it | **unowned register**, raised by DEF-015 s11 | anybody calling a nested function from **outside** the product |
| DEF-046 | ✅ **BUILT s44 (2026-09-03)** | [A value that has not changed re-runs the node](DEF-046-AN-UNCHANGED-VALUE-RE-RUNS-THE-NODE.md) — a code node handed `title: "Pricing"` twice ran twice and **wrote a database row each time**, and it contradicted `run-on-value-change.ts`’s own justification for keeping `Run` (*“an async re-fetch that returns an identical value fires no change”*). 🔴 **The row’s SIZING was wrong and that is worth more than the fix**: *“four lines per family, twelve families”* re-derived as **39 call sites across 15 files in at least THREE shapes** — value setters (the defect), **nine EVENT handlers with no previous value to compare**, and a state guard. One checkbox governs six setters on Query Records. **Richard ruled: value setters only.** Built as ONE deciding function (`valueDidChange` + `shouldRunOnValueChanged`), **primitives only** — an array mutated in place is the same reference, and a comparison that included it would make every collection node go quiet on a real change. **17 setters converted**; the event handlers, the always-object ports and two nodes that **already guarded** are listed with reasons. Gates: **before exit 1 (3 red of 7, every control green), after 7/7**, full runtime suite **146 suites / 2566 passed** (5 red = the SQLite environment, see DEF-045 §3.1), 2 × tsc 0, kit types unchanged. 🔴 **A stub of `Node.prototype` went red** — the shape to check wherever a NodeInstance is impersonated | **unowned register**, raised by P77 s28 D26 | every **builder**, and every app with a side effect behind a value port |
| DEF-047 | ✅ **BUILT s45 (2026-09-03)** | [A stale cloud function renders twice — as a tick and as a warning](DEF-047-A-STALE-CLOUD-FUNCTION-RENDERS-TWICE.md) — the backend card mapped `backendFunctions` with a green ✓ and, one line below, mapped `stale` — **which is defined as a subset of it** — with a warning triangle, so a function the backend still serves that the project no longer has was drawn as a healthy row *and* a problem row, one line apart. 🔴 **The register's citation was the wrong way round**: `:123` *defines* `stale` from `backendFunctions` and filters nothing out of it. 🔴 **Why it survived being screenshotted: nothing here could grade it.** The classification lived in the JSX, `tests-unit` is plain Node (no jsdom, and `Icon` alone makes a spec fail *to run*), and **no spec in `tests/` or `tests-unit/` names this component at all** — so *“trivially fixed”* was true of the filter and false of the fix. Built as a pure `cloudFunctionRows` returning ONE ordered row list, and a single `rows.map` in the view: a name drawn twice is now **unrepresentable** rather than filtered, and the cardinality is asserted. Every shipped `data-test` id and the row order are unchanged. ⚠️ **One deliberate behaviour change** — `stale` is measured against **every** cloud component rather than endpoints only, because the old set could contain a name the project holds as a *worker*, of which its own sentence (*“not in the project”*) would be false; the populations coincide today. Gates: **7/7 exit 0**, editor `tsc` exit 0, and the **reverted arm reddens exactly the 3 defect assertions with 4 controls green**. 🔴 **`test:ci` OWED** — last read `2943 specs, 4 failures, seed 09154, HEAD ef5e90f5` (the AIX-006 floor, by name), which **predates this change** | **unowned register**, found by DEF-015 s12 | anybody who **deletes a cloud function** and then looks at the card |
...
|---|---|---|---|---|
| DEF-028 | ✅ closed s30 | [A build depended on when it was taken](DEF-028-A-BUILD-DEPENDS-ON-WHEN-IT-WAS-TAKEN.md) — **the export filter now settles connection health before it reads it.** `getConnectionHealth` reads no ports: it asks `WarningsModel`, and *"no warning recorded"* is also its answer when nothing has been **evaluated** — a verdict written only on a ~2 s debounce that nothing forced to land. 🔴 **The register said "three callers"; there are eight** across seven entry points, including two AI-authoring paths D13 never named (`sandboxExport`, `componentBench`) — which is why the flush went **inside the filter**, where the ninth caller gets it by construction, and deliberately **not** inside `getConnectionHealth`, whose other caller repaints once per wire per frame. 🔴 **The flush is unconditional on purpose**: "run the pending pass if one is scheduled" is a no-op in the window D13 is about — a freshly imported graph has never scheduled one, and that is what a build taken just after opening a project is made of. Measured unfixed in the real model: **`length = 2` vs `1`**, two exports, no edit between them; 7 failures (4 AIX-006 floor + 3) → **4, the floor exactly**, on a different seed. 🔴 **AC3 passed VACUOUSLY on the first run** — `exportConnection` renames `from/toProperty` to `source/targetPort`, so the helper compared `'undefined->undefined'` and an absence assertion was satisfied by a list of the wrong shape; a positive control (`toContain('image->image')`, a wire that survives in both directions) now proves the helper sees real names. ⚠️ **Two halves left open and registered, not silently absorbed: DEF-034** (a `level:'warning'` wire is dropped exactly like an error one) and **DEF-035** (the `window-focused` schema fetch still decides which ports *exist*) | **P77 [D13](../phase-77-the-site-builder-rescue/DEFECTS-THE-SITE-BUILDER-FOUND.md#d13)** — cost phase 77 **four sessions of evidence pointing two ways** | every **author who exports or deploys** — two builds of one project, different contents, no error |
| DEF-029 | ✅ **s29: BUILT — the runtime can take a file, and the shape it emits was already decided.** A `File Drop` port group on the five visual nodes that share pointer events (`Group`, `Text`, `Image`, `Circle`, `Video`): `Accept File Drops` + `Accepted file types` in, and `Files Dropped` / `Files Rejected` / `File` / `Files` / `File Name` / `File Type` / `File Size In Bytes` / `Is Dragging Over` out. 🔴 **The handoff scoped this as needing 'a decision about what a dropped file is' and it needed none** — `Open File Picker` already emits a browser `File` on a `type: '*'` port and `Upload File`'s input already documents itself as taking exactly that, so the new ports emit the same shape under the same names and wire into the upload path that exists. 🔴 **Off by default, and that is the whole safety argument**: `preventDefault` on `dragover` is what makes an element droppable *and* what stops the browser navigating to the file, so making it unconditional would change every existing project's behaviour on a stray drop; the handlers are installed always and return at event time — not render time, because a port set while the preview runs does not re-render the node. Everything but the one checkbox is a **dynamic port**. ⚠️ **Gating dynamic *outputs* had no precedent here** and was verified end to end before being relied on: the editor's filter keys by name with no plug, and both `PortsTab.buildRows(model, direction)` and the connection popup's `ConnectionBar` pass output plugs through it. 🔴 **Three details separate working from nearly working** — a drag-depth counter (dragging onto a child fires `dragleave` on the parent, and every real drop zone has a child because the label is one), a `dataTransfer.types` check (or dragging selected text lights the zone), and `Files Rejected` rather than silence when nothing matches (`Open File Picker`'s Failure-Contract clause: no completion signal for an operation that produced nothing). 🔴 **The drag events are deliberately outside `blockTouch`** — it blocks every event it covers including unlistened ones, so folding them in would let a child with Block Pointer Events silently kill the drop zone it sits inside. **Eight mutants, each reddening a different row.** 🔴 **A ninth 'survived' and had not**: renaming the dynamic `outputs:` key is a type error, so ts-jest compiled nothing, the suite never ran, and a `✕`-only grep read that as survival — ✅ **report `Tests:` cardinality beside every mutant**. ⚠️ **Graded against `Text`, not `Group`** — `group.ts` cannot be imported in a test at all (an ESM `.js` scroll plugin under a bare `ts-jest` preset; `Tests: 0 total`, which reads like a missing file), registered as an unowned row; Group is covered by a row driving the shared mutator directly. ⚠️ **`File Drop` is classified `plumbing`** beside `Pointer Events`, so it folds into `Advanced CSS` — an appearance judgement and a one-line revert, **Richard's call**. ⬜ **UNDRIVEN in a real editor** ✅ **DRIVEN s36 — the half that mattered is taken.** `cdp.js` gained **`dropfile`** (`Input.dispatchDragEvent`, with `--probe` to read drag-over state mid-drag and `--leave-to` to walk the drag out); a mouse `drag` can never carry files, because a file drag has no mousedown in the page. On a running preview: accepted `.png` gave `Is Dragging Over` **true** during the hover, then `File Name` `drop-me.png` / `File Type` `image/png` / `File Size In Bytes` **78 — the real byte count on disk** / `Files Dropped` fired; a `.pdf` on the same zone fired **`Files Rejected` and not `Files Dropped`**, leaving the value outputs untouched; and the **control** — same file, same command, zone with `Accept File Drops` off — reported `Is Dragging Over: false`, changed nothing, and did not navigate. 🔴 **The control was run AFTER a known-firing signal**, or it would have been indistinguishable from a broken harness. Drag-depth driven over the zone's child label. ⚠️ **`dragCancel` leaving the hover true is NOT reported as a defect** — CDP delivers no `dragleave` for it, and a genuine drag-out released it correctly; unmeasured, not broken. ⬜ **Panel half still owed** (is the `File Drop` group folded into Advanced CSS in the rendered DOM?) 🟡 **HALF-DRIVEN s35** — all ten ports are live in a running editor's node library, grouped `File Drop` and labelled (`Accept File Drops`, `Files Dropped`, `Is Dragging Over`…). 🔴 **The half that matters is still owed**: no file has been dragged onto a running preview. `cdp.js` exposes no `Input.dispatchDragEvent`, and a synthetic DOM event would bypass the `preventDefault`-on-`dragover` behaviour this row is about. ⚠️ A peer reports the ports fold into **Advanced CSS** in the rendered panel | **P77 [D15](../phase-77-the-site-builder-rescue/DEFECTS-THE-SITE-BUILDER-FOUND.md#d15)** — **this unblocks P77 SBR-007 AC3**, whose row reads "still blocked by D15 alone". Measured word-boundary before building: `onDrop`/`onDragOver`/`onDragEnter`/`onDragLeave`/`dataTransfer` = **0** each against known-firing controls `onClick` = 37, `onMouseDown` = 8; `draggable` = 7 is `react-draggable`, the `Drag` node's pointer gesture, not file drop | [A file cannot be dropped](DEF-029-A-FILE-CANNOT-BE-DROPPED.md) — every **builder who wants a drop target** |
...
| DEF-031 | ✅ closed s28 | [A `Text` cannot be ellipsized](DEF-031-A-TEXT-CANNOT-BE-ELLIPSIZED.md) — **a `textOverflow` port, and the render that makes it mean something.** 🔴 The port alone would have been **inert**: `Text` forced `white-space: pre-wrap` + `overflow-wrap: anywhere` after the author's style, so `ellipsis` reached the DOM and did nothing. Default `wrap` is deliberate — a `clip` default would have made this a second **DEF-033** ✅ **DRIVEN s35** — port live in a running editor's node library (enum Wrap/Clip/Ellipsis, default `wrap`); on a genuine render the label goes **42px/2 lines → 21px/1 line**, computed `ellipsis`+`nowrap`+`hidden`, **`scrollWidth` 1419 vs `clientWidth` 893**, and the wrap arm's computed `text-overflow` is `clip` — **`wrap` never reaches the DOM**, AC3 in a browser. ⚠️ **Panel DOM still unread** (the graph is a `<canvas>`; no node was selected) and the `…` glyph was not resolved at capture resolution. 🔴 **The drive found a new row: the port is INERT in a running preview until reload, in both directions — registered as DEF-037** | **P77 [D22](../phase-77-the-site-builder-rescue/DEFECTS-THE-SITE-BUILDER-FOUND.md#d22)** | every **author who wants a single-line label** that degrades instead of wrapping |
...
| id | status | source | what it is | bites |
|---|---|---|---|---|
| DEF-033 | ✅ **BUILT BY P18 2026-09-01 (s67)** — `substring.ts` declares `-1`, description rewritten, catalog regenerated, graded by `substring-declared-default.test.ts` (declaration = `initialize`'s state; `result` rows kept as controls). **HANDED OFF s37 — P18 owns and builds it; P80's involvement is closed** — 🧭 **RULED 2026-08-31: show the truth.** Align the declared default to `-1` so the panel shows what already runs; node behaviour unchanged, nothing already built moves. 🔴 **P18 builds this, not P80** — the ruling was given here and must be relayed. ✅ **RELAYED s37** into [EXP-011 §36.2](../phase-18-code-export-v2/EXP-011-PICKER-COVERAGE.md) (owner `NONE` → `P18`) and into P18's [NEXT-SESSION-PROMPT](../phase-18-code-export-v2/NEXT-SESSION-PROMPT.md). The change is one line — [`substring.ts:52`](../../../packages/noodl-runtime/src/nodes/std-library/substring.ts#L52) `default: 0` → `-1`. ⚠️ **Two riders were added that the ruling did not name**: the port's `description` currently explains the very mismatch being removed and must be rewritten in the same commit, and 🔴 **`result` must NOT change** — a test watching `result` passes identically before and after and cannot grade this fix | **P18 [§36.2](../phase-18-code-export-v2/EXP-011-PICKER-COVERAGE.md)** | **`Substring`'s property panel says `End = 0` and the node behaves as `End = -1`.** The port *declares* `0`; `initialize` writes `-1`; `registerInput` puts a declared default into `_inputValues` **without calling the setter** (`node.ts:137-139`) and the getter reads `_internal.endIndex`. The two are opposite answers, not a near-miss — `-1` returns the rest of the string, `0` returns `''` for every input. ⚠️ The fix is a **decision**, not a one-liner: aligning the declared default to `-1` changes what the panel shows, and aligning `initialize` to `0` changes what every existing `Substring` returns. 🔴 The export agrees with the **node**, so an export is not a workaround for this | every **author who drops a `Substring` and reads its panel** — the number shown is not the number running |
| DEF-034 | ✅ closed | **DEF-028 §6**, this phase | [**A wire the editor calls merely *questionable* was deleted from the build exactly like a broken one.**](DEF-034-A-QUESTIONABLE-WIRE-IS-DELETED-LIKE-A-BROKEN-ONE.md) `getWarnings` did not filter by level, so **any** warning key made `getConnectionHealth` return `healthy: false` and the export filter dropped the wire. 🔴 **The row named half its population**: `con-target-port-gated` (FB-021) *and* `con-type-unconverted` (FIX-025, Richard's own `string → number` report) are both `level: 'warning'`, and both describe wires that **work**. ✅ **MEASURED, s32** — the open half is closed: **313 wires across 35 projects** were being deleted (290 gated + 23 unconverted), over 179 projects / 72,901 connections, read with the **real** evaluator. 🔴 **27 of the 290 had the GATE PORT ITSELF WIRED** — a `States` node or Component Input feeding `flexDirection`/`useIcon`/`useLabel` inside reusable components (`Pretty button`, `Secondary Button`, `Toggle Switch`), so the condition the editor evaluated against a *saved parameter* was never the condition the running app is in. Verified end-to-end on `LearnBook › Pretty button`. ✅ **Both shipped templates measured 0** (Site Builder 0/412, TPL-001 0/273) — this was **never visible from the templates**. ✅ **FIXED**: `getWarnings(ref, {levels})` — the option this model already speaks in three other methods — and `exportComponent` passes `['error']`. The canvas keeps FB-021's dash (unfiltered). ✅ **AC5 DRIVEN s35** — a copy of `LearnBook` in a real `dev:debug` editor, real `exportToJSON(project,{useBundles:false})`: **211 components, 2,587 connections, and `Pretty button` exported 19 of 19 including `vertical gap -> rowGap`.** The build under the drive was read off `exportComponent.toString()` in the renderer, not assumed. 🔴 **Presence is not proof, so both code paths ran over the same connections in the same editor**: fixed (`{levels:['error']}`) keeps 2,587, pre-fix (no args, what shipped before) keeps 2,577 — **10 wires saved across 9 components, every one `level:'warning'`** (`con-target-port-gated` 5, `con-type-unconverted` 5), and **132 still dropped, every one `level:'error'`** (`con-no-source-port` 101, `con-no-target-port` 31) so **AC2 holds live**. ✅ **Two instruments, one number** — the export reported 2,587 and an independent health loop counted 2,587. ✅ **AC3 seen on canvas**: the `Vertical Gap` wire is still drawn **dashed** while the export keeps it | every **author who wires a gated port** — 35 projects on this machine did, and the value stopped arriving |
| DEF-035 | ✅ **CLOSED s34** | **P77 [D13](../phase-77-the-site-builder-rescue/DEFECTS-THE-SITE-BUILDER-FOUND.md#d13)** second half | [Which ports *exist* depended on when you looked](DEF-035-THE-PORTS-DEPEND-ON-WHEN-YOU-LOOKED.md) — `SchemaHandler._store()` wrote `dbCollections = undefined` on **every** outcome that was not a successful read, and that metadata is on disk, so a window focused while the backend was asleep **deleted the project's `prop-*` ports**. A wire into a port that no longer exists is `con-no-target-port` at **`level: 'error'`** — which DEF-034 established is exactly what still deletes a wire from an export. 🔴 **Measured: 3,783 schema-derived wires across 28 of 118 corpus projects.** ⚠️ Not the 9,051 `prop-*` wires — `Model2` and friends mint theirs from a saved `properties` list and are immune; the prefix overstates the population 2.4×. ✅ **Fixed**: `schemaCachePolicy.ts` splits the fetch into `schema` / `not-applicable` / `unavailable`, and only the first two write; `backend:statusChanged` is now a trigger and the handler fetches on construction. ✅ **AC5 DRIVEN s34** — a copy of `LearnBook` in a real Electron editor, real `exportToJSON` → `exportComponent` → health filter, against the pre-fix build rebuilt from `e5b68d30^`. **Fixed: 2,587 connections before and after `window-focused`, identical, `project.json` byte-identical. Pre-fix: one focus event wrote `dbCollections = undefined` TO DISK and the next export was 2,573 — 14 wires gone, no edit between them, 0 gained.** Both arms carry a known-firing signal (`_fetch` called 2 and 1 times) and the loaded build was read off `_fetch.toString()` rather than assumed. 🔴 **The drive also corrected this row's own blast radius 14×.** 3,783 is the population whose ports the schema *generates*; it is not the population that leaves a build, because `recordWiredFieldPorts` (P77 SBR-008) re-mints a `prop-<field>` port for any field a wire already names — *"the wire is the declaration"* — and **the Record family calls it while the User family does not**. Re-derived: **271 wires in 21 projects actually vanish** (User family), **3,579 are protected** (Record). The corpus predicate predicts 14 for `LearnBook`; the editor lost 14, all `net.noodl.user.*`. The missing producer is registered as **DEF-036** | every **author who exports before the editor has spoken to their backend** |
| DEF-036 | ✅ **CLOSED s41 — part 3 RULED to be the CANVAS, so it is satisfied by what already ships and nothing was built for it (§10); AC1–AC5 BUILT AND DRIVEN s40** — the node now says which of two things is wrong and offers a way out. `fetchBuiltInSchema`'s six-way outcome carries a `cause` CODE (never prose to match) and `SchemaHandler.lastOutcome` RETAINS it — §5 said the sentence AC1 wants was *already computed and thrown away*, and it was. Driven in a real editor on a cold project: a `net.noodl.user.User` node draws **“This project's backend is still starting … no fields to read”**, **0** Add buttons; restoring `dbCollections` live clears the notice and mints **9 `prop-*` ports with no reopen** (AC3); the button reads **“Add a field to `_User`”**, is the top element at its own centre, and opens `backend-schema` with `initialTable`; opened on a real running backend the named row is **expanded with its columns drawn** (AC4). **Control: stale schema + unreachable backend ⇒ 0 notices, 0 buttons** — a working node is not warned about. 🔴 **Two defects found IN THE WORK**: (1) `_active_` is the `backendId` port's DECLARED DEFAULT and means *the project's* backend — `Boolean(getParameter('backendId'))` would have switched this off for every Record node silently, so the rule is now `namesOwnBackend` in the graded module with a control; (2) a SECOND press landed on the FIRST node's table — the surface is reused, new props arrive, `useState` had latched — fixed with `openToken` and re-driven both ways. ⚠️ **Owed**: AC5's RENDERING half is graded by rule only (no reachable `_User` table; `TableRow` imports `Icon` so a spec fails TO RUN), `SignUp` was not the node driven, and no external-backend arm was driven. 🧭 **PART 3 RULED 2026-08-31 (s41): THE CANVAS.** His sentence — *"it would remain … but be a dotted line, and the errors would flag"* — describes three things that already ship, measured s38 and driven s39; the only other reading reverses DEF-034. **Nothing built, DEF-034 stands, the 271 wires still leave the build by design.** 🔴 **Closing does NOT discharge §10's four boundaries** — AC5's rendering half is graded by RULE ONLY, `SignUp` was never the node driven, no external-backend arm was driven, and AC4's `status:'schema'` arm used an INJECTED outcome. Say them whenever the row is quoted. ✅ **s39 DROVE PART 3 AND THE s38 READING HOLDS** (§6): in a real editor on a project whose schema cache is absent from disk, the wire **remains** in `comp.graph.connections`, its own paint records **`setLineDash([5])`**, and it carries `con-no-source-port` / `level:'error'` / `showGlobally`. **Controls both ways** — nine healthy wires in the same paint pass recorded no dash, and restoring the schema turns that same wire solid, clears its warning and drops the error total 178→164. Export by name: **2,587→2,573, 14 lost, 0 gained**. ⚠️ **Two instruments read ZERO IN BOTH ARMS before they were fixed** — a counter keyed on `fromProperty` when `exportConnection` emits `sourcePort`, and a mid-session metadata wipe that does not re-mint ports (the empty arm is only real on a COLD LOAD). ✅ **s39 REWROTE §4 FOR OPTION B** — Option A parked verbatim in §8, marked do-not-build; the old AC3 ignore list survives as the new AC5. 🔴 **s38 MEASURED PART 3 AND IT LARGELY ALREADY SHIPS** (§6): a wire to an absent field ALREADY remains on canvas, is ALREADY dashed (`NodeGraphEditorConnection.ts:960` — the canvas passes NO level filter) and ALREADY errors. **"dropped, not dashed" is HALF right** — dropped from the BUILD only. What is left is a 🧭 **decision** (should it survive the export? that CONTRADICTS DEF-034) — **ask, do not guess**. Remaining value is **parts 1+2**, the missing EXPLANATION. 🔴 **§4's AC1/AC2/AC3 ARE OPTION A AND WERE OVERRULED — rewrite before building (§7).** 🧭 **RULED 2026-08-31: Option B — a wire must NOT declare a column on the accounts table.** Overruled on product shape, not cost: the schema is meant to be thought about first, and a door that mints columns from wires teaches people to skip it. 🔴 **Do not re-open by re-arguing the 271 dropped wires.** New scope instead, three parts: (1) an **"Add a field" button on the data node** jumping straight into the schema editor for the table already chosen in that node's dropdown; (2) **when no backend is attached or it is not running — no button, no fields, an explicit warning saying so** (this is the part that closes the person-sentence, not the button); (3) **a wire to a deleted field stays on canvas, drawn dotted, and errors** — ✅ DEF-034 already ships that dash for *questionable* wires, but these are `con-no-target-port`/`con-no-source-port`, both `error`, so today they are **dropped, not dashed** — that is the gap. ⚠️ Parts 1-2 are **our internal DB only**; do not generalise to external backends without asking. See [RICHARD-RULINGS-2026-08-31.md](RICHARD-RULINGS-2026-08-31.md) | **DEF-035 §6.3** (this phase's own drive) | [The Record family keeps its wires without a schema; the User family does not](DEF-036-THE-USER-FAMILY-HAS-NO-NET.md) — `recordWiredFieldPorts` (`record-ports.ts:312`, built by P77 **SBR-008**) walks a node's own connections and mints `prop-<field>` for every field a wire names that the schema did not cover, typed `'*'`: *"the wire is the declaration."* **The Record family calls it. The User family has no equivalent — and it is two code paths, not one**: `userPropertyPorts` gives up on `if (!ctx.selectedCollection)` (`user-ports.ts:242`, serving `User` and `SetUserProperties`), and `SignUp` has its own inline loop in a different package keyed on a different metadata key (`noodl-viewer-react/…/signup.ts:215`, `systemCollections`). `_store()` writes both keys in one act, so one wipe takes out both. 🔴 **Measured, not derived: 271 wires across 21 of 118 corpus projects** — and in DEF-035's drive **all 14 wires that left the build were `net.noodl.user.*` and no Record wire moved.** ⚠️ **Not DEF-035**: that stopped the cache being wiped over an answer we did not get; this is what happens on a read that is genuinely empty, or before a project's first successful read. 🧭 **A decision, not only a fix** — `recordWiredFieldPorts`'s own docblock states its cost (a mistyped `prop-titel` writes a `titel` column instead of warning), and whether the accounts table should accept that cost is Richard's call | every **app with a sign-up form that writes a custom column** |
| DEF-037 | ✅ **BUILT + DRIVEN s37** — shape ruled by Richard this session (**editor-driven re-render + every `styleTag`'d port**); AC1/2/3/5/6 taken in a real editor, 3 mutants each reddening a different set, all gates exit 0. ⚠️ The deployed-app arm (a **wire** driving a tagged port) is covered by test and construction, **not by a published app**. See §8 — 🧭 **RULED 2026-08-31: Option B, DERIVE it — "the preview is supposed to be a true, live, auto updating view of what's in the node canvas at all times."** Hand-annotating the four ports was **overruled on principle**: a preview that is only sometimes truthful is the defect, and marking ports one at a time is exactly how Checkbox was missed while Radio Button was fixed. 🔴 **AC3 as written now CONTRADICTS the ruling** (*"must not re-render the whole preview on every style change"* was a previous session's constraint, not Richard's) — **rewrite AC3, do not try to satisfy both.** 🧭 Offered but NOT ruled on: *always re-render on an **editor-driven** change, keep the DOM fast path for **runtime-driven** ones (a wire animating opacity per frame)* — that satisfies the ruling without making animation slow, but **ask before relying on it.** **AC4 CLOSED s36.** 🔴 **The mechanism is not "the live-preview path"**: it is `react-component-node.ts` `setStyle()`, the setter generated for **every** `inputCss` port, which force-updates React only for a hard-coded allowlist (`opacity` zero-crossing, margins while size is a percentage, `position`/`flexDirection`/`clip`) and otherwise patches the one declaration onto the DOM node. **A `styleTag` port skips the allowlist entirely.** So a **wire** driving such a port at runtime hits this in a **deployed app** too — §4's "editor feedback-loop, not correctness" reading needs correcting. ✅ **The fix already ships**: `inputCss` ports may carry `onChange`, and **six already use it to force a re-render for exactly this reason** (`flexWrap` on Group — *"scroll direction needs to be recomputed"*; `width`/`height` on Radio Button; `fontSize` on three deprecated controls). 🔴 **Sweep found 3 more unfixed instances**, and the sharpest is an asymmetry between siblings: **Checkbox's `Width`/`Height` are declared identically to Radio Button's** — same index, group, `default: 32`, same `styleTag`, both read at render into a different element's style — **and only Radio Button carries the `onChange`.** Also `borderColor` on Checkbox (derives the tick colour) and on Radio Button. ⚠️ **The three new ones are read from source, not driven.** ⚠️ The port extractor was **wrong on its first run** (dropped the first key of every object literal) and was caught only by two ports known by hand to exist. 🧭 Whether to hand-annotate the four remaining ports is **a decision** | **DEF-031 §6.3** (this phase's own drive) | [A style port whose effect needs a sibling property is inert until the preview reloads](DEF-037-A-STYLE-PORT-THAT-NEEDS-A-SIBLING-IS-INERT-LIVE.md) — setting **Text Overflow → Ellipsis** in a **running preview** puts `text-overflow: ellipsis` on the element and **leaves `white-space: pre-wrap`**, so the label keeps wrapping and the port looks broken; setting it back to **Wrap** changes nothing either, so it looks stuck. **Inert in both directions.** The live path patches the port's own declaration and never runs `Text.tsx`'s render, which is where `white-space`/`overflow`/`overflow-wrap` are derived. ✅ **Controlled**: `wordBreak`, the pre-existing sibling `inputCss` port on the same node set the same way in the same session, **applied live immediately** — so this is **not** "style ports don't update", it is **derivation**. `textOverflow` is the first `Text` style port whose effect depends on siblings the render computes. ⚠️ **Population unswept** — whether any other node has such a port is not measured. ⚠️ A reload renders correctly and the parameter persists, so this is a **feedback-loop** defect, not a correctness one | every **author who toggles Text Overflow and sees nothing happen** |
...
- ✅ **CLOSED s44 as [DEF-045](DEF-045-A-CLOUD-FUNCTION-IN-A-FOLDER-HAS-TWO-ADDRESSES.md) — and 🔴 THE ROW BELOW IS WRONG.**
  A nested function **is** reachable: `POST /functions/<name>` with the slash **percent-encoded** answered
  **200** at HEAD, and every in-product caller encodes. What 404’d is the **raw-slash** address a person or a
  third party types. `newFunctionFromStep.ts` (SB-003) had measured that other half and neither row knew of
  the other — **two half-measurements, each reading as a verdict on the other’s question.** Fixed by widening
  the route to `functions/*name`; DEF-015’s card was right to tick it. *The row as recorded, kept:*

- 🔴 **A cloud function in a FOLDER is declared, listed, ticked on the card — and unreachable over
  HTTP.** Owner: **`NONE`** (was). Found by DEF-015 s11 while measuring an arm for phase 77's SBR-006 AC1.
...
- ✅ **CLOSED s45 as [DEF-047](DEF-047-A-STALE-CLOUD-FUNCTION-RENDERS-TWICE.md)** — the defect held
  exactly as recorded, and 🔴 **the citation did not**: `:123` defines `stale` *from* `backendFunctions`
  rather than filtering it out. 🔴 **“Trivially fixed” was the trap** — the one-line filter is trivial
  and **ungradeable**, which is why a defect somebody had a screenshot of stayed open. Built as one
  row list instead. Was: ⚠️ owner **`NONE`**, found by DEF-015 s12 in the AC2 control frame.
...
- ✅ **CLOSED s44 as [DEF-046](DEF-046-AN-UNCHANGED-VALUE-RE-RUNS-THE-NODE.md)** — the defect held exactly as
  recorded; 🔴 **the SIZING did not.** *“Four lines per family, twelve families”* is **39 call sites across 15
  files in three shapes**, and nine of them are **event handlers with no previous value to compare** — a session
  that treated them as one idiom would have broken the data nodes. Richard ruled **value setters only**.
  *The row as recorded, kept:*

- 🔴 **A value that has not changed re-runs a "Run On Value Change" input.** Owner: **`NONE`** (was).
