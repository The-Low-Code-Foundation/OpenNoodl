...
| DEF-035 | ✅ **BUILT s33** | **P77 [D13](../phase-77-the-site-builder-rescue/DEFECTS-THE-SITE-BUILDER-FOUND.md#d13)** second half | [Which ports *exist* depended on when you looked](DEF-035-THE-PORTS-DEPEND-ON-WHEN-YOU-LOOKED.md) — `SchemaHandler._store()` wrote `dbCollections = undefined` on **every** outcome that was not a successful read, and that metadata is on disk, so a window focused while the backend was asleep **deleted the project's `prop-*` ports**. A wire into a port that no longer exists is `con-no-target-port` at **`level: 'error'`** — which DEF-034 established is exactly what still deletes a wire from an export. 🔴 **Measured: 3,783 schema-derived wires across 28 of 118 corpus projects.** ⚠️ Not the 9,051 `prop-*` wires — `Model2` and friends mint theirs from a saved `properties` list and are immune; the prefix overstates the population 2.4×. ✅ **Fixed**: `schemaCachePolicy.ts` splits the fetch into `schema` / `not-applicable` / `unavailable`, and only the first two write; `backend:statusChanged` is now a trigger and the handler fetches on construction. 🔴 **AC5 owed — undriven**: nobody has diffed two exports of one real project taken with the backend stopped and running. `LearnBook` (174 at-risk wires) is the fixture | every **author who exports before the editor has spoken to their backend** |
