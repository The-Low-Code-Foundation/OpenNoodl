...
| SBR-003 | 🟡 s4: built, swept, driven · **AC5 CLOSED s51** | AC1–4 verified live (floor stamps, overlay beats it, delete falls back). 🟢 **AC5 met s51 — the §2 probe is a GATE**: `sb008` §7, 7 specs, 31/31 EXIT=0. Shipped graph `max-width` **`704px`** / shell **704** in a 1280 frame; `var(--sbr003-no-such-token)` control **`none`** / **1232**, with `--site-measure` DEFINED on `:root` in both arms so the control varies the *reference*, not the token. 🔴 **The READING was not new — s5 took it (see the s5 entry below and `SBR-004` §140–141); §5 of the task file said "owed" and was never updated, and this session read that instead of `TASKS.md`.** What was missing was a standing spec, which is what AC5 asks for. 🔴 **The first run read `none` in BOTH arms and fitted the hunted defect exactly** — a fixture fact: `authorSiteTemplate` skips the editor's design-token install, so only the template's MINTED token was absent while every OVERRIDDEN one still resolved → **[D57](DEFECTS-THE-SITE-BUILDER-FOUND.md#d57)**, owner `NONE`, twelve drives still unswept. Person-sentences complete with SBR-004/006/009 |
...
| SBR-011 | 🟢 **s47: AC1, AC3 and AC4 DRIVEN — all three on the DEPLOYED artefact** | **s47 (2026-09-03), inside SBR-014 run 3.** Two headless Chromes on the deployed folder, one owner, one anonymous with a fresh profile and empty `localStorage`; the anonymous page stamped with a `window` property and a MutationObserver. ✅ **The subscription was proved alive FIRST** — `POST /realtime/subscriptions`, principal `anonymous`, in the backend log for *that* page load beside an open `/realtime` stream — without which every silence below is a fact about a dead subscription (§4's own trap). **AC1**: published the person's way (`More → Publish` → `POST /functions/publishPage`); the anonymous nav **gained the link** with the stamped property **unchanged** — §3's *"no-reload half proven by a stamped window property"*, verbatim. **AC3**: Primary → `#3ba55d`, `Save theme`; the anonymous site's resolved `--primary` moved, **1 transition, same stamp**, and restoring repainted it back. **AC4**: draft edit → **0** changes, published edit → **1**, back to back on one page; the draft's canary never appeared even after the published event, and **exactly one** change per event (§4's cardinality trap held). 🔴 **A trap paid for, and the product was right**: the first attempt published via a bare REST `published: true` and the nav did **not** move — because `publishPage` also writes the `"*": {read: true}` ACL and a bare write does not. `published` is a flag; the **ACL** is the gate. ✅ **Drive the product's own path before calling a difference a defect.** ⬜ **AC2** (section edit updating live) and **AC5** (hub-unreachable degradation) still undriven; **D46**'s drive still owed | ruled BUILD, not strike |
| SBR-012 | ⬜ open | |
...
| SBR-014 | 🟢 **s47: RUN 3 — ALL EIGHT STEPS DRIVEN. AC1, AC2, AC3 ALL MET** | **s47 (2026-09-03) drove steps 7 and 8 against the DEPLOYED ARTEFACT** — the folder `deployToFolder` produces, served statically against the same backend on 8604. Nothing went through the editor. **Step 7**: signed into the deployed `/admin/signin`, the page editor's Title arrived **populated from the stored row**, retitle + `Save page` fired **one `PUT` carrying `title`/`slug`/`seoDescription`/`navOrder`/`showInNav` together**, read back from the row (`seoDescription` still `SEO-CANARY-S45` — **nothing dropped**, the exact loss SBR-008 exists for), and a **separate anonymous Chrome** (fresh profile, empty `localStorage`, verified) showed the new title in the deployed nav. ✅ The deploy's health filter was **proved alive, not merely green** — `--sabotage` drops exactly 1 wire and names it, against `droppedByHealthFilter: 0` on the real deploy. **Step 8, three arcs, each with a firing control**: (a) anonymous sees **2 rows** where the owner sees **4** and gets **404** on a draft id while a published id answers on the same route in the same breath — s44 could not measure this because the DB was empty and refused/empty were one reading; (b) a deploy with `cloudservices` **removed** shows *"No backend connected…"* verbatim, **reachable** per `elementFromPoint`, with the live-backend deploy matching **0** and the instrument proved firing there on another sentence (⚠️ a **re-drive on a new population** — SBR-002 AC4 was already met at s4b in the preview); (c) draft edit → **0 changes**, published edit → **1**, same page, same stamp, no reload. **Collected**: SBR-011 **AC1, AC3 and AC4 all driven** on the deploy, the no-reload half proven by a stamped window property. 🟢 **[D40](DEFECTS-THE-SITE-BUILDER-FOUND.md#d40) DIAGNOSED AND FIXED** — one absent project setting. 🔴 New **[D54](DEFECTS-THE-SITE-BUILDER-FOUND.md#d54)**. ⚠️ **Only Richard's two looks still bound this task.** — s46:  s44 reached step 3 and named SBR-008 AC1; s45 fixed that. Step 3 was **still** blocked by a second closed loop one layer up — `SectionRow`'s script returned before `Outputs.built()` whenever the section had no `data`, which is every section the panel creates. Five sections, five `Save` presses, **0 requests**; the same button fired a `PUT` the moment a `data` object existed. Fixed in the generator as **[D52](DEFECTS-THE-SITE-BUILDER-FOUND.md#d52)**, template gate 62/62. Then **steps 3, 4, 5 and 6 driven**: five kinds authored and read back from the stored rows, drag-reorder persisted, published, public site rendered anonymously, Night themed (preview repaints before save, public site after), contact form submitted by a stranger and read by the owner in Messages. 🔴 **Steps 7 (deploy) and 8 (negative arcs) ⬜ unreached.** AC1 not met, **AC2 met**, **AC3 partly met** — first pictures for steps 4 and 5. Also settled **D40**'s harness-vs-product arm (it is the product) and ran **D38**'s named test (confirmed, **1.42:1**). New: **D53**. |
