...
| SBR-007 | ⬜ open — **unblocked** | the deployed-save half is no longer blocked: SBR-008 landed `a14fb8e7` and s19 drove the same mechanism through the dialog on a no-column site (SBR-006 §5.11). ⚠️ **AC2 (drag to reorder) and AC3 (image upload) are untouched and untested**; AC1's public-site half is the cheapest. 🔴 Do **not** plan AC1 around publish working — **D14**: `publishPage`/`duplicatePage` throw at their first node on a fresh site |
