...
> ✅ **VFN-004 is fixed and driven (2026-08-13, session D).** It had merged as built and was **dead
> in the running editor** — see the row below and [DRIVE-2026-08-13-D.md](DRIVE-2026-08-13-D.md).
> Gates on `cline-dev`: **175 suites / 2622 passing**, `tsc` clean.
>
> 🔴 **The transferable lesson: `ComponentModel.id` is optional and mostly absent on a v1 project.**
> Do not key a session-lifetime lookup on it. Use `componentInstanceId` from
> `models/componentIdentity.ts`, and do not "fix" it by minting ids on load — DSG-007 has a live
> gate forbidding that, for reasons that apply harder to components than to projects.
...
| VFN-004 | [VFN-004-THE-WINDOW-SAYS-WHERE-IT-BELONGS.md](VFN-004-THE-WINDOW-SAYS-WHERE-IT-BELONGS.md) | the tab names the component, and clicking it takes you back to the node | 1A | 2 | ✅ **BUILT, SHIPPED DEAD, FIXED AND DRIVEN 2026-08-13 — AC 1, 2, 3, 4 pass live.** 🔴 **It merged with 52 green specs and did nothing in the running editor**: every layer correct, and the field they all agreed on — `ComponentModel.id` — was `undefined`. **v2 assigns every component a guid (0 of ~180 missing); v1 legacy assigns 2 of 7.** Fixed with `componentInstanceId` (`models/componentIdentity.ts`), a `WeakMap`-held session identity minted lazily and **never serialised** — proved on disk, `project.json` still holds exactly 2 of 7 ids after a full drive. 🔴 **Minting an `id` on load was rejected against a live gate** — DSG-007's *"opening a legacy project mints nothing"*, because two disk copies of one project would diverge and `ProjectMerge` keys on `id ?? name`. Keying on `fullName` rejected: a rename orphans the tab. The away mark is **italic, not colour** (identical `rgb(139,149,161)` in both states), captured both ways. 16 new specs, **7 go red under the pre-fix body**. 🔴 **`ed.selection` does not exist — it is `ed.selector._selected`**, and the non-existent one reads `[]`, which nearly filed a false failure. See [DRIVE-2026-08-13-D.md](DRIVE-2026-08-13-D.md). Owed: AC 5 and AC 6, spec-proved not driven |
...
| VFN-006 | [VFN-006-SHOW-ME-WHAT-I-AM-SAVING.md](VFN-006-SHOW-ME-WHAT-I-AM-SAVING.md) | "5 blocks" becomes *those* five blocks, outlined on the workspace | 5 | 2 | ✅ **BUILT 2026-08-13, NOT DRIVEN** (`vfn-d-overlay`) — criteria 3, 4, 5 closed with controls; 1 and 2 closed at the model and the source, **painting owed to a drive**. 31 specs in `tests-unit/vfn-006`. The outline is a `cloneNode` of each block's own `<path>` appended into its `<g>` — no Blockly API touched, gated by name. 🔴 **A control caught criterion 5 passing vacuously**: Blockly fires events on a `setTimeout`, so a synchronous "no events" assertion measures the event loop, not the code. 🔴 The report's *"5 blocks"* is **6** — `n`, `+` and `2` are three blocks — which is a large part of why the sentence needed something to point at. Also fixed here: `BaseDialog`'s measuring copy is `inert`, closing the double tab stop in **every** dialog in the editor |
