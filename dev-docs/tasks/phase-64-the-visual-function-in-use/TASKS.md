...
| VFN-014 | [VFN-014-THE-CODE-NOBODY-CAN-READ.md](VFN-014-THE-CODE-NOBODY-CAN-READ.md) | *View Code* shows the probe-instrumented build, not the program | live test 2026-08-13 | 2 | 📋 open · ✅ **mechanism pinned**: `generatedCode` is generated inside `withBlockProbes`, which is **scoped** — a clean rendering is one call away. ✅ **criterion 5 REPRODUCED AND CLOSED 2026-08-13: `Outputs["result"]` is the inlined body of saved block `test1` (user backpack), and `generatedCode` is NOT stale — the serious candidate is eliminated.** 🔴 it produced a **new criterion 6**: the code cannot say which lines came from a saved block, which is why `result` was unfindable |
