...
// Re-homed out of packages/noodl-preview so the instrument survives with the verdict.
// HERE stays the preview package: every alias below is relative to it.
const SPIKE_DIR = path.dirname(fileURLToPath(import.meta.url));
const HERE = path.resolve(SPIKE_DIR, '../../../../packages/noodl-preview');
...
await esbuild.build({
  entryPoints: [path.join(SPIKE_DIR, 'hls010-spike.ts')],
  bundle: true,
  platform: 'node',
  target: 'node18',
  format: 'cjs',
  outfile: process.env.SPIKE_OUT ?? path.join(SPIKE_DIR, 'hls010-spike.cjs'),
  sourcemap: false,
