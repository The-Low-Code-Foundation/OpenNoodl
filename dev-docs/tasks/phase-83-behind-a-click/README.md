...
**Status: OPEN. HLS-001 (s2), HLS-002 (s3), HLS-003 (s4) and HLS-004 (s5) built, all 2026-09-09 — see each task's `-WHAT-WAS-BUILT.md`. `nodegx export` exists, both front doors are proved byte-identical, the export reads the graph the author saw, and **the exported app now builds** — `npm install && npm run build`, exit 0, arithmetic right on the rendered page. 4 of 14 built, 17 acceptance criteria closed. HLS-012's replies are drafted and await Richard.** **Prefix: `HLS`.** **Release: ⬜ NOT RULED** (see §2; R2–R5 are now ruled).
