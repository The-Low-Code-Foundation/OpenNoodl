...
| 16 | 176 ledger entries (translated 124, deferred 35, backend-only 16, stubbed 1) — **counted from the JSON**, matching the issue. All 143 picker rows join. Adding `status` to every row costs **+12.9%**; the badge on only the 10 deferred costs **+10.6%**. ✅ **Built reading (s10): +19.4% JSON / +22.0% wire**, on the rule actually shipped (non-translated **or** refuses on a wire — 36 of 143 rows); the badge is 55% of it, and the nine translated-but-refusing rows the strict rule would have dropped cost 654 bytes | `coverage-ledger.json` | FLD-013 |
...
| FLD-013 | 🟢 **BUILT 2026-09-10** — An agent learns what will not translate before it designs. `export: {status, badge, structurePorts, contentPorts}` on `get_node_type` (unconditional, at the **default** detail) and on the `list_node_types` rows that have something to say; `exportCoverage` on the payload so silence is readable. The two defer tables **and** `renderRole`'s type→role switch moved out of `plan.ts` into `structurePorts.ts` — one table, two readers. 11/11 spec, three reverted arms. 🔴 The second arm found the hole: **nine of `Circle`'s thirteen structure ports were graded by no test**, `startAngle` among them — closed by a sweep that reads the table (§6.3). Listing ratchet is new: 47,860 → 58,386 wire bytes, attributed (§6.4) | [#37](https://github.com/The-Low-Code-Foundation/NodeGX/issues/37) | — |
