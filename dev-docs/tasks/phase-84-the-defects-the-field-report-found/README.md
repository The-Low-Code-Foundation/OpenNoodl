...
| 8 | `route.substring(0, route.indexOf('?'))` returns `''` for any route with no query string, so a `viewerpatheq` condition on a plain route can never match. Verified: `'/task-page'` → `''` | `CanvasView.ts:58` | FLD-007 |
...
| FLD-007 | **A lesson step that can be completed** — and a sweep for every other condition the same line broke | [#5](https://github.com/The-Low-Code-Foundation/NodeGX/issues/5) | — |
