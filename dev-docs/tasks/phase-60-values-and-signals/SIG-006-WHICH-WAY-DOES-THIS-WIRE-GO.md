...
✅ **Criterion 1 is now met, behind a setting** — see the Register, **R7**. An endpoint glyph is at
...
turned on by **Always show wire direction**. 🔴 **The reason it is not a new default is redundancy,
not density** — an end is visible on ~92% of the wires on screen. The straight-line model that
justified the original refusal overstated the cost by ~3.5×; the live numbers are in R7.
...
- [x] A committed wire shows its direction **without hovering, without selecting, and without either
      ...
      chevrons, **off by default** and enabled by *Always show wire direction*; the default remains
      endpoint glyphs plus hover. The condition is named rather than glossed — see **R7** for why an
      always-on default was declined on redundancy grounds, and for the measurement.
