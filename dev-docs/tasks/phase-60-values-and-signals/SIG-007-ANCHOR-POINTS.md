...
**Status:** ✅ **built, merged and driven — 9 of 9 acceptance criteria** (see §Acceptance). R2 and R3 are fixed;
R8 and R9 remain open and are Richard's own notes on the built thing.
**Status was:** 📋 open, **§0 answered — build it** · **Track SIG** · ⚠️ **the largest task in the phase**
...
| **R2** | 🔴 **Copy/paste already loses wire labels.** `NodeGraphNodeSet.clone()` rebuilds every connection from four fields, so duplicating a component drops `label` and `labelT` — a live CAN-001/CAN-002 defect, found by tracing the precedent rather than by anyone hitting it. Anchors would go the same way. | ✅ **fixed — verified in source 2026-08-12.** `clone()` carries `label`, `labelT` and a **copied** `route` (`{xs, ys}` sliced, not shared — a pasted wire holding its source's arrays would re-route the original the moment either was dragged). ⚠️ This row said *open* for a day after the fix landed beside it |
...
| **R3** | 🔴 **The AI write path drops per-connection fields silently.** `AUTHORED_CONNECTION_FIELDS` declares four; `connectionSchema` is a plain `z.object`, and zod's default is **strip**. So an agent doing read-modify-write on a component returns it with every wire label gone, and nothing errors. ⚠️ The four-field rule is written down as a decision in `vocabulary.ts`, so correcting it means correcting that text too. **Same shape as P58's `update_node.set.children`.** | ✅ **fixed — both doors, verified by running them 2026-08-12.** `carryConnectionPresentation` restores `label`/`labelT`/`route` from the baseline on the `set` branch (7 specs in `connectionPresentation.test.ts`), and the `operations` branch never lost them because `applyOperations` mutates a copy of the baseline — now pinned by a guard in `operationsWritePath.test.ts` rather than left to inference. **The four-field rule is kept, not widened:** an agent has no business authoring where a wire bends, and the schema cost is paid on every call |
