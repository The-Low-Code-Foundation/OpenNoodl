...
| ~~⚠️ **The contract's `objectId?: string` should widen to `string \| number`**~~ | ✅ **Done** (`d3c9b2c2`), orchestrator. Held back all batch on purpose — it recompiles every consumer including two files owned exclusively by in-flight workers. Cost 10 type errors in two files, each fixed by **widening the parameter rather than casting at the call site**: an id reaching a URL path or a query value is fine as a number. Named `RecordId` so the two traps live in one docblock — **do not coerce the PK** (a foreign key is a number too, so `String(article.author_id) === author.objectId` would hold while `author.objectId === article.author_id` did not), and **`Model.get` keys a plain object**, which is the accident that made the mixture harmless and is load-bearing |
| ~~**The four `noodl.byob.*` types**~~ | ✅ **Deleted** (`8eed8141`). ⚠️ **The catalog fallout took three passes and reproduced the register's own warning without anyone touching a catalog by hand**: `catalog:generate` succeeded at 151 types while `catalog:merge` failed on three dangling `relatedNodes`; fixing those it failed again on an *example* demonstrating the deleted types. `cloud-byob-crud` was **retargeted onto the merged family rather than deleted**, every port name taken from step 6's measured driver — guessing is how that very example carried a wrong `class` parameter for months, since the validator skips port checks on dynamic-port nodes |
| ⚠️ **`byob-utils.ts` and its test survive the family they served.** Nothing in the product imports them now, but the file holds real logic — `pickTotalCount`, `buildFieldsParam`, `buildEndpoint`/`buildUrl`/`buildHeaders` — whose **only** coverage is that test, and whose replacements inside `RestDataAdapter` have none of their own. Deleting both would have removed coverage silently. **Kept deliberately; wants its coverage repointed at the real homes** | **unowned — named follow-up, not a silent leftover** |
| ⚠️ **`cloud-library:check` was RED on `cline-dev` and is in CI** — found by BCN-010, not caused by it. Regenerating recovered **2 missing node types and ports on 22 nodes**, including BCN-008's realtime outputs. **It was not in the gate list this phase had been running** | ✅ green now; **add it to the standing gate list** |
| ⚠️ **`RequestMagicLink` and `SignInWith` were registered, not deprecated, and absent from the picker index** — unreachable for an app author. The same silent gap the BYOB family was retired to avoid | ✅ fixed with the deletions (`8eed8141`) |
| ⚠️ **No `auth.*` probe branch exists**, so eleven `conditional` auth cells are permanently closed. Correct today, **wrong the day someone implements Supabase auth** | **unowned — BCN-010 residual** |
...
| ~~The contract's `objectId?: string` should widen to `string \| number`~~ — Directus and PostgREST hand back integers, and coercing only the PK breaks `author.objectId === article.author_id`. `CloudStore._fromJSON`'s declaration said `string` while the value is a number | ✅ **Done** (`d3c9b2c2`) — see the closed row above |
...
| ~~⚠️ **`emailVerified` is HALF closed.** A new sign-up now stores `false`, but a **pre-existing user still reads `undefined`**, because `nodegx-backend`'s password signup never writes the column. One line, outside BCN-006's territory~~ | ✅ **Closed at the backend** by BCN-006 step 5/6 — see the row above |
...
| ~~**BCN-005's authoritative relation parsers have no caller** until the editor's schema sync stores their output; what runs today is the strict subset derivable from the cached schema. Exact call site in [BCN-005-NOTES.md](./BCN-005-NOTES.md) §5~~ | ✅ **Closed** by the editor schema sync + `f33394ea` — see the rows above |
...
| ~~**The repeater binding with a numeric `objectId`** — the one third of the `objectId` risk a headless driver cannot reach~~ | ✅ **Closed** — rendered in a real deploy bundle, orchestrator pass |
