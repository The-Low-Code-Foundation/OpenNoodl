...
| **[NAT-006](NAT-006-THE-API-THE-EDITOR-CANNOT-SEE.md)** ⭐🟡 | **The API the editor cannot see.** 19 pages, 15 routes, **zero** endpoints for people, profiles, RFPs, coaching, University, tutorials or replays. The reason the scope looked thin | platform | **2** | L | 🟡 **READ SURFACE DONE 08-19 s6** — `126a0b6`, 10 endpoints, AC1/2/3/4/6/7 closed. **AC5 = the writes, and it waits on D5** (a ruling, not code). 🔴 Found: returning the read modules' own types LEAKS an internal account id no page renders — and a `Date`-typed column arrives as a raw string on some pooled connections |
...
| **[NAT-007](NAT-007-A-THREAD-YOU-CAN-ACTUALLY-ANSWER.md)** ⭐🟡 | **A thread you can actually answer.** Open it, read every post, render its attachments, **reply** — the flagship. The platform half already exists; this is an editor task | editor / core-ui | **3** | L | 🟡 **READING HALF DONE 08-19 s8** — `2d3960a7`, AC1/2/3/8 closed, AC5 half. Opens in place on both surfaces and was **driven against the live platform** (which serves an older build — NAT-006's endpoints 404 there). 🔴 **AC4/AC6 are writes and wait on D5** — and the platform *already accepts* the editor's token, so D5 is a consent question, not a capability one. 🔴 Found: `externalId` was never sent by the platform, so every click opened `/bench/undefined`; the ports render **twice**, on **both** clients |
...
`nodegx-community/docs/API.md`. 🔴 **The five Tier-3 tasks are unblocked for everything except
writing** — the reply, the RFP response and the coaching request are AC5, they are specified in
that document, and they wait on **D5**. Anybody starting NAT-007..011 should read §3 and §4 of the
...
Tier 3 is four independent surfaces plus the flagship; **NAT-007 went first** (08-19 s8) and the
other three copy its shapes — the row, the four states, the post-body renderer, the six-state
...
surface's view model. 🔴 **Read NAT-007 §Status before starting NAT-008/009/010/011**: it names
