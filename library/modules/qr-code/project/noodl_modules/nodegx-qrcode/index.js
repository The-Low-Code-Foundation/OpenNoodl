/* NodeGX QR Code module — hand-authored (LIB-003 module expansion).
   Bundles: qrcode-generator (MIT, Kazuhiko Arase) + a Noodl SDK shim.
   No dependencies, no network, no API keys. */

// ── Vendored: qrcode-generator (MIT) ─────────────────────────────────────────
//---------------------------------------------------------------------
//
...
/* ── QR Code node ──────────────────────────────────────────────────────────────
 *
 * A pure client-side QR renderer. Uses the vendored `qrcode-generator`
 * (MIT, Kazuhiko Arase) bundled above — no network, no API keys, no backend.
 *
 * The node builds an SVG from the QR module matrix so the foreground/background
 * colours and size are all styleable, then renders it inline (scales crisply to
 * any node size). Follows the chart-js `Noodl.defineModule` pattern; the node is
 * a React node created with the SDK shim's `Noodl.defineReactNode`.
 */
(function () {
  if (typeof Noodl === 'undefined' || typeof Noodl.defineReactNode !== 'function') return;

  function buildQrSvg(text, ecLevel, margin, fg, bg) {
    ...
    for (var row = 0; row < count; row++) {
      for (var col = 0; col < count; col++) {
        if (qr.isDark(row, col)) {
          rects += '<rect x="' + (col + margin) + '" y="' + (row + margin) + '" width="1.02" height="1.02"/>';
        }
      }
    }
    return (
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + size + ' ' + size + '" ' +
      'shape-rendering="crispEdges" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">' +
  ...
  function QRCodeComponent(props) {
    var style = Object.assign({ display: 'inline-block', lineHeight: 0 }, props.style);
    var s = props.size || 256;
    style.width = props.style && props.style.width ? props.style.width : s;
    style.height = props.style && props.style.height ? props.style.height : s;

    var margin = props.margin === undefined || props.margin === null ? 4 : props.margin;
    var fg = props.foreground || '#000000';
    ...
    if (text) {
      try {
        svg = buildQrSvg(text, props.ecLevel, margin, fg, bg);
      } catch (e) {
        svg = null;
      }
    }
    ...
    return React.createElement('div', {
      ...
      style: style,
      ref: props.noodlNode ? undefined : undefined,
      dangerouslySetInnerHTML: svg ? { __html: svg } : undefined
    });
  }

  var qrNode = Noodl.defineReactNode({
    name: 'nodegx.qrcode',
    displayName: 'QR Code',
    category: 'Visual',
    docs: 'https://docs.noodl.net/#/modules/qr-code/README.md',
    ...
    inputProps: {
      text: {
        displayName: 'Text',
        group: 'QR Code',
        type: { name: 'string', allowConnectionsOnly: false },
        default: ''
      },
      ecLevel: {
