...
Legacy dropdown visual, hidden from the node picker — superseded by net.noodl.controls.options (Dropdown).

The original Options (select) visual, kept only so old projects load. The Dropdown control (net.noodl.controls.options) supersedes it with items/value binding, visual states and accessibility.

## When to use it

Do not use in new graphs — use net.noodl.controls.options instead.
...
[Dropdown](./net-noodl-controls-options.md)
