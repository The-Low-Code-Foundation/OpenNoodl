...
Legacy fieldset grouping container, hidden from the picker. Use a plain Group (or Form for submit semantics).

Field Set renders an HTML fieldset-style container for grouping form controls. It is hidden from the node picker; a plain Group provides the same layout role, with CSS Definition covering fieldset-specific styling if genuinely needed.

## When to use it

Do not use in new graphs — use Group instead.
...
| `flexDirection` | Enum (`column`, `row`) | `column` | — |
