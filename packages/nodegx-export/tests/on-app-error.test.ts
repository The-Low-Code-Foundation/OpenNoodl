...
describe('§H the ledger', () => {
  test('On App Error is translated, carries no badge, and the floor is 96', () => {
    ...
    expect(ledger.pickerCoverageFloor).toBe(108); // §61 the component-stack trio (session 86) // §57 Repeater Item, §58 the streaming trio, §59 Hash / Random Bytes / Screen Resolution (session 85) // §56 Filter Records (session 84)
  });
});
