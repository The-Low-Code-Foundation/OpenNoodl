...
describe('§A the fixture, whole — a Record feeds the target, a Variable the record, two buttons the two Dos', () => {
  ...
  test('A10 the ledger rows moved, so the two picker cards carry no badge; the floor is 111', () => {
    ...
    expect(ledger.pickerCoverageFloor).toBe(117); // §66 Subscribe To Changes (session 90) on top of §65 WebSocket (session 89) on top of §64 Server-Sent Events (session 88) on top of §62 the relation pair + §61 the component-stack trio + §60 + §63 (session 86)
  });
});
