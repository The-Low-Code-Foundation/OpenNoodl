...
describe('§A the fixture, whole — two stacks, a tab bar in Replace mode, a wizard with a push and two pops', () => {
  ...
  test('A11 the ledger rows moved, so the three picker cards carry no badge; the floor is 108', () => {
    ...
    expect(ledger.pickerCoverageFloor).toBe(108); // §61 the component-stack trio (session 86)
    expect(String(ledger.$pickerCoverageFloorComment)).toContain('108 after Tier 2.8 row 11');
  });
});
