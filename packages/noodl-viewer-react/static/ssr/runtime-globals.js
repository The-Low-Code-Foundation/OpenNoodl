...
function installRuntimeGlobals({ React, ReactDOMServer, XMLHttpRequest, fetch }) {
  ...
  globalThis.Noodl = {
    defineModule: function (m) {
      // CN-003: adopt the manifest name the injector set just before this kit's
      // script. Must happen here, not in `registerModule` — see the viewer's
      // copy of this bootstrap for why.
      if (m && !m.name && globalThis.__noodl_module_name) m.name = globalThis.__noodl_module_name;
