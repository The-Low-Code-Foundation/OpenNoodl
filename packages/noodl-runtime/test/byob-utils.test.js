...
// ⚠️ Two describes lived here — `parseFilterForConnectedPorts` and `toDirectusFilter`,
// both reached through `byob-query-data`. That module is deleted (BCN-004 step 7, landed
// by BCN-010), and with it the last legacy copy of the Directus filter translator. The
// surviving translator is `@noodl/backend-contract`'s `src/translators/directus.ts`,
// which `tests/translators.test.ts` and `tests/relations.test.ts` cover far more
// thoroughly than these two did — including the operators BCN-003 found `convertFilterOp`
