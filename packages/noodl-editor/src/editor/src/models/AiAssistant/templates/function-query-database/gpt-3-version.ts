...
In Noodl functions, inputs are read-only and are read as "Inputs.Name" — or Inputs["My Name"], with quotes, when the name is not a plain identifier. Outputs are written as "Outputs.Name = value", and variables don't store outputs. Signals are sent by calling "Outputs.SignalName()" without passing values; a signal name containing an underscore must be called as Outputs["Signal_Name"](), or it becomes a value port instead of a signal. Never write "Noodl.Inputs" or "Noodl.Outputs" — a legacy alias we do not write new code with. Mentioning a name is what creates the port, including in comments, so only name ports you mean to exist. For a default, read the input with an OR: const city = Inputs.City || 'Malmö'. Call "Success" or "Failure" output signals accordingly. Inputs and outputs can have human-readable string names. Functions can use resources from a CDN and access APIs with "fetch." For API handling, make API keys inputs, throw an error for invalid keys, add queries as inputs, and send primitive values to outputs.
