...
import { AssistantHandoff, IMPORT_REPORT_FORMAT_VERSION, ImportReport, LegacyFinding, LegacyOutcome } from './types';
...
export function reportSummaryLine(report: ImportReport): string {
  ...
  if (verdict.recommendation === 'proceed') {
    return counts['converted-with-changes'] > 0
      ? `Imported. ${counts['converted-with-changes']} construct${
          counts['converted-with-changes'] === 1 ? ' was' : 's were'
        } rewritten — see the import report.`
  ...
  return `Imported. ${verdict.unconvertedCount} construct${
    verdict.unconvertedCount === 1 ? '' : 's'
  } could not be converted and ${
    verdict.unconvertedCount === 1 ? 'is' : 'are'
  } marked as errors on the canvas — see the import report.`;
}

// ─── The assistant rendering ─────────────────────────────────────────────────

/** Entries an assistant is asked to act on. Notes are not action items. */
const ACTIONABLE: ReadonlySet<LegacyOutcome> = new Set<LegacyOutcome>(['placeholder', 'dropped']);

/** How many entries the handout spells out before it summarises the rest. */
const MAX_ASSISTANT_ENTRIES = 25;

/**
 * The report as an authoring-loop handout.
 *
 * Not the Markdown: that is written for a person reading a file, it repeats the
 * verdict prose, and it costs several times the tokens. This is the same object
 * rendered for an agent that is about to propose a repair — actionable entries
 * first, each with the catalog types it will need to look up, and nothing it
 * cannot act on.
 *
 * Returns `undefined` when there is nothing to say, so the prompt omits the
 * block entirely rather than carrying an empty heading — the same
...
export function renderReportForAssistant(report: ImportReport): string | undefined {
  ...
  const lines: string[] = [];
  lines.push(
    `This project was imported from a legacy Noodl project (${report.source.projectName ?? report.source.dir}).`
  );
  lines.push(
    `${actionable.length} construct${actionable.length === 1 ? '' : 's'} could not be converted. Recommendation: ${
  ...
  const shown = actionable.slice(0, MAX_ASSISTANT_ENTRIES);
  for (const finding of shown) {
    const parts: string[] = [`- [${finding.id}] ${finding.outcome} — ${finding.original}`];
    if (finding.location?.component) {
      parts.push(`in ${finding.location.component}`);
    }
    if (finding.location?.nodeLabel) {
      parts.push(`node "${finding.location.nodeLabel}"`);
    }
    lines.push(parts.join(' | '));
    lines.push(`    ${finding.message}`);
    if (finding.equivalents.length > 0) {
      lines.push(`    consider: ${finding.equivalents.join(', ')}`);
    }
    if (finding.portChanges && finding.portChanges.length > 0) {
      lines.push(`    ports differ: ${finding.portChanges.map((p) => `${p.from}->${p.to}`).join(', ')}`);
    }
  ...
  if (actionable.length > shown.length) {
    lines.push(
      `- … ${actionable.length - shown.length} more entries in the project's import-report.json, not repeated here.`
    );
  }
...
export function renderReportMarkdown(report: ImportReport): string {
  ...
  lines.push('');
  lines.push(
    `Imported from \`${report.source.dir}\`${report.source.projectName ? ` ("${report.source.projectName}")` : ''} on ${
      report.generatedAt
    }.`
  );
  lines.push('');
  ...
  lines.push('');
  lines.push(
    `${report.coverage.findingsEmitted} entr${
      report.coverage.findingsEmitted === 1 ? 'y' : 'ies'
    } below. The remaining ${report.coverage.silentlyConverted} construct${
      report.coverage.silentlyConverted === 1 ? '' : 's'
    } used current node types and converted without comment — they are counted, not listed.`
  );
  lines.push('');
  ...
  if (report.handoff.repairable.length > 0) {
    lines.push(
      `**Repairable (${report.handoff.repairable.length}):** ${report.handoff.repairable
        .map((id) => `\`${id}\``)
        .join(', ')}`
    );
    lines.push('');
  ...
  if (report.handoff.unrepairable.length > 0) {
    lines.push(
      `**No known equivalent (${report.handoff.unrepairable.length}):** ${report.handoff.unrepairable
        .map((id) => `\`${id}\``)
        .join(', ')} — these are rebuilds, not repairs.`
    );
    lines.push('');
  }
  for (const instruction of report.handoff.instructions) {
