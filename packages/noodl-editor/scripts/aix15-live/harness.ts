 ...
 *
 * Flags:
 *   --mode=update|agentic|plan|scope|review|explain|changeset   (required)
...
// AIX-003's round trip (mode `changeset`) — the review chain downstream of a
// live proposal. Kept in its own import block so it can be read as the addition
// it is.
import {
  buildChangeSet,
  excludedWith,
...
const MODES = ['update', 'agentic', 'plan', 'scope', 'review', 'explain', 'changeset'] as const;
