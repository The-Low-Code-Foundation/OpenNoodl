 ...
 * and output signal"**. It does not, and it was verified in source before the
 * copy was written — `Node.Signals.X = function(){}` mints an unlimited number
 * of signal inputs and `Outputs.X()` an unlimited number of signal outputs
 * (`javascriptnodeparser.js`), both *additional* to the built-in `Run` and
...
describe('LGC-001 §2 — every card carries a sentence and a worked example', () => {
  ...
  it('says the other two run on a signal', () => {
    ...
    expect(getChooserNote('JavaScriptFunction').signals.toLowerCase()).toContain('send it a signal');
  });
});
...
describe('LGC-001 §2 — the Function card must not assert a limit the node does not have', () => {
  ...
  it('states that a Function can fire several signals when it is done', () => {
    ...
    expect(signals).toContain('send it a signal');
   ...
   */
  it('nowhere claims one input signal and one output signal', () => {
   ...
   */
  it("the prohibition's patterns match the sentence they forbid", () => {
    const wrong = 'Runs when you send it a signal. It has only one input signal and one output signal.';
    expect(/only one (input )?signal/.test(wrong.toLowerCase())).toBe(true);
  });
});
