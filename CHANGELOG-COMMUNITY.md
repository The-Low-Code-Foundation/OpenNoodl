...
### 💸 The AI got 6× cheaper and 6× faster — and we fixed our own maths
Asking the agent to build a component used to cost about 20¢ and take a minute and a half. It now costs **3.5¢ and takes 16 seconds**, and it gets the component *right first time more often than before*, not less. Nothing was cut from what the agent is told — the same project overview, the same node catalog, the same design-system vocabulary.

Three things did it:

- **We stopped paying for the same words twice.** Every turn of a conversation re-sends everything before it. The reference material the agent reads is identical for every component in a project, so it's now sent once and re-read from a cache at a tenth of the price — which meant reordering each request to put the unchanging material first and *your actual request last*. Roughly three-quarters of everything we now send is served from that cache, and there's a check in the test harness that fails loudly if it ever silently stops working.
- **We told it to think less, and it got better.** The model was running at its deepest reasoning setting because nobody had chosen otherwise. Dialling that down cut cost by another two-thirds — and *fixed* the one prompt in our test set that had stopped working entirely. That component ("a scrolling list of articles") had been spending 32,000 words of internal reasoning across seven attempts and never actually building anything. At the lower setting it thinks briefly, builds it, and passes on the first try. Extra deliberation wasn't buying quality; it was preventing action.
- **We changed the default model on evidence.** Two models tied on quality across the whole test set, so we defaulted to the one that's 2.6× cheaper. The pricier one is still one click away in settings.

**The unglamorous part matters most.** Before any of this, we fixed how we *count*. Cached text is billed at a different rate, and our cost readout couldn't represent that at all — so adding caching would have made our own reporting wrong in the flattering direction, quietly congratulating us for a saving partly invented by the accounting. Counting first meant every number above was measured by an instrument we'd already fixed. We also corrected a stale price in the model registry, and deliberately re-priced the "before" figure with it so the correction can't be mistaken for a saving. One honest gap remains: we've checked our arithmetic against the provider's reported usage, but not yet against an actual invoice.

Two bugs surfaced on the way and were fixed: a malformed request our own new code could have produced (caught by a test, never by a live run), and cancelled builds being reported as "gave up" instead of "cancelled" — which also wasted an extra API call each time.
